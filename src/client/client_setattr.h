 ...
 * and it is opened with WRITE intent: truncate(2) by path requires write
 * permission on the file (unlike ftruncate, whose rights were bound at its own
 * open), and the open gate is where every backend -- engine-gated and
 * remote-DAC proxy alike -- enforces exactly that.  The wire SETATTR cannot
 * make the distinction (which is why servers owner-override size changes, as
 * nfsd does); the client must. */
...
chimera_setattr_open_flags(struct chimera_client_request *request)
{
    if (request->setattr.set_attr.va_set_mask & CHIMERA_VFS_ATTR_SIZE) {
        return CHIMERA_VFS_OPEN_INFERRED | CHIMERA_VFS_OPEN_WRITE_ONLY;
    }
