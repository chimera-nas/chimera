    ...
    struct chimera_client_request  *request)
{
    ...
    int                          lookup = chimera_vfs_compound_add_lookup(compound, request->remove.path,
                                                                          request->remove.path_len, CHIMERA_VFS_ATTR_FH
    ...
    chimera_vfs_compound_add_puthandle(compound, parent_handle, CHIMERA_VFS_OPEN_INFERRED);
    int remove = chimera_vfs_compound_add_remove(compound, request->remove.path,
                                                 request->remove.path_len, request->remove.
                                                 flags);
    if (remove >= 0) {
