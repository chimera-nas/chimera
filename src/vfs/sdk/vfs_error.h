...
enum chimera_vfs_error {
    ...
    CHIMERA_VFS_ECOMPOUND_CONFLICT  = 201,  /* Compound wait-die abort / optimistic
                                             * commit conflict; the caller must abort the
                                             * compound and replay the whole sequence
                                             * from the top (same wait-die ts).
                                             * Deliberately outside the
                                             * errno range so it never collides with a
                                             * protocol status map (the retry loop must
                                             * consume it; it must never reach a client). */
    CHIMERA_VFS_ECOMPOUND_EXHAUSTED = 202,  /* Compound replay ruled out: the
                                             * compound driver gave up after too many
                                             * consecutive ECOMPOUND_CONFLICT replays,
                                             * or an op was ejected so the compound
                                             * must not be replayed.
                                             * Retriable at the protocol layer
                                             * (NFS3ERR_JUKEBOX on the wire, EAGAIN at
                                             * the POSIX layer), unlike a real EIO.
                                             * Deliberately outside the errno range,
                                             * adjacent to ECOMPOUND_CONFLICT. */
