...
enum chimera_vfs_error {
    ...
    CHIMERA_VFS_ETXN_CONFLICT  = 201,  /* Transaction wait-die abort / optimistic
                                        * commit conflict; the caller must abort the
                                        * transaction and retry the whole sequence
                                        * from the top.  Deliberately outside the
                                        * errno range so it never collides with a
                                        * protocol status map (the retry loop must
                                        * consume it; it must never reach a client). */
    CHIMERA_VFS_ETXN_EXHAUSTED = 202,  /* Transaction retry budget exhausted: the
                                        * txn driver gave up after too many
                                        * consecutive ETXN_CONFLICT replays.
