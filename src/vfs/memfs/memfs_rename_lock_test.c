...
static evpl_mutex_t      observation_lock = EVPL_MUTEX_INITIALIZER;
static evpl_cond_t       observation_cond = PTHREAD_COND_INITIALIZER;
static evpl_mutex_t     *watched_mutex;
static int               observed;
static atomic_int        rendezvous_moves;
static pthread_barrier_t moves_barrier;

static int test_mutex_trylock(
    evpl_mutex_t *mutex);
static int test_rwlock_wrlock(
    pthread_rwlock_t *lock);
#define chimera_mutex_trylock test_mutex_trylock
#define pthread_rwlock_wrlock test_rwlock_wrlock
#define vfs_memfs             test_vfs_memfs
#include "memfs.c"
#undef vfs_memfs
#undef chimera_mutex_trylock
...
static int
test_mutex_trylock(evpl_mutex_t *mutex)
{
    int result = chimera_mutex_trylock(mutex);

    if (result == EBUSY && mutex == watched_mutex) {
        evpl_mutex_lock(&observation_lock);
        observed = 1;
        evpl_cond_signal(&observation_cond);
        evpl_mutex_unlock(&observation_lock);
    }
    return result;
} /* test_mutex_trylock */
...
{
    if (atomic_load(&rendezvous_moves)) {
        pthread_barrier_wait(&moves_barrier);
    }
...
wait_contention(void)
{
    ...
    evpl_mutex_lock(&observation_lock);
    while (!observed) {
        assert(!chimera_cond_timedwait(&observation_cond, &observation_lock, &deadline));
    }
    evpl_mutex_unlock(&observation_lock);
} /* wait_contention */

static void
watch_contention(evpl_mutex_t *mutex)
{
    watched_mutex = mutex;
    observed      = 0;
} /* watch_contention */
...
check_existing_cases(void)
{
    ...
    for (unsigned i = 1; i < 5; i++) {
        ...
        evpl_mutex_init(&nodes[i].lock, NULL);
    ...
    evpl_mutex_lock(&nodes[2].lock);
    memfs_rename_at(&thread, &fs, request, NULL);
    assert(completions == 2 && request->status == CHIMERA_VFS_EINVAL);
    evpl_mutex_unlock(&nodes[2].lock);
    free(renamed);
    free(parent);
    ...
    for (unsigned i = 1; i < 5; i++) {
        evpl_mutex_destroy(&nodes[i].lock);
    }
    free(request);
    ...
    const char            *name)
{
    ...
    struct memfs_dirent *entry = memfs_dirent_alloc(&f->thread, n->inum, n->gen,
                                                    S_ISDIR(n->mode), chimera_vfs_hash(name, strlen(name)), name, strlen
                                                        (name));
...
fixture_free(struct rename_fixture *f)
{
    for (unsigned i = 1; i < 9; i++) {
        ...
        evpl_mutex_destroy(&f->nodes[i].lock);
    }
...
check_ancestor_contention(unsigned change)
{
    ...
    evpl_mutex_lock(&f.nodes[2].lock);
    ...
    evpl_mutex_lock(&f.nodes[3].lock);
    evpl_mutex_lock(&f.nodes[4].lock);
    if (change == 1) {
    ...
    evpl_mutex_unlock(&f.nodes[4].lock);
    evpl_mutex_unlock(&f.nodes[3].lock);
    evpl_mutex_unlock(&f.nodes[2].lock);
...
check_regular_parallel(void)
{
    ...
    evpl_mutex_lock(&f.nodes[7].lock);
    ...
    evpl_mutex_unlock(&f.nodes[7].lock);
    assert(!pthread_join(worker, NULL));
    watched_mutex = NULL;
...
check_replacement_contention(void)
{
    ...
    evpl_mutex_lock(&f.nodes[8].lock);
    assert(!pthread_create(&worker, NULL, worker_run, &w));
    wait_contention();
    evpl_mutex_lock(&f.nodes[2].lock);
    evpl_mutex_lock(&f.nodes[7].lock); /* source child must also be released */
    evpl_mutex_unlock(&f.nodes[7].lock);
    evpl_mutex_unlock(&f.nodes[2].lock);
    evpl_mutex_unlock(&f.nodes[8].lock);
    assert(!pthread_join(worker, NULL));
    watched_mutex = NULL;
    assert(w.request->status == CHIMERA_VFS_OK);
