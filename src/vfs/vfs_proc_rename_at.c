    ...
    void                            *private_data)
{
    ...
    if (match_source_fh_len) memcpy(request->rename_at.match_source_fh, match_source_fh, match_source_fh_len);
    ...
    void                            *private_data)
{
    ...
    if (outcome) *outcome = CHIMERA_VFS_RENAME_OUTCOME_UNKNOWN;
    ...
    uint64_t required = 0;
    if (flags & CHIMERA_VFS_RENAME_NOREPLACE) required |= CHIMERA_VFS_CAP_RENAME_NOREPLACE;
    if (flags & CHIMERA_VFS_RENAME_MATCH_SOURCE_FH) required |= CHIMERA_VFS_CAP_RENAME_MATCH_FH;
    if (flags & CHIMERA_VFS_RENAME_MATCH_DEST_FH) required |= CHIMERA_VFS_CAP_RENAME_MATCH_DEST_FH;
    ...
    if (module && chimera_vfs_gate_needed_dac(module->capabilities, cred)) {
        ...
        if (gate->src_child_fh_len) memcpy(gate->src_child_fh, match_source_fh, gate->src_child_fh_len);
    ...
    chimera_vfs_rename_at_dispatch(thread, cred, fh, fhlen, name, namelen,
                                   ...
                                   op_handle, actor, match_source_fh, match_source_fh_len, outcome, callback, private_data);
