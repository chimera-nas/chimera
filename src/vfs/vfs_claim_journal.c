...
static bool
exact_overlap(
    const struct chimera_vfs_claim_exact_range *a,
    const struct chimera_vfs_claim_exact_range *b,
    bool                                        zero_point)
{
...
static bool
record_removed(
    const struct chimera_vfs_claim_journal *j,
    const struct exact_record              *record)
{
...
chimera_vfs_claim_owner_create(
    struct chimera_vfs_file_state    *file,
...
SYMBOL_EXPORT struct chimera_vfs_claim_owner *
chimera_vfs_claim_owner_create_compat(
    struct chimera_vfs_file_state    *file,
...
chimera_vfs_claim_owner_has_locks(struct chimera_vfs_claim_owner *owner)
{
    ...
    return held;
} /* chimera_vfs_claim_owner_has_locks */
...
chimera_vfs_claim_owner_is_retired(struct chimera_vfs_claim_owner *owner)
{
    ...
    evpl_mutex_unlock(&owner->lock);
    return retired;
} /* chimera_vfs_claim_owner_is_retired */

SYMBOL_EXPORT bool
chimera_vfs_claim_owner_retire(
    struct chimera_vfs_claim_owner *owner,
    void (                         *complete )(void *),
    void                           *private_data)
{
...
chimera_vfs_claim_journal_alloc(
    uint32_t max_owners,
...
chimera_vfs_claim_journal_bind(
    struct chimera_vfs_claim_journal *j,
    struct chimera_vfs_claim_owner   *owner)
{
    if (!j || !owner || j->phase != JOURNAL_ACTIVE) {
        return CHIMERA_VFS_EINVAL;
    }
    bool bound = false;
    for (uint32_t i = 0; i < j->num_owners; i++) {
        bound |= j->owners[i] == owner;
    }
    evpl_mutex_lock(&owner->lock);
...
} /* journal_release_owner */

SYMBOL_EXPORT bool
chimera_vfs_claim_journal_unbind_idle(
    struct chimera_vfs_claim_journal *j,
    struct chimera_vfs_claim_owner   *owner)
{
    if (!j || !owner || j->phase != JOURNAL_ACTIVE) {
    ...
    for (uint32_t i = 0; i < j->num_retire_owners; i++) {
        if (j->retire_owners[i].owner == owner) {
            return false;
        }
    }
    for (uint32_t i = 0; i < j->num_changes; i++) {
        if (j->changes[i].owner == owner) {
            return false;
        }
    }
    uint32_t slot = 0;
    while (slot < j->num_owners && j->owners[slot] != owner) {
        slot++;
    }
    if (slot == j->num_owners) {
        return false;
    }
    ...
    uint32_t                          count)
{
    ...
    if (!changes || !excluded) {
        free(changes); free(excluded); return false;
    }
...
SYMBOL_EXPORT enum chimera_vfs_error
chimera_vfs_claim_journal_retire_owner(
    struct chimera_vfs_claim_journal *j,
    struct chimera_vfs_claim_owner   *owner)
{
    if (!j || !owner || j->phase != JOURNAL_ACTIVE) {
    ...
    if (owner->retiring) {
        ...
        * must never wait holding another owner's retirement reservation. */
        ...
        for (struct exact_record *r = owner->records; r; r = r->next) {
            ...
            if (!r->removed_by && ++count > CHIMERA_VFS_CLAIM_RETIRE_MAX_RECORDS) {
                break;
            }
        }
        if (!journal_retire_capacity(j, count)) {
            status = CHIMERA_VFS_ENOSPC;
        }
    }
    if (status == CHIMERA_VFS_OK) {
...
} /* chimera_vfs_claim_journal_retire_checkpoint */

SYMBOL_EXPORT void
chimera_vfs_claim_journal_retire_rewind(
    struct chimera_vfs_claim_journal *j,
    uint32_t                          checkpoint)
{
    ...
    uint32_t                          first)
{
    while (j->num_changes > first) {
        struct exact_change *change = &j->changes[--j->num_changes];
        assert(change->add);
        evpl_mutex_lock(&change->owner->file->lock);
        record_unlink_claims(change->record);
        evpl_mutex_unlock(&change->owner->file->lock);
...
SYMBOL_EXPORT void
chimera_vfs_claim_journal_acquire(
    struct chimera_vfs_claim_journal           *j,
    struct chimera_vfs_claim_owner             *owner,
    ...
    struct chimera_vfs_claim_batch_result      *result)
{
    ...
    if (result->status != CHIMERA_VFS_OK || (!ranges && count)) {
        if (result->status == CHIMERA_VFS_OK) {
            result->status = CHIMERA_VFS_EINVAL;
        }
        result->failed = 0;
        return;
    }
...
SYMBOL_EXPORT void
chimera_vfs_claim_journal_unlock(
    struct chimera_vfs_claim_journal           *j,
    struct chimera_vfs_claim_owner             *owner,
    const struct chimera_vfs_claim_exact_range *ranges,
    uint32_t                                    count,
    struct chimera_vfs_claim_batch_result      *result)
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (j->num_changes == j->max_changes) {
            ...
            if (!match) {
                result->status = contended ? CHIMERA_VFS_EBUSY : CHIMERA_VFS_ENOENT;
            }
        }
        if (result->status != CHIMERA_VFS_OK) {
            result->failed = i;
...
SYMBOL_EXPORT bool
chimera_vfs_claim_journal_io_denied(
    struct chimera_vfs_claim_journal *j,
    struct chimera_vfs_file_state    *file,
    uint64_t                          offset,
    uint64_t                          length,
...
SYMBOL_EXPORT uint32_t
chimera_vfs_claim_journal_excluded(
    const struct chimera_vfs_claim_journal *j,
...
} /* chimera_vfs_claim_journal_seal */
...
chimera_vfs_claim_journal_publish(struct chimera_vfs_claim_journal *j)
{
    ...
    for (uint32_t b = 0; b < j->num_owners; b++) {
        ...
        evpl_mutex_lock(&owner->lock);
        assert(owner->pins);
        if (owner->retire_journal == j) {
            owner->retiring = true;
        }
        evpl_mutex_lock(&owner->file->lock);
        for (uint32_t i = 0; i < j->num_changes; i++) {
            struct exact_change *change = &j->changes[i];
            if (change->owner != owner || change->add) {
                continue;
            }
        ...
        for (uint32_t i = 0; i < j->num_changes; i++) {
            ...
            if (record_removed(j, change->record)) {
                continue;
            }
            for (unsigned k = 0; k < change->record->num_claims; k++) {
...
} /* chimera_vfs_claim_journal_publish */
...
journal_release_owners(struct chimera_vfs_claim_journal *j)
{
    ...
    j->retire_changes    = 0;
    for (uint32_t b = 0; b < j->num_owners; b++) {
        journal_release_owner(j->owners[b]);
    }
    j->num_owners = 0;
} /* journal_release_owners */

static void
journal_release_changes(
    struct chimera_vfs_claim_journal *j,
    bool                              abort)
{
    for (uint32_t i = 0; i < j->num_changes; i++) {
...
} /* chimera_vfs_claim_journal_complete */
...
} /* chimera_vfs_claim_journal_reset */
...
chimera_vfs_claim_journal_free(struct chimera_vfs_claim_journal *j)
{
    ...
    free(j->owners); free(j->retire_owners); free(j->changes); free(j->excluded); free(j);
} /* chimera_vfs_claim_journal_free */
