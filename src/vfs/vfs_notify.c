    ...
    switch (request->opcode) {
        ...
        case CHIMERA_VFS_OP_RENAME_AT:
        {
            if ((request->rename_at.flags & CHIMERA_VFS_RENAME_NO_NOTIFY) ||
                request->rename_at.r_outcome == CHIMERA_VFS_RENAME_OUTCOME_NOOP) break;
        ...
        case CHIMERA_VFS_OP_LINK_AT:
            ...
            break;
        case CHIMERA_VFS_OP_MKDIR_AT:
            if (request->mkdir_at.flags & CHIMERA_VFS_MKDIR_NO_NOTIFY) break;
            chimera_vfs_notify_emit_sync(notify, gate, origin,
            ...
            break;
        case CHIMERA_VFS_OP_MKNOD_AT:
            if (request->mknod_at.flags & CHIMERA_VFS_MKNOD_NO_NOTIFY) break;
            chimera_vfs_notify_emit_sync(notify, gate, origin,
            ...
            break;
        case CHIMERA_VFS_OP_SYMLINK_AT:
            if (request->symlink_at.flags & CHIMERA_VFS_SYMLINK_NO_NOTIFY) break;
            chimera_vfs_notify_emit_sync(notify, gate, origin,
                                         request->fh, request->fh_len,
                                         CHIMERA_VFS_NOTIFY_FILE_ADDED,
...
chimera_vfs_notify_gate_install(struct chimera_vfs_request *request)
{
    ...
    if (request->opcode == CHIMERA_VFS_OP_REMOVE_AT &&
        (request->remove_at.flags & CHIMERA_VFS_REMOVE_NO_NOTIFY)) return;
    if (request->opcode == CHIMERA_VFS_OP_MKDIR_AT &&
        (request->mkdir_at.flags & CHIMERA_VFS_MKDIR_NO_NOTIFY)) return;
    if (request->opcode == CHIMERA_VFS_OP_SYMLINK_AT &&
        (request->symlink_at.flags & CHIMERA_VFS_SYMLINK_NO_NOTIFY)) return;
    if (request->opcode == CHIMERA_VFS_OP_MKNOD_AT &&
        (request->mknod_at.flags & CHIMERA_VFS_MKNOD_NO_NOTIFY)) return;
    if (request->opcode == CHIMERA_VFS_OP_RENAME_AT &&
        (request->rename_at.flags & CHIMERA_VFS_RENAME_NO_NOTIFY)) return;
