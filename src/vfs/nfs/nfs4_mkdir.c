    ...
    void                        *private_data)
{
    ...
    chimera_nfs4_unmarshall_fh(remote_fh, ctx->server->index, request->fh, &request->mkdir.r_attr);
    ...
    void                       *private_data)
{
    ...
    argarray[2].opcreate.objname.data               = (uint8_t *) request->mkdir.name;
    argarray[2].opcreate.objname.len                = request->mkdir.name_len;
