    ...
    void                        *private_data)
{
    ...
    struct chimera_nfs3_mknod_ctx *ctx     = request->plugin_data;
    ...
    void                       *private_data)
{
    struct chimera_nfs_client_server_thread *server_thread = chimera_nfs_thread_get_server_thread(thread, request->fh,
                                                                                                  request->fh_len);
    struct chimera_nfs3_mknod_ctx           *ctx;
    ...
    shared->nfs_v3.send_call_NFSPROC3_MKNOD(&shared->nfs_v3.rpc2, thread->evpl, server_thread->nfs_conn, &rpc2_cred,
                                            &args, 0, 0, NULL, 0, 0, chimera_nfs3_mknod_callback, request);
} /* chimera_vfs_nfs3_mknod_at */
