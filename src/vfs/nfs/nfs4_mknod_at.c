    ...
    void                        *private_data)
{
    ...
    chimera_nfs4_unmarshall_dir_attr(res, 2, &request->mknod_at.r_dir_pre_attr);
    ...
    void                       *private_data)
{
    ...
    /* Op 3: CREATE - create the special file */
    argarray[3].argop                 = OP_CREATE;
    argarray[3].opcreate.objtype.type = type;
    if (type == NF4BLK || type == NF4CHR) {
        uint64_t rdev = (set_attr->va_set_mask & CHIMERA_VFS_ATTR_RDEV) ? set_attr->va_rdev : 0;
        argarray[3].opcreate.objtype.devdata.specdata1 = (rdev >> 32) & 0xFFFFFFFF;
        argarray[3].opcreate.objtype.devdata.specdata2 = rdev & 0xFFFFFFFF;
    }
    argarray[3].opcreate.objname.data = (uint8_t *) request->mknod_at.name;
    argarray[3].opcreate.objname.len  = request->mknod_at.name_len;
    ...
    /* Op 4: GETFH - get file handle for created node */
    argarray[4].argop = OP_GETFH;

    /* Op 5: GETATTR - get attributes for created node */
