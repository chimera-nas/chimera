    ...
    void                       *private_data)
{
    ...
    if (request->rename_at.flags & CHIMERA_VFS_RENAME_NOREPLACE) {
        rc = renameat2(old_fd, fullname, new_fd, full_newname, RENAME_NOREPLACE);
    } else {
        rc = renameat(old_fd, fullname, new_fd, full_newname);
    }

    int renameat_errno = errno;
    if (rc < 0 && renameat_errno == ENOSYS &&
        (request->rename_at.flags & CHIMERA_VFS_RENAME_NOREPLACE)) renameat_errno = ENOTSUP;
    ...
    if (!(request->rename_at.flags & CHIMERA_VFS_RENAME_NOREPLACE) &&
        rc < 0 && (renameat_errno == ENOTEMPTY || renameat_errno == EEXIST)) {
...
static void
chimera_linux_claim_replace(
    struct chimera_vfs_request  *request,
    struct chimera_linux_thread *thread)
{
    struct chimera_linux_shared     *shared = thread->shared;
    struct chimera_linux_range_file *file;
    struct chimera_linux_range      *anchor = NULL, *fresh = NULL;
    struct flock                     fl;
    bool                             test = !!(request->claim_acquire.flags & CHIMERA_VFS_CLAIM_TEST);
    int                              err  = chimera_linux_geometry_flock(request->claim_acquire.whence,
                                                                         request->claim_acquire.offset, request->
                                                                         claim_acquire.length,
                                                                         request->claim_acquire.exclusive ? F_WRLCK :
                                                                         F_RDLCK, &fl);
    ...
    file = chimera_linux_range_file_get(thread, request->fh, request->fh_len,
    ...
    if (fcntl(file->fd, test ? CHIMERA_LINUX_LOCK_GET : CHIMERA_LINUX_LOCK_SET, &fl) < 0) {
        err = errno;
        if (err == EAGAIN || err == EACCES) {
            if (chimera_linux_geometry_flock(request->claim_acquire.whence,
    ...
    void                       *private_data)
{
    ...
    if (request->claim_acquire.flags & CHIMERA_VFS_CLAIM_REPLACE) {
        chimera_linux_claim_replace(request, thread);
        return;
    }
    ...
    struct chimera_linux_thread *thread)
{
    struct chimera_linux_shared     *shared = thread->shared;
    struct chimera_linux_range_file *file;
    struct chimera_linux_range      *range, *tmp, *pieces = NULL;
    struct flock                     fl;
    ...
    bool                             whole    = request->claim_release.whence == SEEK_SET && offset == 0 && length ==
        UINT64_MAX;
    int                              err = chimera_linux_geometry_flock(request->claim_release.whence,
    ...
    pthread_mutex_lock(&shared->range_lock);
    file = chimera_linux_range_file_find(shared, request->fh, request->fh_len,
        ...
        if (request->claim_release.whence == SEEK_END) {
            ...
            * original signed range atomically against its current size. */
            ...
            if (!file) {
                ...
                if (fcntl(file->fd, CHIMERA_LINUX_LOCK_SET, &fl) < 0) {
                    err = errno;
                }
                chimera_linux_range_file_put(shared, file);
            }
    ...
    if (!anchored && request->claim_release.whence == SEEK_END) {
        ...
        * never use this approximation: their raw SEEK_END goes to fcntl. */
        ...
        err    = chimera_linux_geometry_flock(SEEK_SET, offset, length, F_UNLCK, &fl);
        if (err) {
            goto put;
        }
    }
    if (!anchored) {
        ...
        for (range = shared->ranges; range; range = range->next) {
            ...
            for (unsigned side = 0; side < 2; side++) {
                ...
                struct chimera_linux_range *piece = malloc(sizeof(*piece));
                if (!piece) {
                    err = ENOMEM;
                    goto put;
                }
    ...
    void                       *private_data)
{
    struct chimera_linux_thread *thread = private_data;
    struct chimera_linux_shared *shared = thread->shared;
    struct chimera_linux_range  *range, *tmp;
    int                          err = 0;
    ...
    LL_FOREACH_SAFE(shared->ranges, range, tmp)
    {
        ...
        if (range->projected) {
            ...
            if (fcntl(range->file->fd, CHIMERA_LINUX_LOCK_SET, &fl) < 0) {
                err = errno;
                break;
            }
        }
...
SYMBOL_EXPORT struct chimera_vfs_module vfs_linux = {
    ...
    .capabilities = CHIMERA_VFS_CAP_BLOCKING | CHIMERA_VFS_CAP_OPEN_PATH_REQUIRED |
        ...
        CHIMERA_VFS_CAP_DELEGATES_DAC | CHIMERA_VFS_CAP_XATTR | CHIMERA_VFS_CAP_RENAME_NOREPLACE
    ,
