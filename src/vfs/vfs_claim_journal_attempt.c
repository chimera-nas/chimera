...
static void range_poll(struct chimera_vfs_claim_range_attempt *a);

static void
range_timer(struct evpl *evpl, struct evpl_timer *timer)
{
...
range_poll(struct chimera_vfs_claim_range_attempt *a)
{
    ...
    if (a->delay_us < 100000) {
        a->delay_us *= 2;
        if (a->delay_us > 100000) a->delay_us = 100000;
    }
