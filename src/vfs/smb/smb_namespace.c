    ...
    void                           *arg)
{
    ...
    * UTF-16LE with '\\' separators (smb_utf16le_encode does the '/'->'\\'). */
     ...
     * RootDirectory(8) + FileNameLength(4) + FileName(UTF-16LE). */
    buffer_length = (uint32_t) (16 + name16_len);
    ...
    void                           *arg)
{
    ...
    /* The input buffer immediately follows the IOCTL fixed fields the server
    * reads: StructureSize(2) + CtlCode(4) + FileId(16) + InputOffset(4) +
    * InputCount(4) + MaxInputResponse(4) + OutputOffset(4) + OutputCount(4) +
    * MaxOutputResponse(4) + Flags(4) + Reserved2(4) = 54 bytes, so the input
    * starts at SMB2 header + 54.  (The server skips to InputOffset, so only
    * the field read-order must match -- there is no leading Reserved(2).) */
