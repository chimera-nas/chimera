    ...
    bool                      one_shot)
{
     ...
     * used (0) below and the call no-ops, exactly like the old core (which
     * treated ACKED as not-BREAKING without re-arming it).  Re-arming to
     * IDLE happens only through a real re-open (grant_coalesce). */
    ...
    if (should_invoke) {
         ...
         * they keep full mode until DELEGRETURN/revoke (R11).  The
         * INTERNAL implicit claim likewise keeps its full mode while
         * draining -- its I/O is still in flight, so a conflicting
         * deny-open or delegation grant must keep waiting until the drain
         * actually drops the claim (the old effective_granted downgraded
         * mid-break holders for PROTO_SMB2 only, R60/R62).  Lock-bit deny
         * rows read RAW used regardless, so a lock acquirer keeps waiting
         * for the real ack (lock1). */
        if (claim->grant) {
    ...
    void                          **out_private)
{
    ...
    *out_private = NULL;
    ...
    if (file) {
         ...
         * the inline-confirm drain orders it before any relock confirm. */
...
{
    bool legacy_oplock = (victim->construct == CHIMERA_CONSTRUCT_OPLOCK_II ||
                          victim->construct == CHIMERA_CONSTRUCT_OPLOCK_EX ||
                          victim->construct == CHIMERA_CONSTRUCT_OPLOCK_BATCH);
    ...
    switch (trigger) {
        ...
        case CHIMERA_TRIGGER_SMB_LOCK:
             ...
             * self-exempt by owner/KEY; a legacy oplock WITHOUT the write
             * cache (a LEVEL_II grant, or an exclusive/batch already
             * downgraded to a read cache) confers no write coherence and
             * NEVER self-exempts (batch6: a write by one LEVEL_II holder
             * breaks BOTH LEVEL_II oplocks, including the writer's own).
             * An exclusive/batch holder still holding CW is coherent and
             * self-exempts like a lease.  (Mode-scored, not construct-
             * scored: the old rule was is_oplock && !(granted & W).) */
...
/* Triggers whose row may also select a holder ALREADY mid-break, to DEEPEN
 * its floor (begin_break's BREAKING branch: break_floor &= floor, no new
 * notification, no epoch bump).  This is how a write / truncating open
 * arriving behind an in-flight RWH->RH open-break drives the cascade on to
 * NONE once the holder acks (smb2.lease.breaking3 / v2_breaking3) -- the
 * old walkers (break_reads_for_write, break_caching_for_open,
 * break_caching_for_namespace) never filtered on break_state.  The
 * dir-content walker skipped mid-break holders (old dir_lease_break's IDLE
 * check), and RANGE_LOCK / NS_FULL / FLUSH handle mid-break holders in
 * their own engines, so those keep the IDLE/ACKED-only prefilter. */
static inline bool
chimera_vfs_claim_trigger_deepens(enum chimera_claim_trigger trigger)
{
    switch (trigger) {
        case CHIMERA_TRIGGER_WRITE:
        case CHIMERA_TRIGGER_SMB_LOCK:
...
} /* chimera_vfs_claim_trigger_deepens */
    ...
    uint8_t                           retain)
{
    ...
    for (cur = file->claims[CHIMERA_CLAIM_CLASS_CACHE]; cur; cur = cur->next) {
        ...
        if (cur->break_state == CHIMERA_CLAIM_BREAK_BREAKING &&
            !chimera_vfs_claim_trigger_deepens(trigger)) {
            continue;
        }
            ...
            if ((skip_handle && cur->op_handle == skip_handle) ||
                (skip_actor &&
                 (chimera_claim_owner_equal(&cur->owner, &skip_actor->owner) ||
                  chimera_claim_owner_same_key(&cur->owner, &skip_actor->owner)))) {
                continue;
            }
    ...
    uint64_t                  fh_hash)
{
    ...
    had = chimera_vfs_claim_trigger_ns_full(state, file, NULL, NULL, false);
