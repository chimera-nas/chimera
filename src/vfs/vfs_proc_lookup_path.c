    ...
    void                           *private_data)
{
    ...
    memcpy(lp_request->lookup_path.parent_fh, oh->fh, oh->fh_len);
    lp_request->lookup_path.parent_fh_len = oh->fh_len;
    ...
    chimera_vfs_lookup(
        ...
        component,
        componentlen,
        (final ? lp_request->lookup_path.attr_mask : CHIMERA_VFS_ATTR_FH) | CHIMERA_VFS_ATTR_MODE,
        0,
        chimera_vfs_lookup_path_complete,
        lp_request);
    ...
    void                           *private_data)
{
    ...
    target = lp_request->lookup_path.path + strlen(lp_request->lookup_path.path) + 1;
    ...
    void                  *private_data)
{
    ...
    remaining_len = strlen(lp_request->lookup_path.pathc);
    ...
    if (target[0] == '/') {
        ...
        start_fh     = lp_request->lookup_path.parent_fh;
        start_fh_len = lp_request->lookup_path.parent_fh_len;
    }
    ...
    if (remaining_len > 0) {
        ...
        if (new_pathlen >= CHIMERA_VFS_PATH_MAX) {
            lp_request->lookup_path.callback(CHIMERA_VFS_ENAMETOOLONG,
                                             NULL,
                                             lp_request->lookup_path.private_data);
            chimera_vfs_request_free(thread, lp_request);
            return;
        ...
        memcpy(new_path + target_length + 1, lp_request->lookup_path.pathc, remaining_len);
    ...
    /* Reset path pointer */
    lp_request->lookup_path.path    = new_path;
    lp_request->lookup_path.pathc   = new_path;
    lp_request->lookup_path.pathlen = new_pathlen;
    ...
    void                     *private_data)
{
    ...
    if ((attr->va_set_mask & CHIMERA_VFS_ATTR_MODE) && S_ISLNK(attr->va_mode)) {
        if (!final) {
            ...
            follow_symlink = 1;
        } else if (lp_request->lookup_path.flags & CHIMERA_VFS_LOOKUP_FOLLOW) {
    ...
    if (follow_symlink) {
        ...
        lp_request->lookup_path.symlink_count++;
        if (lp_request->lookup_path.symlink_count > CHIMERA_VFS_SYMLOOP_MAX) {
            lp_request->lookup_path.callback(CHIMERA_VFS_ELOOP,
                                             NULL,
                                             lp_request->lookup_path.private_data);
            chimera_vfs_request_free(thread, lp_request);
            return;
    ...
    void                              *private_data)
{
    ...
    lp_request->lookup_path.flags         = flags;
    lp_request->lookup_path.symlink_count = 0;
    lp_request->lookup_path.private_data  = private_data;
    lp_request->lookup_path.callback      = callback;
    lp_request->lookup_path.parent_fh_len = 0;
