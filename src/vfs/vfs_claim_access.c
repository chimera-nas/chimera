...
SYMBOL_EXPORT bool
chimera_vfs_claim_access_fence_acquire(struct chimera_vfs_claim_access_fence *fence,
                                     struct chimera_vfs_file_state *file,
                                     const void *cookie)
{
    if (!fence || !file || !cookie) return false;
    if (fence->file) return fence->file == file && fence->cookie == cookie;
...
chimera_vfs_claim_access_fence_release(struct chimera_vfs_claim_access_fence *fence)
{
    ...
    if (last) file->access_fence_cookie = NULL;
    fence->file = NULL;
    fence->cookie = NULL;
    pthread_mutex_unlock(&file->lock);
    if (last) chimera_vfs_claim_pump_pending(file->state, file);
...
chimera_vfs_claim_access_owner_alloc(struct chimera_vfs_file_state *file,
                                     const struct chimera_vfs_claim *template_claim)
{
    if (!file || !template_claim || template_claim->file ||
        template_claim->klass != CHIMERA_CLAIM_CLASS_ACCESS) return NULL;
    ...
    if (!owner->file) { free(owner); return NULL; }
...
{
    if (atomic_fetch_sub(&owner->refs, 1) != 1) return;
...
chimera_vfs_claim_access_owner_retire(struct chimera_vfs_claim_access_owner *owner)
{
    ...
    if (drain) access_unlink(owner);
    pthread_mutex_unlock(&owner->lock);
    if (drain) chimera_vfs_claim_replacement_complete(owner->file);
}
...
chimera_vfs_claim_access_journal_alloc(uint32_t capacity)
{
    ...
    if (!j->changes) { free(j); return NULL; }
    j->capacity = capacity;
    return j;
}

static int
access_latest(const struct chimera_vfs_claim_access_journal *j,
              const struct chimera_vfs_claim_access_owner *owner)
{
    for (uint32_t i = j->count; i > 0; i--) {
        if (j->changes[i - 1].owner == owner) return (int) i - 1;
    }
    return -1;
}

static enum chimera_vfs_error
access_stage(struct chimera_vfs_claim_access_journal *j,
             struct chimera_vfs_claim_access_owner *owner,
             bool retire, uint8_t used, uint8_t denied)
{
    if (!j || !owner || j->phase != ACCESS_ACTIVE) return CHIMERA_VFS_EINVAL;
    pthread_mutex_lock(&owner->lock);
...
chimera_vfs_claim_access_journal_retire(struct chimera_vfs_claim_access_journal *j,
                                      struct chimera_vfs_claim_access_owner *owner)
{
...
SYMBOL_EXPORT enum chimera_vfs_error
chimera_vfs_claim_access_journal_narrow(struct chimera_vfs_claim_access_journal *j,
                                      struct chimera_vfs_claim_access_owner *owner,
                                      uint8_t used, uint8_t denied)
{
    return access_stage(j, owner, false, used, denied);
}

SYMBOL_EXPORT uint32_t
chimera_vfs_claim_access_journal_excluded(const struct chimera_vfs_claim_access_journal *j,
                                        ...
                                        const struct chimera_vfs_claim **out, uint32_t capacity)
{
    ...
    for (uint32_t i = 0; i < j->count; i++) {
        ...
        if (owner->file != file || access_latest(j, owner) != (int) i) continue;
        if (out && count < capacity) out[count] = &owner->claim;
        count++;
    }
    return count;
}
...
chimera_vfs_claim_access_journal_test(const struct chimera_vfs_claim_access_journal *j,
                                     const struct chimera_vfs_file_state *file,
                                     const struct chimera_vfs_claim *probe,
                                     struct chimera_vfs_claim_conflict *conflict)
{
    assert(probe);
    if (conflict) memset(conflict, 0, sizeof(*conflict));
    if (!j) return CHIMERA_CLAIM_GRANTED;
    for (uint32_t i = 0; i < j->count; i++) {
        const struct access_change *change = &j->changes[i];
        struct chimera_vfs_claim_access_owner *owner = change->owner;
        if (owner->file != file || change->retire || access_latest(j, owner) != (int) i) continue;
        pthread_mutex_lock(&owner->lock);
        ...
        if (blocked && conflict) chimera_vfs_claim_conflict_fill(&view, conflict);
        pthread_mutex_unlock(&owner->lock);
        if (blocked) return CHIMERA_CLAIM_DENIED;
    }
...
chimera_vfs_claim_access_journal_publish(struct chimera_vfs_claim_access_journal *j)
{
    ...
    for (uint32_t i = 0; i < j->count; i++) {
        ...
        struct chimera_vfs_claim_access_owner *owner = change->owner;
        if (access_latest(j, owner) != (int) i) continue;
        pthread_mutex_lock(&owner->lock);
        assert(owner->busy == j);
...
static void
access_release_owners_to(struct chimera_vfs_claim_access_journal *j, uint32_t checkpoint)
{
    while (j->count > checkpoint) {
        ...
        if (last) {
            if (owner->retiring && !owner->retired) access_unlink(owner);
            owner->busy = NULL;
        }
        pthread_mutex_unlock(&owner->lock);
        if (last) chimera_vfs_claim_replacement_complete(owner->file);
...
SYMBOL_EXPORT void
chimera_vfs_claim_access_journal_rewind(struct chimera_vfs_claim_access_journal *j, uint32_t checkpoint)
{
