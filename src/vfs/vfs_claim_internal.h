...
/* chimera_vfs_claim_backend_capable() is declared in vfs_claim.h: callers
 * outside the claim core use it to decide whether an acquire has to reach a
 * VFS thread at all. */
 ...
 * after the cb has fired (waits out an in-flight inline confirm). */
bool
chimera_vfs_claim_backend_ticket_cancel(
    struct chimera_vfs_state           *state,
    struct chimera_vfs_pending_acquire *ticket);

/* Drain queued RELEASE work for `file` inline on `thread` (ordered before
 * a same-file confirm). */
void
...
bool chimera_vfs_claim_conflicts(const struct chimera_vfs_claim *holder,
                                const struct chimera_vfs_claim *probe);
