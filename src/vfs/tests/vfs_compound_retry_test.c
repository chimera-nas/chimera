    ...
    char **argv)
{
    ...
    i_look_a  = chimera_vfs_compound_add_lookup(cp, "a", 1, CHIMERA_VFS_ATTR_MASK_STAT, 0);
    i_look_b  = chimera_vfs_compound_add_lookup(cp, "b", 1, CHIMERA_VFS_ATTR_MASK_STAT, 0);
    ...
    chimera_vfs_compound_add_lookup(cp, "nonexistent", 11, CHIMERA_VFS_ATTR_MASK_STAT, 0);
    i_getattr = chimera_vfs_compound_add_getattr(cp,
                                                 CHIMERA_VFS_ATTR_MASK_STAT);
    ...
    int missing_remove = chimera_vfs_compound_add_remove(cp, "missing-wcc", 11, 0, 0, 0);
...

    {
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, root_fh_len);
        assert(chimera_vfs_compound_add_lookup(cp, name, sizeof(name), 0, 0) < 0);
     ...
     * have distinct cache entries, so physical access alone is insufficient. */
    {
        ...
        cp                = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        opened = chimera_vfs_compound_add_open(cp, "grant-union", 11, CHIMERA_VFS_OPEN_CREATE |
                                               CHIMERA_VFS_OPEN_READ_ONLY, 0, &attrs, 0, 0, 0);
        chimera_vfs_compound_submit(cp, compound_cb, &ctx);
        wait_done(&ctx);
        assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
        ...
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        opened = chimera_vfs_compound_add_open(cp, "grant-union", 11, CHIMERA_VFS_OPEN_READ_ONLY, 0, NULL, 0, 0, 0);
        chimera_vfs_compound_submit(cp, compound_cb, &ctx);
        wait_done(&ctx);
        assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
        ...
        chimera_vfs_compound_add_setattr(cp, NULL, &attrs, 0, 0);
        chimera_vfs_compound_submit(cp, compound_cb, &ctx);
        wait_done(&ctx);
        assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
        chimera_vfs_compound_free(cp);
         ...
         * entry can serve the physical reopen: neither grant may be dropped. */
        {
            ...
            chimera_vfs_compound_add_putfh(cp, file_fh, (int) file_fh_len);
            upgraded = chimera_vfs_compound_add_open(cp, NULL, 0, CHIMERA_VFS_OPEN_READ_ONLY |
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &user);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        upgraded = chimera_vfs_compound_add_open(cp, NULL, 0, CHIMERA_VFS_OPEN_READ_ONLY, 0, NULL, 0, 0, 0);
        chimera_vfs_compound_set_op_prepare(cp, upgraded, inherit_open_grant_prepare, old);
        chimera_vfs_compound_submit(cp, compound_cb, &ctx);
        wait_done(&ctx);
     ...
     * that is.  Under ASAN a handle released twice or leaked shows up here. */
    {
        ...
        i_lk   = chimera_vfs_compound_add_lookup(cp, "a", 1, CHIMERA_VFS_ATTR_MASK_STAT, 0);
        chimera_vfs_compound_add_getattr(cp, CHIMERA_VFS_ATTR_MASK_STAT);
     ...
     * already holding. */
    {
        ...
        i_commit = chimera_vfs_compound_add_commit(cp, 0, 0, CHIMERA_VFS_ATTR_MODE, 0);
    ...
    /* ---- READDIR of the current object ---- */
    {
        ...
        i_rd = chimera_vfs_compound_add_readdir(cp, 0, 0, 8192, 8192, 32, CHIMERA_VFS_ATTR_MASK_STAT, 0);
     ...
     * than making a second round trip for what it just created. */
    {
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        i_open = chimera_vfs_compound_add_open(cp, "o1", 2, CHIMERA_VFS_OPEN_CREATE |
                                               CHIMERA_VFS_OPEN_WRITE_ONLY, 0, &sattr, CHIMERA_VFS_ATTR_MASK_STAT, 0, 0)
        ;
     ...
     * a directory), so the mode comes back with it. */
    {
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        i_open = chimera_vfs_compound_add_open(cp, "a", 1, CHIMERA_VFS_OPEN_READ_ONLY,
                                               CHIMERA_VFS_COMPOUND_OPEN_REGULAR_ONLY, NULL, CHIMERA_VFS_ATTR_MASK_STAT,
                                               0, 0);
     ...
     * before it opened. */
    {
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        i_open        = chimera_vfs_compound_add_open(cp, "o2", 2, CHIMERA_VFS_OPEN_CREATE, 0, &sattr, 0, 0, 0);
        ctx.callbacks = 0;
        chimera_vfs_compound_submit(cp, compound_cb, &ctx);
        wait_done(&ctx);
        assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        i_open = chimera_vfs_compound_add_open(cp, "o2", 2, CHIMERA_VFS_OPEN_CREATE,
                                               CHIMERA_VFS_COMPOUND_OPEN_ATTRS_ON_CREATE_ONLY, &sattr,
    ...
    * attributes, so the object it finds is left exactly as it was. */
    {
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        i_open = chimera_vfs_compound_add_open(cp, "x1", 2, CHIMERA_VFS_OPEN_CREATE |
                                               CHIMERA_VFS_OPEN_EXCLUSIVE, 0, &sattr, 0, 0, 0);
        ctx.callbacks = 0;
        chimera_vfs_compound_submit(cp, compound_cb, &ctx);
        wait_done(&ctx);
        assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        i_open = chimera_vfs_compound_add_open(cp, "x1", 2, CHIMERA_VFS_OPEN_CREATE | CHIMERA_VFS_OPEN_EXCLUSIVE,
                                               CHIMERA_VFS_COMPOUND_OPEN_EXCLUSIVE_RETRY, &sattr,
                                               CHIMERA_VFS_ATTR_MASK_STAT, 0, 0);
    ...
    /* ---- an OPEN with no name re-opens the current object ---- */
    {
        ...
        i_open = chimera_vfs_compound_add_open(cp, NULL, 0, CHIMERA_VFS_OPEN_READ_ONLY, 0, NULL, 0, 0, 0);
     ...
     * current object is still the directory afterwards. */
    {
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        i_mkdir = chimera_vfs_compound_add_create(cp, CHIMERA_VFS_COMPOUND_CREATE_DIR, "nd", 2, NULL, 0, &sattr,
                                                  CHIMERA_VFS_ATTR_MASK_STAT, 0, 0);
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        i_ln = chimera_vfs_compound_add_create(cp, CHIMERA_VFS_COMPOUND_CREATE_SYMLINK, "sl", 2, "nd", 2, NULL,
                                               CHIMERA_VFS_ATTR_MASK_STAT, 0, 0);
     ...
     * for itself -- from an NFSv4 stateid, say -- needs the first. */
    {
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        i_open = chimera_vfs_compound_add_open(cp, "sa", 2, CHIMERA_VFS_OPEN_CREATE |
                                               CHIMERA_VFS_OPEN_WRITE_ONLY, 0, &sattr, 0, 0, 0);
        ctx.callbacks = 0;
        chimera_vfs_compound_submit(cp, compound_cb, &ctx);
        wait_done(&ctx);
        assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
        ...
        i_sa  = chimera_vfs_compound_add_setattr(cp, NULL, &sattr, 0, CHIMERA_VFS_ATTR_MASK_STAT);
        ...
        cp   = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        i_sa = chimera_vfs_compound_add_setattr(cp, oh, &sattr, 0, CHIMERA_VFS_ATTR_MASK_STAT);
    ...
    * Reject directory I/O before dispatching a backend data operation. */
    for (int write = 0; write < 2; write++) {
        ...
        io_index = write ? chimera_vfs_compound_add_write(cp, NULL, 0, 0, 0, NULL, 0, 0, 0, NULL) :
            chimera_vfs_compound_add_read(cp, NULL, 0, 1, iov, 1, 0, NULL, NULL, 0);
        chimera_vfs_compound_submit(cp, compound_cb, &ctx);
        wait_done(&ctx);
    ...
    * the caller takes it, and releases what was never taken. */
    {
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        i_open = chimera_vfs_compound_add_open(cp, "rd", 2, CHIMERA_VFS_OPEN_CREATE |
                                               CHIMERA_VFS_OPEN_WRITE_ONLY |
                                               CHIMERA_VFS_OPEN_READ_ONLY, 0, &sattr, 0, 0, 0);
        ctx.callbacks = 0;
        chimera_vfs_compound_submit(cp, compound_cb, &ctx);
        wait_done(&ctx);
        assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
     ...
     * deliberately not symmetric, and this is where that shows. */
    {
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        i_open = chimera_vfs_compound_add_open(cp, "wr", 2, CHIMERA_VFS_OPEN_CREATE |
                                               CHIMERA_VFS_OPEN_WRITE_ONLY |
                                               CHIMERA_VFS_OPEN_READ_ONLY, 0, &sattr, 0, 0, 0);
        ctx.callbacks = 0;
        chimera_vfs_compound_submit(cp, compound_cb, &ctx);
        ...
        cp   = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        i_wr = chimera_vfs_compound_add_write(cp, oh, 0, 8, 2, &wiov, 1, 0, 0, NULL);
        i_rd = chimera_vfs_compound_add_read(cp, oh, 0, 8, rdiov, 16, 0, NULL, NULL, 0);
     ...
     * the second one. */
    {
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        i_src = chimera_vfs_compound_add_create(cp, CHIMERA_VFS_COMPOUND_CREATE_DIR, "rnsrc", 5, NULL, 0, &sattr,
                                                CHIMERA_VFS_ATTR_MASK_STAT, 0, 0);
        chimera_vfs_compound_add_create(cp, CHIMERA_VFS_COMPOUND_CREATE_NODE, "f", 1, NULL, 0, &sattr,
                                        CHIMERA_VFS_ATTR_MASK_STAT, 0, 0);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        i_dst = chimera_vfs_compound_add_create(cp, CHIMERA_VFS_COMPOUND_CREATE_DIR, "rndst", 5, NULL, 0, &sattr
                                                , CHIMERA_VFS_ATTR_MASK_STAT, 0, 0);
        ctx.callbacks = 0;
        chimera_vfs_compound_submit(cp, compound_cb, &ctx);
        wait_done(&ctx);
        ...
        i_look = chimera_vfs_compound_add_lookup(cp, "g", 1, CHIMERA_VFS_ATTR_MASK_STAT, 0);
        ...
        i_ga   = chimera_vfs_compound_add_lookup(cp, "h", 1, CHIMERA_VFS_ATTR_MASK_STAT, 0);
    ...
    /* Reject a mutation before any VFS work, then replay the attempt. */
    {
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        denied = chimera_vfs_compound_add_remove(cp, "a", 1, 0, 0, 0);
    ...
    * rebuilt rather than accumulated when a rejected attempt is repeated. */
    {
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        skipped = chimera_vfs_compound_add_remove(cp, "a", 1, 0, 0, 0);
    ...
    * reject before a following conditional truncate can change the file. */
    {
        ...
        cp                = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        opened = chimera_vfs_compound_add_open(cp, "reserve-test", 12, CHIMERA_VFS_OPEN_CREATE |
                                               CHIMERA_VFS_OPEN_INFERRED, 0, &attrs, CHIMERA_VFS_ATTR_SIZE, 0, 0);
        chimera_vfs_compound_submit(cp, compound_cb, &ctx);
        wait_done(&ctx);
        assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
        ...
        opened   = chimera_vfs_compound_add_open(cp, NULL, 0, CHIMERA_VFS_OPEN_INFERRED, 0, NULL, 0, 0, 0);
        reserved = chimera_vfs_compound_add_reserve(cp, opened, &requested);
        memset(&attrs, 0, sizeof(attrs));
        attrs.va_set_mask = CHIMERA_VFS_ATTR_SIZE;
        attrs.va_size     = 0;
        truncated         = chimera_vfs_compound_add_setattr(cp, NULL, &attrs, 0, 0);
     ...
     * its private context. Both callbacks see the resolved cursor on retry. */
    {
        ...
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        chimera_vfs_compound_add_lookup(cp, "a", 1, CHIMERA_VFS_ATTR_FH, 0);
    ...
    }
    {
        ...
        chimera_vfs_compound_add_lookup(cp, "missing-finish-test", 19, 0, 0);
        chimera_vfs_compound_set_finish_handler(cp, reject_first_finish, &test);
        chimera_vfs_compound_submit(cp, finish_retry_complete, &test);
        wait_done(&ctx);
        assert(test.finishes == 2 && test.callbacks == 2 && test.publications == 1);
     ...
     * ownership without pretending the memfs backend supports rollback. */
    {
        ...
        set_index = chimera_vfs_compound_add_setattr(cp, NULL, &attrs, 0, 0);
     ...
     * spuriously denies an owner's mode-000 file despite an open data fd. */
    {
        ...
        cp = chimera_vfs_compound_alloc(ctx.vfs_thread, &cred);
        chimera_vfs_compound_add_putfh(cp, root_fh, (int) root_fh_len);
        chimera_vfs_compound_add_open(cp, "setattr-data", 12, CHIMERA_VFS_OPEN_CREATE | CHIMERA_VFS_OPEN_EXCLUSIVE, 0, &
        ...
        set_index = chimera_vfs_compound_add_setattr(cp, NULL, &attrs, 0, CHIMERA_VFS_ATTR_SIZE);
        get_index = chimera_vfs_compound_add_gethandle(cp);
        chimera_vfs_compound_submit(cp, compound_cb, &ctx);
        wait_done(&ctx);
        assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
    ...
    /* Typed named streams share ordinary OPEN provenance and retry ownership. */
    {
        ...
        int                      stream = chimera_vfs_compound_add_open_stream(cp, "compound-stream", 15,
                                                                               CHIMERA_VFS_OPEN_CREATE |
                                                                               CHIMERA_VFS_OPEN_EXCLUSIVE, NULL,
                                                                               CHIMERA_VFS_ATTR_SIZE);
        int                      sized = chimera_vfs_compound_add_setattr(cp, NULL, &size, 0, CHIMERA_VFS_ATTR_SIZE);
        chimera_vfs_compound_submit(cp, compound_cb, &ctx);
        wait_done(&ctx);
        assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
