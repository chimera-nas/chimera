...
make_owner(struct chimera_vfs_file_state *file, unsigned id)
{
...
probe(struct chimera_vfs_file_state *file, struct chimera_vfs_claim_access_journal *j)
{
    struct chimera_claim_owner identity = { .proto = CHIMERA_CLAIM_PROTO_SMB2,
...
static void
fence_complete(enum chimera_vfs_claim_result result, struct chimera_vfs_claim *claim,
               const struct chimera_vfs_claim_conflict *conflict, void *private)
{
...
check_revoke_storage_anchor(struct chimera_vfs_file_state *file)
{
    ...
    assert(chimera_vfs_claim_grant_acquire(file->state, file, &cache, 0, 1,
        CHIMERA_CLAIM_GRANT_EXACT, &member, NULL, &grant, NULL) == CHIMERA_CLAIM_GRANTED);
...
struct member_join {
    ...
    pthread_barrier_t attached, finish;
    bool acquire;
};
static void *
join_member(void *private_data)
{
    ...
    if (join->acquire) {
        assert(chimera_vfs_claim_grant_acquire_member(join->file->state, join->file,
            ...
            &join->member, &join->member.next, &join->grant, NULL) == CHIMERA_CLAIM_GRANTED);
    } else {
        join->grant = chimera_vfs_claim_grant_coalesce_member(join->file,
            &join->template.owner, join->template.used, 0, &join->member, &join->member.next);
        assert(join->grant);
    }
    ...
    pthread_barrier_wait(&join->attached);
    pthread_barrier_wait(&join->finish);
    return NULL;
}
static void
check_atomic_member_attach(struct chimera_vfs_file_state *file)
{
    ...
    for (unsigned int acquire = 0; acquire < 2; acquire++) {
        ...
        assert(chimera_vfs_claim_grant_acquire_member(file->state, file, &join.template,
            0, 1, 43, CHIMERA_CLAIM_GRANT_EXACT, &first, &first.next, &grant, NULL) == CHIMERA_CLAIM_GRANTED);
        assert(grant->members == &first && !first.next && grant->epoch == 43);
        assert(!pthread_barrier_init(&join.attached, NULL, 2));
        assert(!pthread_barrier_init(&join.finish, NULL, 2));
        pthread_t worker;
        assert(!pthread_create(&worker, NULL, join_member, &join));
        pthread_barrier_wait(&join.attached);
        pthread_mutex_lock(&file->lock);
        ...
        chimera_vfs_claim_grant_release(file->state, grant, true);
        pthread_barrier_destroy(&join.attached);
        pthread_barrier_destroy(&join.finish);
    }
}

static void
unexpected_oplock_break(struct chimera_vfs_claim *claim, uint8_t needed, void *private_data)
{
...
first_grant_member(void *private_data)
{
    ...
    pthread_barrier_wait(join->start);
    assert(chimera_vfs_claim_grant_acquire_member(join->file->state, join->file,
...
check_first_grant_race(struct chimera_vfs_file_state *file)
{
    ...
    for (unsigned int run = 0; run < 32; run++) {
        pthread_barrier_t start;
        assert(!pthread_barrier_init(&start, NULL, 3));
        ...
        for (unsigned int i = 0; i < 2; i++) {
            ...
            assert(!pthread_create(&workers[i], NULL, first_grant_member, &joins[i]));
        }
        pthread_barrier_wait(&start);
        for (unsigned int i = 0; i < 2; i++) assert(!pthread_join(workers[i], NULL));
...
static void
seeded_epoch_break(struct chimera_vfs_claim *claim, uint8_t needed, void *private_data)
{
...
static void
check_prepared_read_lease(struct chimera_vfs_file_state *file, bool directory)
{
    struct chimera_claim_owner identity = { .proto = CHIMERA_CLAIM_PROTO_SMB2,
    ...
    pthread_mutex_unlock(&file->lock);
    struct chimera_claim_actor actor = { .owner = { .proto = CHIMERA_CLAIM_PROTO_SMB2,
        .client_key = 927, .owner_lo = 927 } };
...
static enum chimera_vfs_claim_result
narrow_probe(struct chimera_vfs_file_state *file, struct chimera_vfs_claim_access_journal *journal,
             uint8_t used, uint8_t denied)
{
    ...
    if (journal) {
        ...
        if (result != CHIMERA_CLAIM_GRANTED) return result;
