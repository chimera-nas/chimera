    ...
    void                        *private)
{
    ...
    (void) index;
    assert(*status == CHIMERA_VFS_OK);
    f->dynamic_open = chimera_vfs_compound_add_open(cp, NULL, 0, CHIMERA_VFS_OPEN_PATH | CHIMERA_VFS_OPEN_DIRECTORY, 0,
                                                    NULL, CHIMERA_VFS_ATTR_MODE, 0, 0);
    ...
    const struct chimera_vfs_cred *cred)
{
    ...
    chimera_vfs_compound_add_open(cp, NULL, 0, CHIMERA_VFS_OPEN_PATH | CHIMERA_VFS_OPEN_DIRECTORY, 0, NULL, 0, 0, 0);
    ...
    const struct chimera_vfs_cred *cred)
{
    for (unsigned mode = 0; mode < 6; mode++) {
        ...
        assert(test.old);
        assert(chimera_vfs_claim_try_acquire(f->vfs->vfs_state, test.file,
                                             chimera_vfs_claim_access_owner_claim(test.old), NULL) ==
        ...
        add_group(cp, 0, 1, NULL, -1, true, f);
        chimera_vfs_compound_add_putfh(cp, f->fh, f->fh_len);
        assert(chimera_vfs_compound_add_open(cp, NULL, 0, CHIMERA_VFS_OPEN_PATH | CHIMERA_VFS_OPEN_DIRECTORY, 0, NULL, 0
                                             , 0, 0) == 2);
    ...
    const struct chimera_vfs_cred *cred)
{
    ...
    chimera_vfs_compound_add_putfh(cp, f->fh, f->fh_len);
    assert(chimera_vfs_compound_add_open(cp, NULL, 0, CHIMERA_VFS_OPEN_PATH | CHIMERA_VFS_OPEN_DIRECTORY, 0, NULL, 0, 0,
                                         0) == 1);
    ...
    for (unsigned i = 2; i <= 4; i++) {
        chimera_vfs_compound_set_op_prepare(cp, i, closed_anchor_prepare, &test);
    }
    ...
    const struct chimera_vfs_cred *cred)
{
    for (unsigned failure = 0; failure < 2; failure++) {
        ...
        for (unsigned i = 0; i < 3; i++) {
            assert(chimera_vfs_claim_try_acquire(f->vfs->vfs_state, test.file,
                                                 chimera_vfs_claim_access_owner_claim(owners[i]), NULL) ==
                   CHIMERA_CLAIM_GRANTED);
        }
        ...
        if (failure) {
            assert(chimera_vfs_claim_access_journal_retire(peer, test.base) == CHIMERA_VFS_OK);
        }
        struct chimera_vfs_compound             *cp = chimera_vfs_compound_alloc(f->thread, cred);
        ...
        if (!failure) {
            assert(chimera_vfs_compound_add_range_batch(cp, test.new_range, &old, 1, false) == 4);
        }
...
static void
check_handle_state_dac(
    struct fixture                *f,
    const struct chimera_vfs_cred *root,
    const struct chimera_vfs_cred *user)
{
    ...
    int                             child_op  = chimera_vfs_compound_add_open_at(cp, "hs-private", 10,
                                                                                 ...
                                                                                 CHIMERA_VFS_OPEN_EXCLUSIVE, &attrs,
                                                                                 CHIMERA_VFS_ATTR_FH);
    chimera_vfs_compound_submit(cp, completed, f);
    wait_done(f);
    assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
    ...
    const struct chimera_vfs_cred *cred)
{
    ...
    chimera_vfs_compound_add_putfh(cp, f->fh, f->fh_len);
    for (unsigned i = 0; i < 3; i++) {
        chimera_vfs_compound_add_delete_key_at(cp, keys[i], 4);
    }
    chimera_vfs_compound_submit(cp, completed, f);
    wait_done(f);
    assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
    ...
    const struct chimera_vfs_cred *user)
{
    ...
    chimera_vfs_compound_add_putfh(cp, f->fh, f->fh_len);
    chimera_vfs_compound_add_remove(cp, "matched-remove", 14, 0, 0, 0);
    chimera_vfs_compound_submit(cp, completed, f);
    wait_done(f);
    assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
    chimera_vfs_compound_free(cp);
    ...
    cp = chimera_vfs_compound_alloc(f->thread, root);
    chimera_vfs_compound_add_putfh(cp, f->fh, f->fh_len);
    int                              removed = chimera_vfs_compound_add_remove(cp, "matched-remove", 14,
    ...
    cp = chimera_vfs_compound_alloc(f->thread, root);
    chimera_vfs_compound_add_putfh(cp, f->fh, f->fh_len);
    int lookup = chimera_vfs_compound_add_lookup(cp, "matched-remove", 14,
    ...
    removed                                                  = chimera_vfs_compound_add_remove(cp, "matched-remove", 14,
                                                                                               CHIMERA_VFS_REMOVE_NO_NOTIFY,
                                                                                               0, 0);
...
stream_test_open(
    struct fixture                 *f,
    const struct chimera_vfs_cred  *cred,
    struct chimera_vfs_open_handle *base,
    bool                            create)
{
    struct chimera_vfs_compound *cp = chimera_vfs_compound_alloc(f->thread, cred);
    ...
    int                          opened = chimera_vfs_compound_add_open_stream(cp, "fork", 4,
                                                                               CHIMERA_VFS_OPEN_INFERRED | (create ?
                                                                                                            CHIMERA_VFS_OPEN_CREATE
                                                                                                            |
    ...
    unsigned                        length)
{
    ...
    struct chimera_vfs_compound          *cp   = chimera_vfs_compound_alloc(f->thread, root);
    int                                   read = chimera_vfs_compound_add_read(cp, handle, 0, 8, iov, 16, 0, NULL, NULL,
                                                                               0);
    ...
    const struct chimera_vfs_cred *root)
{
    ...
    int                             opened = chimera_vfs_compound_add_open_at(cp, "overwrite", 9,
                                                                              ...
                                                                              CHIMERA_VFS_OPEN_WRITE_ONLY, &attrs,
                                                                              CHIMERA_VFS_ATTR_FH);
    chimera_vfs_compound_submit(cp, completed, f);
    wait_done(f);
    assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
    ...
    cp = chimera_vfs_compound_alloc(f->thread, root);
    for (unsigned i = 0; i < 3; i++) {
        chimera_vfs_compound_add_write(cp, handles[i], 0, 8, 2, &payload, 1, 0, 0, NULL);
    }
    chimera_vfs_compound_submit(cp, completed, f);
    wait_done(f);
    assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
    ...
    chimera_vfs_compound_free(cp);
    for (unsigned i = 0; i < 3; i++) {
        overwrite_check_data(f, root, handles[i], 8);
    }
    ...
    for (unsigned i = 0; i < 3; i++) {
        chimera_vfs_release(f->thread, handles[i]);
    }
} /* check_overwrite */

static void
check_mkdir_notification(
    struct fixture                *f,
    const struct chimera_vfs_cred *root)
{
    struct remove_test               test   = { .fixture = f };
    struct chimera_vfs_notify_watch *parent = chimera_vfs_notify_watch_create(f->vfs->vfs_notify,
                                                                              f->fh, f->fh_len,
                                                                              CHIMERA_VFS_NOTIFY_DIR_ADDED, 0,
                                                                              remove_notified, &test);
    ...
    int                              created = chimera_vfs_compound_add_create(cp, CHIMERA_VFS_COMPOUND_CREATE_DIR,
                                                                               "quiet-directory", 15, NULL, 0, &attrs,
    ...
    cp = chimera_vfs_compound_alloc(f->thread, root);
    chimera_vfs_compound_add_putfh(cp, f->fh, f->fh_len);
    int looked = chimera_vfs_compound_add_lookup(cp, "quiet-directory", 15, CHIMERA_VFS_ATTR_FH, 0);
    chimera_vfs_compound_submit(cp, completed, f);
    wait_done(f);
    assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
...
static void
check_special_create_notification(
    struct fixture                *f,
    const struct chimera_vfs_cred *root,
    const struct chimera_vfs_cred *user)
{
    ...
    int                          dir = chimera_vfs_compound_add_create(cp, CHIMERA_VFS_COMPOUND_CREATE_DIR,
                                                                       "special-notify", 14, NULL, 0, &attrs,
                                                                       CHIMERA_VFS_ATTR_FH, 0, 0);
    chimera_vfs_compound_submit(cp, completed, f);
    wait_done(f);
    assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
    ...
    for (unsigned sync = 0; sync < 2; sync++) {
        ...
        struct chimera_vfs_notify_watch *watch = chimera_vfs_notify_watch_create(f->vfs->vfs_notify,
                                                                                 fh, fh_len,
                                                                                 CHIMERA_VFS_NOTIFY_FILE_ADDED, 0,
                                                                                 remove_notified, &test);
        assert(watch);
        if (sync) {
            chimera_vfs_notify_watch_set_sync(f->vfs->vfs_notify, watch, &test);
        }
            ...
            for (unsigned symlink = 0; symlink < 2; symlink++) {
                for (unsigned quiet = 0; quiet < 2; quiet++) {
                    ...
                    int  created = chimera_vfs_compound_add_create(cp, symlink ?
                                                                   ...
                                                                   CHIMERA_VFS_COMPOUND_CREATE_NODE, name, len, symlink
                    ? "target" : NULL, symlink ? 6 : 0, &attrs, CHIMERA_VFS_ATTR_FH | CHIMERA_VFS_ATTR_MODE, 0, 0);
                    if (quiet) {
                    ...
                    cp = chimera_vfs_compound_alloc(f->thread, cred);
                    chimera_vfs_compound_add_putfh(cp, fh, fh_len);
                    int looked = chimera_vfs_compound_add_lookup(cp, name, len, CHIMERA_VFS_ATTR_FH, 0);
                    chimera_vfs_compound_submit(cp, completed, f);
                    wait_done(f);
                    assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
    ...
    cp            = chimera_vfs_compound_alloc(f->thread, root);
    chimera_vfs_compound_add_putfh(cp, fh, fh_len);
    dir = chimera_vfs_compound_add_create(cp, CHIMERA_VFS_COMPOUND_CREATE_DIR, "private", 7, NULL, 0, &attrs,
                                          CHIMERA_VFS_ATTR_FH, 0, 0);
    chimera_vfs_compound_submit(cp, completed, f);
    wait_done(f);
    assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
    ...
    for (unsigned symlink = 0; symlink < 2; symlink++) {
        ...
        int created = chimera_vfs_compound_add_create(cp, symlink ?
                                                      CHIMERA_VFS_COMPOUND_CREATE_SYMLINK :
                                                      CHIMERA_VFS_COMPOUND_CREATE_NODE, "denied", 6, symlink ?
                                                      "target" : NULL, symlink ? 6 : 0, &attrs, CHIMERA_VFS_ATTR_FH, 0,
                                                      0);
        chimera_vfs_compound_op_args(cp, created)->namespace_flags = symlink ?
            CHIMERA_VFS_SYMLINK_NO_NOTIFY : CHIMERA_VFS_MKNOD_NO_NOTIFY;
        chimera_vfs_compound_submit(cp, completed, f);
...
} /* rename_backend_complete */

static struct chimera_vfs_compound *
rename_compound(
    struct fixture                 *f,
    const struct chimera_vfs_cred  *cred,
    struct chimera_vfs_open_handle *expected,
    const char                     *destination)
{
    ...
    int                             renamed = chimera_vfs_compound_add_rename(cp, "matched-remove", 14, destination,
                                                                              strlen(destination),
    ...
    struct chimera_vfs_open_handle *expected)
{
    ...
    chimera_vfs_compound_add_putfh(cp, f->fh, f->fh_len);
    int                                   looked = chimera_vfs_compound_add_lookup(cp, name, strlen(name),
                                                                                   CHIMERA_VFS_ATTR_FH, 0);
    chimera_vfs_compound_submit(cp, completed, f);
    wait_done(f);
    ...
    const struct chimera_vfs_cred *cred)
{
    ...
    cp = chimera_vfs_compound_alloc(f->thread, cred);
    chimera_vfs_compound_add_putfh(cp, f->fh, f->fh_len);
    chimera_vfs_compound_add_setattr(cp, NULL, &public_dir, 0, 0);
    ...
    for (unsigned int unprivileged = 0; unprivileged < 2; unprivileged++) {
        ...
        chimera_vfs_rename_at_checked_result(f->thread, unprivileged ? &user : cred,
                                             f->fh, f->fh_len, "quiet-renamed", 13, f->fh, f->fh_len, "matched-remove",
                                             14,
    ...
    chimera_vfs_compound_add_savefh(cp);
    chimera_vfs_compound_add_putfh(cp, f->fh, f->fh_len);
    chimera_vfs_compound_add_link(cp, "matched-alias", 13, 0, 0, 0);
    chimera_vfs_compound_submit(cp, completed, f); wait_done(f);
    assert(chimera_vfs_compound_status(cp) == CHIMERA_VFS_OK);
    chimera_vfs_compound_free(cp);
    test.notifications = 0;
    parent             = chimera_vfs_notify_watch_create(f->vfs->vfs_notify,
                                                         f->fh, f->fh_len, CHIMERA_VFS_NOTIFY_RENAMED, 0,
                                                         remove_notified, &test);
    assert(parent);
    chimera_vfs_notify_watch_set_sync(f->vfs->vfs_notify, parent, &test);
    chimera_vfs_rename_at_checked_result(f->thread, cred,
...
static void
coordinate_identity_bad_prepare(
    struct chimera_vfs_compound *cp,
    uint32_t                     index,
    enum chimera_vfs_error      *status,
    void                        *private)
    ...
    const struct chimera_vfs_cred *cred)
{
    struct chimera_vfs_file_state *file = chimera_vfs_state_get(f->thread->vfs->vfs_state,
                                                                f->fh, f->fh_len, chimera_vfs_hash(f->fh, f->fh_len),
                                                                true);
    ...
    for (unsigned mode = 0; mode < 6; mode++) {
        ...
        chimera_vfs_compound_free(cp);
        if (!test.retired) {
            assert(chimera_vfs_claim_owner_retire(test.owner, NULL, NULL));
        }
    ...
    * group do not execute, and an unrelated final group still succeeds. */
    ...
    for (unsigned dynamic = 0; dynamic < 2; dynamic++) {
        ...
        if (dynamic) {
            ...
            f.dynamic_open = chimera_vfs_compound_add_open(cp, NULL, 0, CHIMERA_VFS_OPEN_PATH |
                                                           CHIMERA_VFS_OPEN_DIRECTORY, 0, NULL, 0, 0, 0);
        }
        add_group(cp, 0, 2, NULL, -1, true, &f);
    ...
    cp = chimera_vfs_compound_alloc(f.thread, &root);
    chimera_vfs_compound_add_putfh(cp, f.fh, f.fh_len);
    int producer = chimera_vfs_compound_add_open(cp, NULL, 0, CHIMERA_VFS_OPEN_PATH | CHIMERA_VFS_OPEN_DIRECTORY, 0,
                                                 NULL, 0, 0, 0);
