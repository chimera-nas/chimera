...
/*
 * Real-dirfd fchmodat: the open_at + setattr chain runs as ONE caller-owned
...
struct chimera_posix_fchmodat_ctx {
     ...
     * non-NULL set_attr; nothing is created, so nothing is applied). */
...
static void
chimera_posix_fchmodat_end_callback(
 ...
 * status, upgraded to the commit status when only the commit fails (e.g. the
 * retriable ECOMPOUND_EXHAUSTED, EAGAIN at this layer). */
static void
chimera_posix_fchmodat_finish(
    ...
    void                     *private_data)
{
    ...
    chimera_posix_fchmodat_finish(ctx, error_code);
} /* chimera_posix_fchmodat_setattr_complete */
    ...
    struct chimera_client_request *request)
{
    ...
    /* Set up the scratch set_attr for the open_at */
