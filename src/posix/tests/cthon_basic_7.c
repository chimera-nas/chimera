...
// Test rename and link
// Based on cthon/basic/test7.c from Connectathon 2004
//
    ...
    char **argv)
{
    ...
    fprintf(stdout, "%s: rename and link\n", cthon_Myname);
    ...
    for (ct = 0; ct < count; ct++) {
        for (fi = 0; fi < files; fi++) {
            ...
            // Create hard link back to original name
            if (chimera_posix_link(new, str) < 0) {
                cthon_error("can't link %s to %s", new, str);
                posix_test_fail(&env);
            }

            // Verify link count
            if (chimera_posix_stat(str, &statb) < 0) {
                cthon_error("can't stat %s after link", str);
                posix_test_fail(&env);
            }
            if (statb.st_nlink != 2) {
            ...
            if (chimera_posix_unlink(new) < 0) {
                cthon_error("can't unlink %s", new);
                posix_test_fail(&env);
            }

            // Verify link count back to 1
            if (chimera_posix_stat(str, &statb) < 0) {
                cthon_error("can't stat %s after unlink", str);
                posix_test_fail(&env);
            }
            if (statb.st_nlink != 1) {
