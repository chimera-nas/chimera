    ...
    char **argv)
{
    ...
    fprintf(stderr, "Testing fsync...\n");

    data_len = strlen(test_data);

    // Create test file
    fd = chimera_posix_open("/test/fsync_test", O_CREAT | O_RDWR | O_TRUNC, 0644);
    ...
    fprintf(stderr, "fsync on open file passed\n");

    // Write more data and fsync again
    if (chimera_posix_write(fd, test_data, data_len) != (ssize_t) data_len) {
    ...
    if (rc != 0) {
        fprintf(stderr, "second fsync failed: %s\n", strerror(errno));
        chimera_posix_close(fd);
        posix_test_fail(&env);
    }

    fprintf(stderr, "second fsync passed\n");

    chimera_posix_close(fd);

    fprintf(stderr, "fsync test passed\n");
