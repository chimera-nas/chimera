...
// Timing support
void cthon_starttime(void);
void cthon_endtime(struct timeval *tv);

// Error reporting
void cthon_error(const char *fmt, ...);

// Parameter parsing
long cthon_getparm(const char *parm, long min, const char *label);

// Directory tree operations
void cthon_dirtree(int lev, int files, int dirs, const char *fname,
                   const char *dname, int *totfiles, int *totdirs);
void cthon_rmdirtree(int lev, int files, int dirs, const char *fname,
                     const char *dname, int *totfiles, int *totdirs, int ignore);

// Test directory setup
void cthon_testdir(const char *dir);
int cthon_mtestdir(const char *dir);

// Completion
void cthon_complete(void);

// Path manipulation helpers
void cthon_pushdir(const char *name);
void cthon_popdir(void);
const char *cthon_getcwd(void);
void cthon_setcwd(const char *path);

// Wrapper for creating files (creat() equivalent)
int cthon_creat(const char *path, mode_t mode);
