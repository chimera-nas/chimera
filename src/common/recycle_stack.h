...
static inline void chimera_stack_init(chimera_stack *s)
{
    evpl_mutex_init(&s->lock, NULL);
    s->head = NULL;
}
static inline void chimera_stack_destroy(chimera_stack *s)
{
    evpl_mutex_destroy(&s->lock);
}
static inline void chimera_stack_node_init(chimera_stack_node *n)
{
    n->next = NULL;
}
static inline void chimera_stack_push(chimera_stack *s, chimera_stack_node *n)
{
...
static inline void chimera_stack_pop_lock(chimera_stack *s)
{
    evpl_mutex_lock(&s->lock);
}
static inline void chimera_stack_pop_unlock(chimera_stack *s)
{
    evpl_mutex_unlock(&s->lock);
}
/* Caller holds the consumer lock for the entire magazine refill. */
static inline chimera_stack_node *chimera_stack_pop_locked(chimera_stack *s)
{
...
static inline chimera_stack_batch *chimera_stack_pop_all(chimera_stack *s)
{
    chimera_stack_node *head;
    evpl_mutex_lock(&s->lock);
    head = s->head;
