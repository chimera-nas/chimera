...
chimera_native_compare(volatile void *p, uint64_t expected, uint64_t desired, size_t size)
{
    switch (size) {
    ...
    case 8: return (uint64_t) _InterlockedCompareExchange64((volatile __int64 *) p, (__int64) desired, (__int64) expected);
    default: abort();
    }
}

static inline uint64_t
chimera_native_exchange(volatile void *p, uint64_t desired, size_t size)
{
    switch (size) {
...
static inline uint64_t
chimera_native_add(volatile void *p, uint64_t value, size_t size)
{
    switch (size) {
...
static inline uint64_t
chimera_native_add_result(volatile void *p, uint64_t value, size_t size)
{
    return chimera_native_add(p, value, size) + value;
}

static inline uint64_t
chimera_native_or(volatile void *p, uint64_t value, size_t size)
{
    switch (size) {
...
static inline uint64_t
chimera_native_and(volatile void *p, uint64_t value, size_t size)
{
    switch (size) {
...
static inline int
chimera_native_cas(volatile void *p, void *expected, uint64_t desired, size_t size)
{
