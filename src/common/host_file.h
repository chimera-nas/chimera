...
static inline DWORD chimera_host_permission_acl(PSID owner, unsigned mode, PACL *acl)
{
...
static inline int chimera_host_create_private(const char *path)
{
    ...
    if (!GetTokenInformation(token, TokenUser, user, bytes, &bytes)) { goto done; }
    ...
    if (fd < 0) { CloseHandle(file); error = ERROR_TOO_MANY_OPEN_FILES; }
