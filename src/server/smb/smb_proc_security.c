    ...
    void                        *private_data)
{
    ...
    if (request->smb2_hdr.command == SMB2_SET_INFO) {
        ...
        if (!chimera_vfs_identity_cached(vfs, CHIMERA_VFS_IDENTITY_BY_UID, attr->va_uid, NULL)) {
            ctx->pending++;
            chimera_vfs_identity_resolve(thread, CHIMERA_VFS_IDENTITY_BY_UID, attr->va_uid,
                                         NULL, smb_security_identity_done, ctx);
        }
        if (!chimera_vfs_identity_cached(vfs, CHIMERA_VFS_IDENTITY_BY_GID, attr->va_gid, NULL)) {
            ctx->pending++;
    ...
    struct smb_vfs_command      *command)
{
    ...
    int set = chimera_vfs_compound_add_setattr(compound, command->handle, NULL, 0);
    ...
    void                        *private_data)
{
    ...
    int                             length = chimera_smb_acl_to_sd(attr->va_uid, attr->va_gid, attr->va_mode & (S_IFMT |
                                                                                                                07777),
                                                                   acl,
                                                                   !!(info & OWNER_SECURITY_INFORMATION), !!(info &
                                                                                                             GROUP_SECURITY_INFORMATION),
                                                                   !!(info & DACL_SECURITY_INFORMATION), request->
                                                                   query_info.sec_buf,
