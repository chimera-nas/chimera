...
static void chimera_smb_teardown_doc_start(
    void *private_data);
static void chimera_smb_teardown_stream_done(
...
} /* smb_open_retire_finish */

static void
smb_open_retire_tick(
    struct evpl       *evpl,
    ...
    void *private_data)
{
    ...
    if (open->range_owner) {
        chimera_vfs_claim_owner_retire(open->range_owner, NULL, NULL);
    }
    smb_open_retire_tick(thread->evpl, &open->retire_timer);
} /* chimera_smb_open_file_retire_async */

static void
smb_retire_drain_wakeup(
    struct evpl       *evpl,
    struct evpl_timer *timer)
{
    (void) evpl;
...
} /* chimera_smb_open_file_retire_drain */
    ...
    void                                        *private_data)
{
    ...
    if (scan->consume) {
        ...
        if (peer->parent_fh_len == doc->parent_fh_len && peer->name_len == doc->name_len &&
            ...
            !memcmp(peer->name, doc->name, peer->name_len)) {
    ...
    const struct chimera_vfs_doc_info *doc)
{
    ...
    if (doc->pending_state) {
        ...
        evpl_mutex_lock(&doc->pending_state->lock);
        ...
        evpl_mutex_unlock(&doc->pending_state->lock);
...
static void
smb_teardown_doc_compound_done(
    ...
    void                        *private_data)
{
    ...
    if (status != CHIMERA_VFS_OK) {
        chimera_smb_debug("teardown delete-on-close: remove '%.*s' failed (%d)",
...
chimera_smb_teardown_doc_start(void *private_data)
{
    ...
    struct chimera_vfs_compound         *compound = chimera_vfs_compound_alloc(
        ctx->vfs_thread, &ctx->doc_info.cred);
    chimera_vfs_compound_add_putfh(compound, ctx->doc_info.parent_fh,
                                   ctx->doc_info.parent_fh_len);
    int                                  op = chimera_vfs_compound_add_remove(compound, ctx->doc_info.name,
...
chimera_smb_close(struct chimera_smb_request *request)
{
    ...
    evpl_mutex_lock(&request->close.open_file->tree->open_files_lock[bucket]);
    chimera_smb_lock_abort_parked(request->close.open_file);
    evpl_mutex_unlock(&request->close.open_file->tree->open_files_lock[bucket]);
...
smb_close_cache_present(struct smb_vfs_command *command)
{
    ...
    if (!open || (command->state && command->state->producer)) {
        return false;
    }
...
smb_close_cache_pin(struct smb_vfs_command *command)
{
    ...
    if (!(open->flags & CHIMERA_SMB_OPEN_FILE_CLOSED) && open->grant) {
        ...
        if (valid) {
            ...
            if (valid) {
                attempt->cache_grant = chimera_vfs_claim_pin_grant(&open->grant->claim);
            }
            evpl_mutex_unlock(&file->lock);
...
} /* chimera_smb_close_compound_uses_doc */

static bool smb_close_has_doc(
...
} /* smb_close_compound_bound_eligible */
...
} /* smb_close_compound_file_id */

static void
smb_close_attrs_complete(
    struct chimera_vfs_compound *compound,
    uint32_t                     index,
    enum chimera_vfs_error      *status,
    void                        *private_data)
    ...
    void                        *private_data)
{
    ...
    (void) compound; (void) index;
    struct smb_close_attempt *attempt = command->private_data;
    if (attempt && attempt->doc_mode) {
        smb_doc_close_complete(compound, index, status, command);
    }
    if (*status == CHIMERA_VFS_OK) {
...
} /* smb_close_compound_ends_batch */

static void
smb_close_compound_publish(
    struct chimera_vfs_compound *compound,
    struct smb_vfs_command      *command)
{
    struct smb_close_attempt *attempt = command->private_data;
    ...
    if (attempt && attempt->committed) {
        ...
        if (attempt->doc_mode) {
            smb_doc_close_publish(compound, command);
        }
...
smb_close_compound_release(struct smb_vfs_command *command)
{
    ...
    if (attempt) {
        ...
        if (attempt->cache_file) {
            chimera_vfs_state_put(state, attempt->cache_file);
        }
...
chimera_smb_close_release(struct chimera_smb_request *request)
{
    ...
    chimera_vfs_claim_access_owner_retire(open->base_access_owner);
    if (owner) {
        chimera_vfs_claim_owner_retire(owner, NULL, NULL);
    }
