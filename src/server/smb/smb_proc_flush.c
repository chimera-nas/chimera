...
static int
smb_flush_compound_build(struct chimera_vfs_compound *compound,
                         struct smb_vfs_command *command)
{
    (void) command;
    return chimera_vfs_compound_add_commit(compound, 0, UINT64_MAX, 0);
}

static void
smb_flush_compound_prepare(struct chimera_vfs_compound *compound, uint32_t index,
