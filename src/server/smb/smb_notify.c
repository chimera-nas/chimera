...
{
    if (!state || atomic_fetch_sub(&state->refs, 1) != 1) { return; }
    ...
    if (state->watch) { chimera_vfs_notify_watch_destroy(state->vfs_notify, state->watch); }
...
uint32_t
chimera_smb_notify_admit(struct chimera_server_smb_thread *thread,
    ...
    struct chimera_smb_notify_state **result)
{
    ...
    *result = NULL;
    ...
    if ((open->flags & CHIMERA_SMB_OPEN_FILE_CLOSED) || !open->handle) {
        ...
        if (closing) { status = SMB2_STATUS_FILE_CLOSED; }
        else {
        ...
        if (!state) { status = SMB2_STATUS_INSUFFICIENT_RESOURCES; }
        else {
            ...
            } else { open->notify_state = state; *result = state; }
        }
    }
    pthread_mutex_unlock(&open->tree->open_files_lock[bucket]);
 ...
 * ---------------------------------------------------------------- */
static void
smb_notify_fail(struct chimera_smb_notify_request *nr, uint32_t status)
{
    struct evpl_iovec iov;
    uint8_t          *buf, *p;
    int               smb2_len;
...
static void
smb_notify_admission_tick(struct evpl *evpl, struct evpl_timer *timer)
{
    ...
    uint32_t status = chimera_smb_notify_admit(nr->thread,
        session_handle && session_handle->is_channel ? session_handle->session : NULL, nr->open_file,
        chimera_smb_map_completion_filter(nr->completion_filter), nr->watch_tree, &state);
    if (status == SMB2_STATUS_PENDING) {
        evpl_add_oneshot_timer(evpl, timer, smb_notify_admission_tick, 1000);
        return;
    }
    if (status != SMB2_STATUS_SUCCESS) { smb_notify_fail(nr, status); return; }
    pthread_mutex_lock(&state->lock);
    if (state->closing) {
        pthread_mutex_unlock(&state->lock);
