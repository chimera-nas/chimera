    ...
    struct chimera_smb_open_file     *open_file)
{
    ...
    if (grant) chimera_smb_grant_remove_member(grant, open_file);
    ...
    if (grant) chimera_vfs_claim_grant_release(state, grant, true /* pump */);
    ...
    struct chimera_smb_open_file     *open_file)
{
    ...
    if (base_handle) chimera_vfs_release(thread->vfs_thread, base_handle);
...
static void
smb_lock_compound_prepare(struct chimera_vfs_compound *compound, uint32_t index,
                          enum chimera_vfs_error *status, void *private_data)
{
    ...
    if (command->state->flags & CHIMERA_SMB_OPEN_FILE_FLAG_DIRECTORY) {
        ...
        if (!lock->replay) {
            ...
            if (!lock->count && lock->post_status) command->status = lock->post_status;
        }
    }
    if (command->status != SMB2_STATUS_SUCCESS) *status = CHIMERA_VFS_EINVAL;
}

static void
smb_lock_compound_coordinate(struct chimera_vfs_compound *compound, uint32_t index,
                             ...
                             void *private_data)
{
    ...
    if (!command->state->producer) {
        ...
        if (open->flags & CHIMERA_SMB_OPEN_FILE_CLOSED) {
            ...
            if (!open->range_owner) status = CHIMERA_VFS_ENOSPC;
        }
...
static void
smb_lock_owner_prepare(struct chimera_vfs_compound *compound, uint32_t index,
                        enum chimera_vfs_error *status, void *private_data)
{
    struct smb_vfs_command *command = private_data;
    struct smb_lock_compound *lock = command->private_data;
...
static void
smb_lock_owner_complete(struct chimera_vfs_compound *compound, uint32_t index,
                         enum chimera_vfs_error *status, void *private_data)
{
    struct smb_vfs_command *command = private_data;
...
static void
smb_lock_compound_range_prepare(struct chimera_vfs_compound *compound, uint32_t index,
                                enum chimera_vfs_error *status, void *private_data)
{
    struct smb_vfs_command *command = private_data;
    struct smb_lock_compound *lock = command->private_data;
    ...
    if (lock->replay) {
        ...
        if (!command->state->producer) command->state->range_owner = command->open->range_owner;
...
static bool
smb_lock_compound_is_canceled(struct chimera_vfs_compound *compound, uint32_t index,
                              void *private_data)
{
    struct smb_vfs_command *command = private_data;
    struct chimera_smb_request *request = command->request;
...
static void
smb_lock_compound_wait(struct chimera_vfs_compound *compound, uint32_t index, void *private_data)
{
    struct smb_vfs_command *command = private_data;
    struct chimera_smb_request *request = command->request;
    struct chimera_smb_open_file *open = command->open;
    ...
    if (!open->parked_lock_req) open->parked_lock_req = request;
...
static void
smb_lock_compound_complete(struct chimera_vfs_compound *compound, uint32_t index,
                           enum chimera_vfs_error *status, void *private_data)
{
    struct smb_vfs_command *command = private_data;
    struct smb_lock_compound *lock = command->private_data;
...
static int
smb_lock_compound_build(struct chimera_vfs_compound *compound, struct smb_vfs_command *command)
{
    struct chimera_smb_request *request = command->request;
    ...
    if (command->state && command->state->producer) {
        ...
        if (owner >= 0) chimera_vfs_compound_set_op_callbacks(compound, owner,
...
smb_lock_compound_release(struct smb_vfs_command *command)
{
    ...
    if (request->lock.vfs_compound && open) {
        unsigned bucket = open->file_id.vid & CHIMERA_SMB_OPEN_FILE_BUCKET_MASK;
        pthread_mutex_lock(&open->tree->open_files_lock[bucket]);
        if (open->parked_lock_req == request) open->parked_lock_req = NULL;
