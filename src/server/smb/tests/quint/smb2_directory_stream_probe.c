    ...
    unsigned int            count)
{
    ...
    body[3]                                        = SMB2_FILE_STANDARD_INFO_T; p32(body, 4, 4096); memset(body + 24,
                                                                                                           0xff, 16);
    offset += 104;
    header  = wire + offset;
...
main(void)
{
    ...
    uint32_t               bytes;
    assert(smb2_create_opts(conn, "directory", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                            MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, NULL, &base) == ST_SUCCESS);
    assert(smb2_create(conn, "directory\\child", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, NULL, &child) == ST_SUCCESS);
    assert(smb2_write(conn, child.file_id, 0, "child", 5, &bytes) == ST_SUCCESS);
    assert(smb2_create(conn, "directory:sibling", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, NULL, &sibling) == ST_SUCCESS);
    assert(smb2_write(conn, sibling.file_id, 0, "sibling", 7, &bytes) == ST_SUCCESS);

    chain(conn, "directory:new:$DATA", MBT_FILE_CREATE, MBT_FILE_NON_DIRECTORY_FILE, 2, 0, true, NULL, 0);
    chain(conn, "directory:new:$DATA", MBT_FILE_OPEN, MBT_FILE_NON_DIRECTORY_FILE, 1, 4, false, NULL, 0);
    chain(conn, "directory:new:$DATA", MBT_FILE_OPEN_IF, 0, 1, 4, true, NULL, 0);
    chain(conn, "directory:open-if", MBT_FILE_OPEN_IF, MBT_FILE_NON_DIRECTORY_FILE, 2, 0, true, NULL, 0);
    const uint32_t dispositions[] = { MBT_FILE_OVERWRITE, MBT_FILE_OVERWRITE_IF, MBT_FILE_SUPERSEDE };
    for (unsigned int i = 0; i < 3; i++) {
        chain(conn, "directory:new:$DATA", dispositions[i], MBT_FILE_NON_DIRECTORY_FILE,
              dispositions[i] == MBT_FILE_SUPERSEDE ? 0 : 3, 0, true, NULL, 0);
        if (dispositions[i] != MBT_FILE_OVERWRITE) {
            char name[64]; snprintf(name, sizeof(name), "directory:created-%u", i);
            chain(conn, name, dispositions[i], MBT_FILE_NON_DIRECTORY_FILE, 2, 0, false, NULL, 0);
        }
    ...
    assert(smb2_create(conn, "directory:missing", MBT_FILE_OVERWRITE, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, NULL, &rejected) == ST_OBJECT_NAME_NOT_FOUND);
    atomic_store(&armed, 0); assert(atomic_load(&submissions) == 1);
    assert(smb2_create(conn, "directory:missing", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, NULL, &rejected) == ST_OBJECT_NAME_NOT_FOUND);

    /* A stream SHARE denial still happens before overwrite. */
    assert(smb2_create(conn, "directory:new", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_READ, NULL, &seed) == ST_SUCCESS);
    assert(smb2_create(conn, "directory:new", MBT_FILE_OVERWRITE, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, NULL, &rejected) == ST_SHARING_VIOLATION);
    ...
    assert(smb2_create_opts(conn, "directory:new", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS,
                            MBT_FILE_SHARE_RWD, MBT_FILE_NON_DIRECTORY_FILE | 0x00200000u, NULL, &seed) == ST_SUCCESS);
    assert(seed.end_of_file == 4);
    assert(!(g32(conn->rbuf + 4 + SMB2_HDR_SIZE, 56) & SMB2_FILE_ATTRIBUTE_DIRECTORY));
    assert_bytes(conn, seed.file_id, "fork");
    assert(smb2_close(conn, seed.file_id) == ST_SUCCESS);
    assert(smb2_create_opts(conn, "directory:doc", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                            MBT_FILE_SHARE_RWD, MBT_FILE_NON_DIRECTORY_FILE | MBT_FILE_DELETE_ON_CLOSE, NULL, &seed) ==
           ST_SUCCESS);
    assert(smb2_write(conn, seed.file_id, 0, "doc", 3, &bytes) == ST_SUCCESS);
    assert(smb2_close(conn, seed.file_id) == ST_SUCCESS);
    assert(smb2_create(conn, "directory:doc", MBT_FILE_OPEN, MBT_FILE_READ_ACCESS,
                       MBT_FILE_SHARE_RWD, NULL, &rejected) == ST_OBJECT_NAME_NOT_FOUND);
    ...
    assert(smb2_create_dur(holder, "directory:durable", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                           MBT_FILE_SHARE_RWD, &caching, &durable, &seed) == ST_SUCCESS && seed.has_dhnq);
    ...
    assert(smb2_create_dur(reconnected, "", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS,
                           MBT_FILE_SHARE_RWD, NULL, &durable, &seed) == ST_SUCCESS);
    ...
    assert(smb2_create_opts(conn, "directory:new", MBT_FILE_OPEN, MBT_FILE_READ_ACCESS,
                            MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, NULL, &rejected) == 0xc0000103u); /* NOT_A_DIRECTORY */
    assert(smb2_create_opts(conn, "directory::$DATA", MBT_FILE_OPEN, MBT_FILE_READ_ACCESS,
                            MBT_FILE_SHARE_RWD, 0, NULL, &rejected) == 0xc00000bau); /* FILE_IS_A_DIRECTORY */
