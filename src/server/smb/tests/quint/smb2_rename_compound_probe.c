...
{
    typedef void (*create_fn)(struct chimera_smb_request *);
    create_fn next = (create_fn) dlsym(RTLD_NEXT, "chimera_smb_create");
    assert(next);
    if (atomic_load(&watch_disconnected_suffix)) atomic_fetch_add(&suffix_dispatches, 1);
    next(request);
}
...
{
    typedef void (*complete_fn)(struct chimera_smb_request *, unsigned int);
    ...
    next(request, status);
    if (drained) atomic_store(&suffix_drained, 1);
}
...
{
    typedef bool (*begin_fn)(struct chimera_smb_namespace_open_token *,
        struct chimera_smb_namespace_registry *, const void *);
    begin_fn next = (begin_fn) dlsym(RTLD_NEXT, "chimera_smb_namespace_open_begin");
    assert(next);
    bool ready = next(token, registry, owner);
    if (!ready) atomic_fetch_add(&constructor_denials, 1);
    return ready;
}
...
static void
rename_poll(struct evpl *evpl, struct evpl_timer *timer)
{
    ...
    __typeof__(&chimera_vfs_rename_at_checked_result_actor) next =
        (__typeof__(&chimera_vfs_rename_at_checked_result_actor)) dlsym(RTLD_NEXT, "chimera_vfs_rename_at_checked_result_actor");
    assert(next);
...
__attribute__((visibility("default"))) void
chimera_vfs_rename_at_checked_result_actor(struct chimera_vfs_thread *thread,
    const struct chimera_vfs_cred *cred, const void *fh, int fh_len,
    const char *name, int name_len, const void *new_fh, int new_fh_len,
    const char *new_name, int new_name_len, const uint8_t *target_fh, int target_fh_len,
    unsigned int flags, uint64_t pre_mask, uint64_t post_mask, const uint8_t *lease_key,
    struct chimera_vfs_open_handle *handle, const struct chimera_claim_actor *actor,
    const uint8_t *match_fh, uint32_t match_fh_len,
    enum chimera_vfs_rename_outcome *outcome,
    chimera_vfs_rename_at_callback_t callback, void *private_data)
{
    __typeof__(&chimera_vfs_rename_at_checked_result_actor) next =
        (__typeof__(&chimera_vfs_rename_at_checked_result_actor)) dlsym(RTLD_NEXT, "chimera_vfs_rename_at_checked_result_actor");
    assert(next);
    if (atomic_load(&scan_mode)) atomic_fetch_add(&scan_mutations, 1);
    ...
    if (((flags & CHIMERA_VFS_RENAME_NOREPLACE) && atomic_exchange(&hold_rename, 0)) ||
        ...
        if (actor) rename_gate.actor = *actor;
...
static void
open_attr_copy(struct chimera_vfs_attrs *dst, const struct chimera_vfs_attrs *src)
{
...
static void
open_result_poll(struct evpl *evpl, struct evpl_timer *timer)
{
...
static void
open_result_done(enum chimera_vfs_error status, struct chimera_vfs_open_handle *handle,
    struct chimera_vfs_attrs *set, struct chimera_vfs_attrs *attr,
...
__attribute__((visibility("default"))) void
chimera_vfs_open_at(struct chimera_vfs_thread *thread, const struct chimera_vfs_cred *cred,
    struct chimera_vfs_open_handle *parent, const char *name, int name_len, unsigned int flags,
    struct chimera_vfs_attrs *set, uint64_t mask, uint64_t pre_mask, uint64_t post_mask,
    chimera_vfs_open_at_callback_t callback, void *private_data)
{
    ...
    if (atomic_load(&watch_disconnected_suffix) &&
        ...
        !memcmp(name, "mutation-abandoned-suffix", name_len)) atomic_fetch_add(&suffix_backend_opens, 1);
...
__attribute__((visibility("default"))) void
chimera_vfs_notify_emit_lease(struct chimera_vfs_notify *notify, const uint8_t *fh,
    uint16_t fh_len, uint32_t action, const char *name, uint16_t name_len,
    const char *old_name, uint16_t old_name_len, uint64_t skip_lo, uint64_t skip_hi, bool has_skip)
{
    typedef void (*notify_fn)(struct chimera_vfs_notify *, const uint8_t *, uint16_t,
        uint32_t, const char *, uint16_t, const char *, uint16_t, uint64_t, uint64_t, bool);
...
__attribute__((visibility("default"))) void
chimera_vfs_notify_emit_actor(struct chimera_vfs_notify *notify, const uint8_t *fh,
    uint16_t fh_len, uint32_t action, const char *name, uint16_t name_len,
    const char *old_name, uint16_t old_name_len, const struct chimera_claim_actor *actor)
{
    typedef void (*notify_fn)(struct chimera_vfs_notify *, const uint8_t *, uint16_t,
        uint32_t, const char *, uint16_t, const char *, uint16_t, const struct chimera_claim_actor *);
...
static void
scan_poll(struct evpl *evpl, struct evpl_timer *timer)
{
...
static void
scan_page_reset(struct chimera_vfs_compound *compound, uint32_t index, void *arg)
{
    (void) arg;
    scan_page_entries = 0;
...
static int
scan_page_append(struct chimera_vfs_compound *compound, uint32_t index,
    uint64_t inum, uint64_t cookie, const char *name, int namelen,
    const struct chimera_vfs_attrs *attrs, void *arg)
{
    (void) arg;
    assert(!scan_page_entries++);
    assert(namelen == 1 && (name[0] == 'a' || name[0] == 'b'));
    if (!scan_first_child) scan_first_child = name[0];
...
static void
scan_finish(struct chimera_vfs_compound *compound, void *arg)
{
    (void) arg;
    for (uint32_t i = 0; i < chimera_vfs_compound_num_ops(compound); i++) {
...
__attribute__((visibility("default"))) void
chimera_vfs_recall_caching_fh(struct chimera_vfs_thread *thread,
    const struct chimera_vfs_cred *cred, const uint8_t *fh, uint32_t fh_len,
    chimera_vfs_recall_fh_callback_t callback, void *private_data)
{
    ...
    assert(next);
    if (atomic_load(&scan_mode)) atomic_fetch_add(&scan_recalls, 1);
...
static void
cancel_initial_move_complete(struct chimera_vfs_compound *compound, uint32_t index,
    ...
    void                           *private_data)
{
    ...
    if (atomic_load(&cancel_initial_move)) {
        ...
        for (uint32_t i = 0; i < chimera_vfs_compound_num_ops(cp); i++) {
            const struct chimera_vfs_compound_op *op = chimera_vfs_compound_op(cp, i);
            if (op->type != CHIMERA_VFS_COMPOUND_OP_RENAME) continue;
...
static void
send_runs_groups(struct packet *p, struct smb2_conn *c, unsigned int runs, int groups)
{
...
static void
send_runs(struct packet *p, struct smb2_conn *c, unsigned int runs)
{
    send_runs_groups(p, c, runs, runs == 1 ? (int) p->count : -1);
}

static void doc_send(struct packet *p, struct smb2_conn *c) { send_runs(p, c, 1); }
...
static void
replace_op(struct packet *p, struct smb2_conn *c, const uint8_t fid[16], const char *name)
{
...
stat_open(
    ...
    const char       *name)
{
    create_op(p, c, name, FILE_OPEN, 0x80u);
} /* stat_open */
...
provisional_rename_cases(struct smb2_conn *c)
{
    ...
    for (unsigned int existing = 0; existing < 2; existing++) {
        ...
        if (existing) {
            assert(smb2_create(c, old, FILE_CREATE, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
            assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
        }
        memset(&p, 0, sizeof(p));
        create_op(&p, c, old, existing ? FILE_OPEN : FILE_CREATE, FILE_ALL_ACCESS);
        ...
        assert(smb2_create(c, old, FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &probe) == ST_OBJECT_NAME_NOT_FOUND
               );
        assert(smb2_create(c, next, FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
        assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
    }
    /* Failed RENAME does not invalidate the produced handle or undo CREATE. */
    memset(&p, 0, sizeof(p));
    create_op(&p, c, "provisional-collision", FILE_CREATE, FILE_ALL_ACCESS);
    ...
    * admission before replaying the producer's OPEN/cache coordination. */
    memset(&p, 0, sizeof(p));
    create_op(&p, c, "provisional-collision", FILE_OPEN, FILE_ALL_ACCESS);
...
replace_if_exists_cases(struct smb2_conn *c)
{
    ...
    assert(smb2_create(c, "replace-absent-old", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &base) == ST_SUCCESS);
    hardlink(c, base.file_id, "replace-absent-alias");
    assert(smb2_create(c, "replace-absent-alias", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &alias) == ST_SUCCESS);
    assert(smb2_create(c, "replace-absent-old:fork:$DATA", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &stream) == ST_SUCCESS);
    ...
    memset(&p, 0, sizeof(p));
    create_op(&p, c, "replace-producer-old", FILE_CREATE, FILE_ALL_ACCESS);
    ...
    assert(smb2_create(c, "replace-occupied", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &target) == ST_SUCCESS);
    assert(smb2_write(c, target.file_id, 0, "target", 6, &bytes) == ST_SUCCESS && bytes == 6);
    memset(&p, 0, sizeof(p));
    create_op(&p, c, "replace-prefix", FILE_CREATE, FILE_ALL_ACCESS);
    ...
    assert(smb2_create(c, "replace-prefix", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &probe) == ST_OBJECT_NAME_NOT_FOUND);
    ...
    assert(smb2_create(c, "replace-unheld-target", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &target) == ST_SUCCESS);
    assert(smb2_close(c, target.file_id) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    create_op(&p, c, "replace-unheld-source", FILE_CREATE, FILE_ALL_ACCESS);
    ...
    assert(smb2_create(c, "replace-unheld-source", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &probe) == ST_OBJECT_NAME_NOT_FOUND);
    ...
    assert(smb2_create(c, "replace-same-inode", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &base) == ST_SUCCESS);
    ...
    assert(smb2_create(c, "replace-same-inode", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
    assert(smb2_read(c, probe.file_id, 0, 4, data, &bytes) == ST_SUCCESS &&
        bytes == 4 && !memcmp(data, "same", 4));
    assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
    assert(smb2_create(c, "replace-same-alias", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
    assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
    assert(smb2_close(c, base.file_id) == ST_SUCCESS);
...
same_inode_doc_cases(struct smb2_conn *c)
{
    ...
    for (unsigned int legacy = 0; legacy < 2; legacy++) {
        ...
        assert(smb2_create(c, source, FILE_CREATE, FILE_ALL_ACCESS, FILE_SHARE_RWD,
            NULL, &open) == ST_SUCCESS);
        hardlink(c, open.file_id, alias);
        uint64_t caps = module->capabilities;
        if (legacy) module->capabilities &= ~CHIMERA_VFS_CAP_RENAME_NOREPLACE;
        struct packet p = { 0 };
        replace_op(&p, c, open.file_id, alias);
        query(&p, c, open.file_id, 1, 0x30);
        /* Legacy RENAME submits directly; only the QUERY suffix contributes
         * a VFS command group in this deliberately forced boundary case. */
        if (legacy) send_runs_groups(&p, c, 1, 1); else doc_send(&p, c);
        ...
        assert(smb2_create(c, source, FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD,
            NULL, &probe) == ST_OBJECT_NAME_NOT_FOUND);
        assert(smb2_create(c, alias, FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD,
...
cancel_after_initial_move_case(struct smb2_conn *c)
{
    ...
    assert(smb2_create(c, "cancel-move-old", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &source) == ST_SUCCESS);
    assert(smb2_write(c, source.file_id, 0, "kept", 4, &count) == ST_SUCCESS && count == 4);
    hardlink(c, source.file_id, "cancel-move-alias");
    assert(smb2_create(c, "cancel-move-alias", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &alias) == ST_SUCCESS);
    struct packet p = { 0 };
    ...
    assert(smb2_create(c, "cancel-move-old", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &probe) == ST_OBJECT_NAME_NOT_FOUND);
    assert(smb2_create(c, "cancel-move-new", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
...
static void
replacement_delayed_open_cases(struct smb2_env *env, struct smb2_conn *c)
{
    ...
    for (unsigned int legacy = 0; legacy < 2; legacy++) {
        ...
        assert(smb2_create(c, source_name, FILE_CREATE, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, NULL, &source) == ST_SUCCESS);
        assert(smb2_create(c, target_name, FILE_CREATE, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, NULL, &target) == ST_SUCCESS);
        assert(smb2_close(c, target.file_id) == ST_SUCCESS);
        ...
        create_op(&create, d, target_name, FILE_OPEN, FILE_ALL_ACCESS);
        if (legacy) {
...
static void
replacement_constructor_races(struct smb2_env *env, struct smb2_conn *c)
{
    struct smb2_conn *d = smb2_conn_open(env);
    smb2_handshake(d);
    for (unsigned int legacy = 0; legacy < 2; legacy++) {
        ...
        create_op(&late, d, target_name, FILE_OPEN, FILE_ALL_ACCESS);
        ...
        if (legacy) p32(late.data + SMB2_HDR_SIZE, 40, 0x1000);
...
static void
replace_destination_races(struct smb2_env *env, struct smb2_conn *c)
{
    struct smb2_conn *d = smb2_conn_open(env);
    ...
    for (unsigned int disappears = 0; disappears < 2; disappears++) {
        ...
        struct packet p = { 0 };
        assert(smb2_create(c, old, FILE_CREATE, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, NULL, &source) == ST_SUCCESS);
        assert(smb2_write(c, source.file_id, 0, "source", 6, &bytes) == ST_SUCCESS && bytes == 6);
        if (disappears) {
            assert(smb2_create(d, dest, FILE_CREATE, FILE_ALL_ACCESS,
                FILE_SHARE_RWD, NULL, &target) == ST_SUCCESS);
        }
        query(&p, c, source.file_id, 1, 0x30);
        ...
        assert(smb2_create(d, dest, FILE_OPEN, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
    ...
    assert(smb2_create(c, "replace-stale-source", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &source) == ST_SUCCESS);
    assert(smb2_create(d, "replace-stale-other", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &target) == ST_SUCCESS);
    ...
    assert(smb2_create(d, "replace-stale-unwanted", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &probe) == ST_OBJECT_NAME_NOT_FOUND);
...
stream_holder_rename_cases(struct smb2_conn *c)
{
    ...
    uint32_t bytes;
    uint8_t data[8];
    assert(smb2_create(c, "stream-rename-old", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &base) == ST_SUCCESS);
    hardlink(c, base.file_id, "stream-rename-alias");
    assert(smb2_create(c, "stream-rename-old:fork:$DATA", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &stream) == ST_SUCCESS);
    assert(smb2_create(c, "stream-rename-old:sibling:$DATA", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &sibling) == ST_SUCCESS);
    assert(smb2_create(c, "stream-rename-alias:fork:$DATA", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &alias) == ST_SUCCESS);
    ...
    assert(smb2_create(c, "stream-rename-old:fork:$DATA", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &probe) == ST_OBJECT_NAME_NOT_FOUND);
    assert(smb2_create(c, "stream-rename-final:fork:$DATA", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    ...
    assert(smb2_create(c, "stream-doc-base", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &base) == ST_SUCCESS);
    assert(smb2_create_opts(c, "stream-doc-base:fork:$DATA", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, FILE_DELETE_ON_CLOSE, NULL, &stream) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    ...
    assert(smb2_close(c, stream.file_id) == ST_SUCCESS);
    assert(smb2_create(c, "rename-dir\\stream-doc-moved:fork:$DATA", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &probe) == ST_OBJECT_NAME_NOT_FOUND);
    assert(smb2_create(c, "rename-dir\\stream-doc-moved", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
    assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
    assert(smb2_close(c, base.file_id) == ST_SUCCESS);

    /* A readonly, same-name retry is safe even with live stream peers. */
    assert(smb2_create(c, "stream-noop", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &base) == ST_SUCCESS);
    assert(smb2_create(c, "stream-noop:fork:$DATA", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &stream) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
...
closed_stream_doc_rename_cases(struct smb2_conn *c)
{
    for (int legacy = 0; legacy < 2; legacy++) {
        ...
        const char *moved = legacy ? "rename-dir\\pending-doc-legacy-moved" : "rename-dir\\pending-doc-native-moved";
        ...
        assert(smb2_create_opts(c, stream_name, FILE_CREATE, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, FILE_DELETE_ON_CLOSE, NULL, &doc) == ST_SUCCESS);
        assert(smb2_create(c, alias_stream, FILE_OPEN, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, NULL, &survivor) == ST_SUCCESS);
        ...
        if (legacy) {
            ...
            assert(smb2_create(c, moved, FILE_CREATE, FILE_ALL_ACCESS,
                FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
            assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
        }
        ...
        assert(smb2_create(c, moved_stream, FILE_OPEN, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, NULL, &probe) == ST_OBJECT_NAME_NOT_FOUND);
        assert(smb2_create(c, alias_stream, FILE_OPEN, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, NULL, &probe) == ST_OBJECT_NAME_NOT_FOUND);
        assert(smb2_create(c, moved, FILE_OPEN, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
        assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
        assert(smb2_close(c, base.file_id) == ST_SUCCESS);
    }
...
static void
pending_stream_rename_case(struct smb2_env *env, struct smb2_conn *c)
{
    struct smb2_conn *d = smb2_conn_open(env);
    struct smb2_create_out base, stream;
    struct packet p = { 0 };
    smb2_handshake(d);
    assert(smb2_create(c, "pending-stream-old", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &base) == ST_SUCCESS);
    assert(smb2_create(c, "pending-stream-old:fork:$DATA", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &stream) == ST_SUCCESS);
    assert(smb2_close(c, stream.file_id) == ST_SUCCESS);
    create_op(&p, d, "pending-stream-old:fork:$DATA", FILE_OPEN, FILE_ALL_ACCESS);
...
static void
directory_native_cases(struct smb2_env *env, struct smb2_conn *c)
{
    struct smb2_create_out dir, peer, nested, child, probe, sibling;
    struct packet p = { 0 };
    uint32_t bytes;
    uint8_t data[16];
    assert(smb2_create(c, "native-directory-unrelated", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &sibling) == ST_SUCCESS);
    assert(smb2_create_opts(c, "native-directory-old", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, FILE_DIRECTORY_FILE, NULL, &dir) == ST_SUCCESS);
    assert(smb2_create_opts(c, "native-directory-old\\nested", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, FILE_DIRECTORY_FILE, NULL, &nested) == ST_SUCCESS);
    assert(smb2_create(c, "native-directory-old\\nested\\contents", FILE_CREATE, FILE_ALL_ACCESS,
    ...
    assert(smb2_create_opts(c, "native-directory-old", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, FILE_DIRECTORY_FILE, NULL, &peer) == ST_SUCCESS);
    ...
    assert(smb2_create_opts(c, "native-directory-final", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, FILE_DIRECTORY_FILE, NULL, &dir) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    create_op(&p, c, "native-directory-private-sibling", FILE_CREATE, FILE_ALL_ACCESS);
    ...
    memset(inherited, 0xff, sizeof(inherited));
    memset(&p, 0, sizeof(p));
    create_op(&p, c, "native-directory-private-moved", FILE_OPEN, FILE_ALL_ACCESS);
    p32(p.data + p.previous + SMB2_HDR_SIZE, 40, FILE_DIRECTORY_FILE);
    ...
    assert(smb2_create_opts(c, "native-directory-producer", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, FILE_DIRECTORY_FILE, NULL, &dir) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    ...
    assert(smb2_create_opts(c, "native-directory-gated", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, FILE_DIRECTORY_FILE, NULL, &dir) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    create_op(&p, c, "native-directory-gated\\private-child", FILE_CREATE, FILE_ALL_ACCESS);
...
static void
directory_mutator_races(struct smb2_env *env, struct smb2_conn *c)
{
    for (unsigned int mode = 0; mode < 5; mode++) {
        ...
        assert(smb2_create_opts(c, old, FILE_CREATE, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, FILE_DIRECTORY_FILE, NULL, &dir) == ST_SUCCESS);
        assert(smb2_create_opts(d, source_name, FILE_CREATE, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, mode == 2 ? FILE_DIRECTORY_FILE : 0, NULL, &source) == ST_SUCCESS);
        struct packet p = { 0 }, late = { 0 };
        ...
        if (mode == 1 || mode == 4) {
            ...
            p32(value, 16, 2 * len);
            for (unsigned int i = 0; i < len; i++) p16(value, 20 + 2 * i, target[i]);
        ...
        if (mode == 4) {
            ...
            create_op(&late, d, "mutation-abandoned-suffix", FILE_CREATE, FILE_ALL_ACCESS);
        ...
        atomic_store(&armed, 0);
        if (mode == 4) {
            assert(smb2_create(c, "mutation-abandoned-suffix", FILE_OPEN, FILE_ALL_ACCESS,
                FILE_SHARE_RWD, NULL, &probe) == ST_OBJECT_NAME_NOT_FOUND);
        }
        /* The blocked/canceled mutator did not move its existing source. */
        assert(smb2_create_opts(c, source_name, FILE_OPEN, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, mode == 2 ? FILE_DIRECTORY_FILE : 0, NULL, &probe) == ST_SUCCESS);
        assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
        if (mode != 4) assert(smb2_close(d, source.file_id) == ST_SUCCESS);
        assert(smb2_close(c, dir.file_id) == ST_SUCCESS);
    }
}
...
directory_failed_move_case(struct smb2_conn *c)
{
    ...
    assert(smb2_create_opts(c, "private-move-dir", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, FILE_DIRECTORY_FILE, NULL, &dir) == ST_SUCCESS);
    assert(smb2_create(c, "private-move-source", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &source) == ST_SUCCESS);
    struct packet p = { 0 };
...
static void
directory_scan_cases(struct smb2_env *env, struct smb2_conn *c)
{
    struct smb2_conn *d = smb2_conn_open(env);
    smb2_handshake(d);
    for (int mode = 1; mode <= 5; mode++) {
        ...
        assert(smb2_create(d, child_name, FILE_CREATE, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, NULL, &child) == ST_SUCCESS);
        if (mode == 5) {
            snprintf(child_name, sizeof(child_name), "%s\\b", old);
            assert(smb2_create(d, child_name, FILE_CREATE, FILE_ALL_ACCESS,
                FILE_SHARE_RWD, NULL, &other) == ST_SUCCESS);
        }
        scan_first_child = 0;
        ...
        assert(smb2_create_opts(d, mode == 1 ? old : moved, FILE_OPEN, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, FILE_DIRECTORY_FILE, NULL, &probe) == ST_OBJECT_NAME_NOT_FOUND);
        assert(smb2_create_opts(d, mode == 1 ? moved : old, FILE_OPEN, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, FILE_DIRECTORY_FILE, NULL, &probe) == ST_SUCCESS);
        assert(smb2_close(d, probe.file_id) == ST_SUCCESS);
...
main(void)
{
    ...
    create_op(&p, c, "cold-producer-old", FILE_CREATE, FILE_ALL_ACCESS);
    ...
    memset(&p, 0, sizeof(p));
    assert(smb2_create(c, "rename-old", FILE_OPEN_IF, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &a) == ST_SUCCESS);
    assert(smb2_create(c, "rename-old", FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &b) == ST_SUCCESS);
    hardlink(c, a.file_id, "rename-alias");
    assert(smb2_create(c, "rename-alias", FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &alias) == ST_SUCCESS);
    ...
    assert(smb2_create(c, "rename-old", FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &probe) ==
           ST_OBJECT_NAME_NOT_FOUND);
    assert(smb2_create(c, "rename-next", FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &probe) ==
           ST_OBJECT_NAME_NOT_FOUND);
    ...
    assert(smb2_create(c, "rename-alias", FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
    assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
    assert(smb2_create(c, "rename-final", FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
    assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
    ...
    * the source parent identity; a basename alone would target the root. */
    ...
    assert(smb2_create_opts(c, "rename-dir", FILE_OPEN_IF, FILE_ALL_ACCESS,
                            FILE_SHARE_RWD, FILE_DIRECTORY_FILE, NULL, &directory) == ST_SUCCESS);
    assert(smb2_close(c, directory.file_id) == ST_SUCCESS);
    assert(smb2_create(c, "rename-dir\\old", FILE_OPEN_IF, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &a) == ST_SUCCESS);
    assert(smb2_create(c, "rename-dir\\old", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &b) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    query(&p, c, a.file_id, 1, 0x30);
    ...
    assert(smb2_create(c, "rename-dir\\last", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
    assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
    ...
    assert(smb2_create_opts(c, "rename-other", FILE_OPEN_IF, FILE_ALL_ACCESS,
                            FILE_SHARE_RWD, FILE_DIRECTORY_FILE, NULL, &directory) == ST_SUCCESS);
    assert(smb2_close(c, directory.file_id) == ST_SUCCESS);
    assert(smb2_create(c, "rename-dir\\last", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &a) == ST_SUCCESS);
    ...
    assert(smb2_create(c, "rename-move-alias", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &alias) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    ...
    assert(smb2_create(c, "rename-returned", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
    assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
    ...
    assert(smb2_create_opts(c, "rename-doc-old", FILE_OPEN_IF, FILE_ALL_ACCESS,
                            FILE_SHARE_RWD, FILE_DELETE_ON_CLOSE, NULL, &a) == ST_SUCCESS);
    assert(smb2_create(c, "rename-doc-old", FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &b) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    rename_op(&p, c, b.file_id, "rename-dir\\rename-doc-new");
    query(&p, c, a.file_id, 1, 0x30);
    doc_send(&p, c);
    name_result(c, 1, "rename-dir\\rename-doc-new");
    assert(smb2_create(c, "rename-doc-old", FILE_OPEN_IF, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
    assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    close_op(&p, c, a.file_id);
    close_op(&p, c, b.file_id);
    doc_send(&p, c);
    assert(smb2_create(c, "rename-dir\\rename-doc-new", FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &probe) ==
           ST_OBJECT_NAME_NOT_FOUND);
    assert(smb2_create(c, "rename-doc-old", FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
    assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
    ...
    for (int retry = 0; retry < 2; retry++) {
        ...
        assert(smb2_create(c, old, FILE_OPEN_IF, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &a) == ST_SUCCESS);
        memset(&p, 0, sizeof(p));
        stat_open(&p, d, old);
        producer_gate.calls  = 0;
        ...
        assert(smb2_create(c, old, FILE_OPEN_IF, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
        atomic_store(&release_producer, 1);
