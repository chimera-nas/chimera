...
void *smb_access_retire_test_handle(void *private_data)
{
    struct chimera_smb_open_file *open = private_data;
    return open->handle;
}

int smb_access_retire_test_closed(void *private_data)
{
    struct chimera_smb_open_file *open = private_data;
    return !!(open->flags & CHIMERA_SMB_OPEN_FILE_CLOSED);
}

int smb_access_retire_test_timer(void *private_data, struct evpl_timer *timer)
{
    struct chimera_smb_open_file *open = private_data;
