    ...
    unsigned              count)
{
    ...
    for (unsigned i = 0; i < count; i++) {
        ...
        if (cmd->opcode == SMB2_CREATE) {
            ...
            p32(b, 32, MBT_FILE_SHARE_RWD); p32(b, 36, MBT_FILE_CREATE); p32(b, 40, MBT_FILE_NON_DIRECTORY_FILE);
            p16(b, 44, 120); p16(b, 46, length * 2);
            for (unsigned n = 0; n < length; n++) {
                p16(b, 56 + n * 2, (uint8_t) cmd->create_name[n]);
            }
            body = 56 + length * 2;
    ...
    int               before)
{
    ...
    while (c->ninterim == before) {
        smb2_pump(c->env); assert(smb2c_now_ms() < deadline);
    }
} /* wait_interim */
static void
blocking(
    struct smb2_conn *holder,
    struct smb2_conn *waiter,
    const uint8_t    *held,
    ...
    unsigned          mode)
{
    ...
    while (waiter->nreply_app < expected) {
        smb2_pump(waiter->env); assert(smb2c_now_ms() < deadline);
    }
    assert(atomic_load(&attempts) == 1);
    if (mode < 2) {
        assert(g32(waiter->rbuf + 4, 8) == (mode == 0 ? ST_SUCCESS : ST_CANCELLED));
    } else {
        assert(atomic_load(&last_execution) == CHIMERA_VFS_EINTR);
    }
    if (mode == 0) {
        assert(smb2_lock(waiter, wanted, 0, 4, SMB2_LOCKFLAG_UNLOCK) == ST_SUCCESS);
    } else {
        assert(smb2_lock(holder, held, 0, 4, SMB2_LOCKFLAG_UNLOCK) == ST_SUCCESS);
    }
} /* blocking */
    ...
    struct smb2_conn *peer)
{
    ...
    assert(smb2_create(peer, "private-lock-survives.txt", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, NULL,
                       &other) ==
           ST_SUCCESS);
    assert(smb2_lock(peer, other.file_id, 0, 4, SMB2_LOCKFLAG_EXCLUSIVE | SMB2_LOCKFLAG_FAIL_IMMEDIATELY) ==
           ST_LOCK_NOT_GRANTED);
...
checked_ranges(
    struct smb2_conn          *c,
    const uint8_t             *fid,
    const struct lock_element *ranges,
    unsigned                   count,
    uint32_t                   sequence,
    uint32_t                   expected,
    unsigned                   remaining,
    ...
    struct smb2_conn *peer)
{
    ...
    assert(smb2_create(c, "exact-batches", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, NULL, &a) ==
           ST_SUCCESS);
    ...
    struct smb2_conn *peer)
{
    ...
    assert(smb2_create(c, "canonical-same-lease", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, &lease, &owner) == ST_SUCCESS && owner.lease_state == SMB2_LEASE_RWH);
    assert(smb2_create(c, "canonical-same-lease", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, &lease, &same) == ST_SUCCESS && same.lease_state == SMB2_LEASE_RWH);
    ...
    assert(smb2_create(c, "canonical-distinct", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, NULL, &owner) == ST_SUCCESS);
    assert(smb2_create(peer, "canonical-distinct", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, NULL, &same) == ST_SUCCESS);
...
main(void)
{
    ...
    assert(smb2_create(c, "locks.txt", MBT_FILE_OPEN_IF, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, NULL, &a) ==
           ST_SUCCESS);
    assert(smb2_create(c, "locks.txt", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, NULL, &b) == ST_SUCCESS);
