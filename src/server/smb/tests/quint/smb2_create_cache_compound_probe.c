...
extern void smb_create_cache_inspect(
    void *,
    unsigned int,
    unsigned int,
    unsigned int);
extern void smb_persistent_create_expect(
    bool,
    int);
extern unsigned int smb_persistent_create_checks(
    void);
extern unsigned int smb_lease_key_pending_count(
    void);
extern void smb_lease_unlink_after_lookup(
    const char *);
extern unsigned int smb_lease_unlink_count(
    void);
    ...
    void                        *private_data)
{
    ...
    for (uint32_t i = 0; i < chimera_vfs_compound_num_ops(compound); i++) {
        ...
        if (op->type == CHIMERA_VFS_COMPOUND_OP_RESERVE_ACCESS) {
            smb_create_cache_inspect(op->callback_private, expected_lease_members, expected_lease_epoch,
                                     expected_lease_state); found++;
        }
    }
    ...
    bool              related)
{
    ...
    if (related) {
        p32(h, 16, SMB2_FLAGS_RELATED_OPERATIONS);
    }
    ...
    uint8_t           level)
{
    ...
    int                    size = smb2c_build_create(c, name, MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                                                     MBT_FILE_SHARE_RWD, MBT_FILE_NON_DIRECTORY_FILE, &oplock);
    ...
    for (unsigned int i = 0; i < p.count; i++) {
        ...
        if (i == 1) {
            assert(h[SMB2_HDR_SIZE + 2] == level);
        }
...
suffix_create(
    ...
    const char                   *name,
    uint32_t                      access,
    const struct smb2_oplock_req *cache)
{
    int      size = smb2c_build_create(c, name, MBT_FILE_OPEN, access, MBT_FILE_SHARE_RWD,
                                       MBT_FILE_NON_DIRECTORY_FILE, cache);
    uint8_t *b = append(p, c, SMB2_CREATE, size, false);
    ...
    const uint8_t    *other_fid)
{
    ...
    for (unsigned int i = 0; i < sizeof(levels); i++) {
        ...
        uint32_t               bytes;
        assert(smb2_create(c, name, MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD,
    ...
    memset(&p, 0, sizeof(p));
    suffix_create(&p, c, "lease-private-suffix", MBT_FILE_READ_ATTRIBUTES, &lease);
    ...
    deadline = smb2c_now_ms() + 10000;
    while (!smb2_conn_nbreaks(c) && smb2c_now_ms() < deadline) {
        smb2_pump(c->env);
    }
...
overwrite_client_cap(struct smb2_conn *c)
{
    ...
    for (unsigned int i = 0; i < 3; i++) {
        ...
        captured_create_len = 0; c->capture_create_reply_fn = capture_create;
        arm(1, 0);
        smb2_create_post(c, name, dispositions[i], MBT_FILE_READ_ATTRIBUTES, MBT_FILE_SHARE_RWD, &req);
        uint64_t          deadline = smb2c_now_ms() + 10000;
        while (!smb2_conn_nbreaks(c) && smb2c_now_ms() < deadline) {
            smb2_pump(c->env);
        }
        ...
        while (!captured_create_len && smb2c_now_ms() < deadline) {
            smb2_pump(c->env);
        }
...
} /* overwrite_client_cap */

static unsigned int
lease_context_length(const struct smb2_create_out *out)
{
    const struct smb2_rsp_ctx *context = smb2c_create_ctx_find(out, "RqLs");

    assert(context);
    return context->wire_len;
} /* lease_context_length */
static void
regular_lease_chain(
    struct smb2_conn *c,
    const uint8_t    *prefix,
    unsigned int      mode)
{
    ...
    query(&p, c, prefix);
    int                    size = smb2c_build_create(c, name, MBT_FILE_CREATE, MBT_FILE_READ_ACCESS,
                                                     MBT_FILE_SHARE_RWD, MBT_FILE_NON_DIRECTORY_FILE, &lease);
    uint8_t               *b = append(&p, c, SMB2_CREATE, size, false);
    memcpy(b, c->sbuf + 4 + SMB2_HDR_SIZE, size);
    query(&p, c, NULL);
    b = append(&p, c, SMB2_CLOSE, 24, true); p16(b, 0, 24); memset(b + 8, 0xff, 16);
    ...
    for (unsigned int i = 0; i < p.count; i++) {
        assert(g64(h, 24) == p.mid + i && g32(h, 8) == ST_SUCCESS);
        if (i == 1) {
            assert(h[SMB2_HDR_SIZE + 2] == SMB2_OPLOCK_LEVEL_LEASE);
        }
        uint32_t next = g32(h, 20); assert(!!next == (i + 1 < p.count));
        if (next) {
            h += next;
...
read_lease_cases(
    ...
    const uint8_t    *prefix)
{
    for (unsigned int mode = 1; mode < 8; mode += 2) {
        regular_lease_chain(c, prefix, mode);
    }
    for (unsigned int v1 = 0; v1 < 2; v1++) {
        ...
        arm(1, 0);
        assert(smb2_create(c, name, MBT_FILE_CREATE, MBT_FILE_READ_ACCESS,
                           MBT_FILE_SHARE_RWD, &lease, &holder) == ST_SUCCESS); disarm(1);
        ...
        assert(smb2_create(c, name, MBT_FILE_OPEN, MBT_FILE_READ_ACCESS,
                           MBT_FILE_SHARE_RWD, &lease, &joined) == ST_SUCCESS); disarm(1);
        assert(joined.lease_state == holder.lease_state && joined.lease_epoch == holder.lease_epoch);
        assert(lease_context_length(&joined) == lease_context_length(&holder));
        lease.lease_state = 0; arm(1, 0);
        assert(smb2_create(c, name, MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                           MBT_FILE_SHARE_RWD, &lease, &none) == ST_SUCCESS); disarm(1);
        ...
        arm(1, 0);
        assert(smb2_create(c, "cache-create-prefix", MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                           MBT_FILE_SHARE_RWD, &lease, &rejected) == ST_INVALID_PARAMETER); disarm(1);
        ...
        for (unsigned int create_if = 0; create_if < 2; create_if++) {
            ...
            arm(1, 0);
            assert(smb2_create(c, other, create_if ? MBT_FILE_OPEN_IF : MBT_FILE_CREATE, MBT_FILE_READ_ACCESS,
                               MBT_FILE_SHARE_RWD, &lease, &rejected) == ST_INVALID_PARAMETER); disarm(1);
            assert(smb2_create(c, other, MBT_FILE_OPEN, MBT_FILE_READ_ACCESS,
    ...
    smb2_create_post(c, "native-read-join-strong", MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                     MBT_FILE_SHARE_RWD, &lease);
    uint64_t deadline = smb2c_now_ms() + 10000;
    while (!atomic_load(&held) && smb2c_now_ms() < deadline) {
        smb2_pump(c->env);
    }
    ...
    smb2_create_post(peer, "native-read-join-strong", MBT_FILE_OPEN, MBT_FILE_READ_ACCESS,
    ...
    arm(1, 0);
    assert(smb2_create(c, "native-read-join-strong", MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                       MBT_FILE_SHARE_RWD, &lease, &joined) == ST_SUCCESS); disarm(1);
    ...
    arm(1, 0);
    assert(smb2_create(c, "native-read-join-strong", MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                       MBT_FILE_SHARE_RWD, &lease, &high_join) == ST_SUCCESS); disarm(1);
    ...
    struct smb2_conn *peer)
{
    for (unsigned int v1 = 0; v1 < 2; v1++) {
        for (unsigned int mode = 0; mode < 8; mode++) {
            ...
            arm(1, 0);
            assert(smb2_create(c, name, MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                               MBT_FILE_SHARE_RWD, &lease, &first) == ST_SUCCESS); disarm(1);
            assert(first.has_lease && first.lease_state == granted);
            if (granted) {
                assert(first.lease_epoch == (v1 ? 0 : 61));
            }
            lease.lease_epoch = 900;
            arm(1, 0);
            assert(smb2_create(c, name, MBT_FILE_OPEN_IF, MBT_FILE_READ_ATTRIBUTES,
                               MBT_FILE_SHARE_RWD, &lease, &joined) == ST_SUCCESS); disarm(1);
            assert(joined.lease_state == granted);
            if (granted) {
                assert(joined.lease_epoch == first.lease_epoch);
            }
            assert(!smb2_conn_nbreaks(c));
            assert(smb2_close(c, joined.file_id) == ST_SUCCESS);
    ...
    struct smb2_create_out holder, joined, opened;
    arm(1, 0);
    assert(smb2_create(c, "native-upgrade-retry", MBT_FILE_CREATE, MBT_FILE_READ_ACCESS,
                       MBT_FILE_SHARE_RWD, &lease, &holder) == ST_SUCCESS); disarm(1);
    ...
    reject_first           = 1; atomic_store(&finishes, 0); atomic_store(&hold_next, 1); arm(1, 0);
    int      replies = c->nreply_app;
    smb2_create_post(c, "native-upgrade-retry", MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                     MBT_FILE_SHARE_RWD, &lease);
    uint64_t deadline = smb2c_now_ms() + 10000;
    while (!atomic_load(&held) && smb2c_now_ms() < deadline) {
        smb2_pump(c->env);
    ...
    assert(smb2_create(peer, "native-high-cap", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, &lease, &holder) == ST_SUCCESS);
    assert(holder.lease_state == lease.lease_state);
    struct smb2_oplock_req requester = lease; requester.lease_key[2] = 2;
    arm(1, 0);
    assert(smb2_create(c, "native-high-cap", MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                       MBT_FILE_SHARE_RWD, &requester, &opened) == ST_SUCCESS); disarm(1);
    ...
    * peer's keyed READ grant allows RH, never a fresh conflicting W. */
    arm(1, 0);
    smb2_create_post(c, "native-high-cap", MBT_FILE_OPEN, MBT_FILE_READ_ACCESS,
                     MBT_FILE_SHARE_RWD, &requester);
    deadline = smb2c_now_ms() + 10000;
    while (!smb2_conn_nbreaks(peer) && smb2c_now_ms() < deadline) {
        smb2_pump(c->env);
    }
    struct smb2_break brk;
...
hintless_and_overwrite_cases(struct smb2_conn *c)
{
    ...
    for (unsigned int hinted = 0; hinted < 2; hinted++) {
        for (unsigned int existing = 0; existing < 2; existing++) {
            for (unsigned int i = 0; i < sizeof(dispositions) / sizeof(dispositions[0]); i++) {
                char                   name[64]; snprintf(name, sizeof(name), "lease-disposition-%u-%u-%u", hinted,
                                                          existing, i);
                ...
                :
                    !existing && (dispositions[i] == MBT_FILE_OPEN || dispositions[i] == MBT_FILE_OVERWRITE) ?
                    ST_OBJECT_NAME_NOT_FOUND : ST_SUCCESS;
                arm(1, 0);
                assert(smb2_create_opts(c, name, dispositions[i], MBT_FILE_ALL_ACCESS,
                                        MBT_FILE_SHARE_RWD, hinted ? MBT_FILE_NON_DIRECTORY_FILE : 0, &lease, &opened)
                       ==
                       expected);
                disarm(1);
                if (expected != ST_SUCCESS) {
                    continue;
                }
                ...
                assert(opened.action == (!existing ? 2 : dispositions[i] == MBT_FILE_SUPERSEDE ? 0 : truncated ? 3 : 1))
                ;
                uint8_t data[5];
                assert(smb2_read(c, opened.file_id, 0, sizeof(data), data, &bytes) ==
                       (existing && !truncated ? ST_SUCCESS : ST_END_OF_FILE));
                if (existing && !truncated) {
                    assert(bytes == 5 && !memcmp(data, "keep!", 5));
                }
                assert(smb2_close(c, opened.file_id) == ST_SUCCESS);
            }
        }
    ...
    assert(smb2_create_opts(c, "lease-own-overwrite", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                            MBT_FILE_SHARE_RWD, 0, &lease, &holder) == ST_SUCCESS);
    ...
    int                    replies = c->nreply_app;
    smb2_create_post_opts(c, "lease-own-overwrite", MBT_FILE_OVERWRITE, MBT_FILE_ALL_ACCESS,
                          MBT_FILE_SHARE_RWD, 0, &lease);
    uint64_t               deadline = smb2c_now_ms() + 10000;
    while (!atomic_load(&held) && smb2c_now_ms() < deadline) {
        smb2_pump(c->env);
    }
    ...
    * rejects creating variants before any new namespace entry can appear. */
    ...
    assert(smb2_create(c, "lease-key-victim", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, NULL, &victim) == ST_SUCCESS);
    ...
    for (unsigned int i = 3; i < sizeof(dispositions) / sizeof(dispositions[0]); i++) {
        arm(1, 0);
        assert(smb2_create_opts(c, "lease-key-victim", dispositions[i], MBT_FILE_ALL_ACCESS,
                                MBT_FILE_SHARE_RWD, 0, &lease, &opened) == ST_INVALID_PARAMETER); disarm(1);
        uint8_t data[5];
        assert(smb2_read(c, victim.file_id, 0, sizeof(data), data, &bytes) == ST_SUCCESS &&
               bytes == 5 && !memcmp(data, "keep!", 5));
        arm(1, 0);
        assert(smb2_create_opts(c, "lease-key-missing", dispositions[i], MBT_FILE_ALL_ACCESS,
                                MBT_FILE_SHARE_RWD, 0, &lease, &opened) ==
               (dispositions[i] == MBT_FILE_OVERWRITE ? ST_OBJECT_NAME_NOT_FOUND : ST_INVALID_PARAMETER)); disarm(1);
        assert(smb2_create(c, "lease-key-missing", MBT_FILE_OPEN, MBT_FILE_READ_ACCESS,
                           MBT_FILE_SHARE_RWD, NULL, &opened) == ST_OBJECT_NAME_NOT_FOUND);
    }
    arm(1, 0);
    assert(smb2_create_opts(c, "lease-key-missing", MBT_FILE_OPEN, MBT_FILE_READ_ACCESS,
                            MBT_FILE_SHARE_RWD, 0, &lease, &opened) == ST_OBJECT_NAME_NOT_FOUND); disarm(1);
    ...
    assert(smb2_create_opts(c, "lease-hintless-directory", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                            MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, NULL, &directory) == ST_SUCCESS);
    assert(smb2_create(c, "lease-hintless-directory\\child", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
    ...
    for (unsigned int i = 0; i < 2; i++) {
        arm(1, 0);
        assert(smb2_create_opts(c, "lease-hintless-directory", i ? MBT_FILE_OPEN_IF : MBT_FILE_OPEN,
                                MBT_FILE_READ_ATTRIBUTES, MBT_FILE_SHARE_RWD, 0, &lease, &directory) == ST_SUCCESS);
        disarm(1);
        assert(directory.has_lease && directory.lease_state == (SMB2_LEASE_READ | SMB2_LEASE_HANDLE));
        assert(smb2_close(c, directory.file_id) == ST_SUCCESS);
    }
    assert(smb2_create(c, "lease-hintless-directory\\child", MBT_FILE_OPEN, MBT_FILE_READ_ACCESS,
                       MBT_FILE_SHARE_RWD, NULL, &child) == ST_SUCCESS);
    uint8_t data[5];
    assert(smb2_read(c, child.file_id, 0, sizeof(data), data, &bytes) == ST_SUCCESS &&
           bytes == 5 && !memcmp(data, "child", 5));
    assert(smb2_close(c, child.file_id) == ST_SUCCESS);
} /* hintless_and_overwrite_cases */
    ...
    const uint8_t    *prefix)
{
    for (unsigned int mode = 0; mode < 8; mode++) {
        ...
        if (mode == 7) {
            ...
            int      replies = c->nreply_app;
            smb2_create_post_opts(c, name, MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                                  MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, &lease);
            uint64_t deadline = smb2c_now_ms() + 10000;
            while (!atomic_load(&held) && smb2c_now_ms() < deadline) {
                smb2_pump(c->env);
            }
            ...
            assert(smb2_create_opts(c, name, mode & 1 ? MBT_FILE_CREATE : MBT_FILE_OPEN_IF,
                                    MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, &lease, &first) ==
                   ST_SUCCESS)
            ;
        }
        disarm(1);
        ...
        lease.lease_epoch = 900;
        arm(1, 0);
        assert(smb2_create_opts(c, name, MBT_FILE_OPEN_IF, MBT_FILE_READ_ATTRIBUTES,
                                MBT_FILE_SHARE_RWD, 0, &lease, &joined) == ST_SUCCESS); disarm(1);
    ...
    lease.lease_state  = SMB2_LEASE_READ;
    assert(smb2_create_opts(c, "native-directory-retry", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                            MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, &lease, &holder) == ST_SUCCESS);
    lease.lease_state     |= SMB2_LEASE_HANDLE | SMB2_LEASE_WRITE;
    expected_lease_members = 1; expected_lease_epoch = holder.lease_epoch;
    expected_lease_state   = holder.lease_state;
    reject_first           = 1; atomic_store(&finishes, 0); atomic_store(&hold_next, 1); arm(1, 0);
    int      replies = c->nreply_app;
    smb2_create_post_opts(c, "native-directory-retry", MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                          MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, &lease);
    uint64_t deadline = smb2c_now_ms() + 10000;
    while (!atomic_load(&held) && smb2c_now_ms() < deadline) {
        smb2_pump(c->env);
    ...
    arm(1, 0);
    assert(smb2_create_opts(c, "native-directory-wrong-key", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                            MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, &lease, &rejected) == ST_INVALID_PARAMETER);
    disarm(1);
    assert(smb2_create_opts(c, "native-directory-wrong-key", MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                            MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, NULL, &rejected) == ST_OBJECT_NAME_NOT_FOUND);
    ...
    assert(smb2_create(peer, "native-directory-retry\\child", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, NULL, &child) == ST_SUCCESS);
    deadline = smb2c_now_ms() + 10000;
    while (!smb2_conn_nbreaks(c) && smb2c_now_ms() < deadline) {
        smb2_pump(c->env);
    ...
    arm(1, 0);
    assert(smb2_create_opts(c, "native-directory-retry", MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                            MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, &lease, &joined) == ST_SUCCESS); disarm(1);
    ...
    bool              persistent)
{
    ...
    assert(smb2_create_opts(c, name, MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD,
                            MBT_FILE_DIRECTORY_FILE, &lease, &directory) == ST_SUCCESS); disarm(2);
    assert(directory.oplock == SMB2_OPLOCK_LEVEL_NONE && !directory.lease_state);
    assert(smb2_close(c, directory.file_id) == ST_SUCCESS);
    arm(1, 0);
    assert(smb2_create_opts(c, name, MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES, MBT_FILE_SHARE_RWD,
                            0, &lease, &directory) == ST_SUCCESS); disarm(persistent ? 3 : 2);
    assert(directory.oplock == SMB2_OPLOCK_LEVEL_NONE && !directory.lease_state);
    assert(smb2_close(c, directory.file_id) == ST_SUCCESS);
} /* directory_lease_fallback */

static void
directory_parent_key_compound(
    struct smb2_conn *c,
    struct smb2_conn *peer)
{
    struct smb2_oplock_req lease = {
    ...
    assert(smb2_create_opts(c, "parent-key-native", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                            MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, &lease, &parent) == ST_SUCCESS);
    assert(smb2_create_opts(peer, "parent-key-native", MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                            MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, &other, &observer) == ST_SUCCESS);
    for (unsigned int directory = 0; directory < 2; directory++) {
        ...
        int                    size        = smb2c_build_create(c,
                                                                directory ? "parent-key-native\\directory" :
                                                                "parent-key-native\\file", MBT_FILE_CREATE,
                                                                MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD,
                                                                directory ? MBT_FILE_DIRECTORY_FILE :
                                                                MBT_FILE_NON_DIRECTORY_FILE, &child_lease);
    ...
    struct smb2_conn *peer)
{
    for (unsigned int doc = 0; doc < 2; doc++) {
        ...
        assert(smb2_create_opts(c, dirname, MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD,
                                MBT_FILE_DIRECTORY_FILE, NULL, &setup) == ST_SUCCESS);
        assert(smb2_close(c, setup.file_id) == ST_SUCCESS);
        if (!doc) {
            assert(smb2_create(c, target, MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD,
                               NULL, &setup) == ST_SUCCESS);
            assert(smb2_close(c, setup.file_id) == ST_SUCCESS);
        }
        struct smb2_oplock_req  lease = {
        ...
        uint8_t                *ctx    = header + g32(header + SMB2_HDR_SIZE, 48);
        for (;;) {
            if (g16(ctx, 6) == 4 && !memcmp(ctx + g16(ctx, 4), "RqLs", 4)) {
                break;
            }
        ...
        if (doc) {
            assert(smb2c_create_ctx_find(&child, "DH2Q"));
        }
        assert(smb2_create_opts(c, dirname, MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES, MBT_FILE_SHARE_RWD,
                                MBT_FILE_DIRECTORY_FILE, &lease, &parent) == ST_SUCCESS);
        assert(smb2_create_opts(peer, dirname, MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES, MBT_FILE_SHARE_RWD,
                                MBT_FILE_DIRECTORY_FILE, &lease, &observer) == ST_SUCCESS);
    ...
    struct smb2_conn *peer)
{
    ...
    assert(smb2_create(c, "global-key-first", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, NULL, &first) == ST_SUCCESS);
    assert(smb2_close(c, first.file_id) == ST_SUCCESS);
    atomic_store(&hold_next, 1); atomic_store(&finishes, 0); arm(1, 0);
    smb2_create_post(c, "global-key-first", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, &lease);
    uint64_t               deadline = smb2c_now_ms() + 10000;
    while (!atomic_load(&held) && smb2c_now_ms() < deadline) {
        smb2_pump(c->env);
    }
    ...
    smb2_create_post(sibling, "global-key-rejected", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, &lease);
    ...
    deadline = smb2c_now_ms() + 10000;
    while (smb_lease_key_pending_count() == pending && smb2c_now_ms() < deadline) {
        smb2_pump(c->env);
    }
    ...
    assert(smb2_create(sibling, "global-key-rejected", MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                       MBT_FILE_SHARE_RWD, NULL, &rejected) == ST_OBJECT_NAME_NOT_FOUND);
    assert(smb2_create(sibling, "global-key-first", MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                       MBT_FILE_SHARE_RWD, &lease, &second) == ST_SUCCESS && second.lease_epoch == first.lease_epoch);
    assert(smb2_close(sibling, second.file_id) == ST_SUCCESS);
    assert(smb2_create(peer, "global-key-other-client", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, &lease, &second) == ST_SUCCESS);
    ...
    smb2_create_post(peer, "global-key-first", MBT_FILE_OPEN, MBT_FILE_READ_ACCESS, MBT_FILE_SHARE_RWD, NULL);
    deadline = smb2c_now_ms() + 10000;
    while (!smb2_conn_nbreaks(c) && smb2c_now_ms() < deadline) {
        smb2_pump(c->env);
    }
    ...
    assert(smb2_create(sibling, "global-key-rejected", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, &lease, &second) == ST_SUCCESS);
    assert(smb2_close(sibling, second.file_id) == ST_SUCCESS);
} /* client_lease_key_cases */
...
static void
bound_lease_disappearing_name(struct smb2_conn *c)
{
    for (unsigned int legacy = 0; legacy < 2; legacy++) {
        for (unsigned int directory = 0; directory < 2; directory++) {
            ...
            assert(smb2_create_opts(c, name, MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                                    MBT_FILE_SHARE_RWD, options, &lease, &holder) == ST_SUCCESS && holder.has_lease);
            assert(smb2_create_dur_opts(c, name, MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD,
                                        options, &lease, legacy ? &dur : NULL, &opened) == ST_OBJECT_NAME_COLLISION);
            smb_lease_unlink_after_lookup(name);
            assert(smb2_create_dur_opts(c, name, MBT_FILE_OPEN_IF, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD,
                                        options, &lease, legacy ? &dur : NULL, &opened) == ST_INVALID_PARAMETER);
            assert(smb_lease_unlink_count() == 1);
            assert(smb2_create_opts(c, name, MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                                    MBT_FILE_SHARE_RWD, options, NULL, &opened) == ST_OBJECT_NAME_NOT_FOUND);
            assert(smb2_close(c, holder.file_id) == ST_SUCCESS);
        }
    }
} /* bound_lease_disappearing_name */
...
static void
cross_client_durable_io(struct smb2_env *env)
{
    ...
    assert(smb2_create_dur(reclaimed, "", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS,
                           MBT_FILE_SHARE_RWD, NULL, &reconnect, &reopened) == ST_SUCCESS);
...
directory_disabled_policy(void)
{
    ...
    smb2_handshake(c);
    directory_lease_fallback(c, "directory-leasing-disabled", false, false);
...
persistent_create_truth_cases(void)
{
        ...
        for (unsigned int cached = 0; cached < 2; cached++) {
            ...
            if (fixture != 0) {
                assert(smb2_create_opts(c, base, MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD,
                                        base_directory ? MBT_FILE_DIRECTORY_FILE : MBT_FILE_NON_DIRECTORY_FILE,
                                        NULL, &setup) == ST_SUCCESS);
                assert(smb2_close(c, setup.file_id) == ST_SUCCESS);
            }
            ...
            assert(smb2_create_dur_opts(c, name, fixture == 1 ? MBT_FILE_OPEN_IF : MBT_FILE_CREATE,
                                        MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_READ | MBT_FILE_SHARE_WRITE,
                                        stream ? MBT_FILE_NON_DIRECTORY_FILE : MBT_FILE_DIRECTORY_FILE,
            ...
            uint32_t                bytes;
            if (stream) {
                assert(smb2_write(c, created.file_id, 0, "fork", 4, &bytes) == ST_SUCCESS);
            }
            ...
            assert(smb2_create(reconnect, wrong_name, MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
            ...
            assert(smb2_create_dur(reconnect, "", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS,
                                   MBT_FILE_SHARE_RWD, &lease, &claim, &reopened) == ST_INVALID_PARAMETER);
            claim.reconnect_flags = 0;
            assert(smb2_create_dur(reconnect, "", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS,
                                   MBT_FILE_SHARE_RWD, &lease, &claim, &reopened) == ST_SUCCESS);
            ...
            assert(smb2_create_opts(reconnect, base, MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS, 0,
                                    fixture == 2 ? MBT_FILE_NON_DIRECTORY_FILE : MBT_FILE_DIRECTORY_FILE,
                                    NULL, &setup) == ST_SUCCESS);
    ...
    for (unsigned int directory = 0; directory < 2; directory++) {
        ...
        assert(smb2_create_dur_opts(c, name, directory ? MBT_FILE_OPEN : MBT_FILE_CREATE,
                                    MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, directory ? MBT_FILE_DIRECTORY_FILE :
    ...
    assert(smb2_create_dur(c, "persistent-put-failure", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                           MBT_FILE_SHARE_RWD, NULL, &dur, &created) == 0xc0000185u); /* STATUS_IO_DEVICE_ERROR */
    assert(smb2_create(c, "persistent-put-failure", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS, 0,
                       NULL, &setup) == ST_SUCCESS);
    assert(smb2_close(c, setup.file_id) == ST_SUCCESS);

    dur.create_guid[2] = 4;
    smb_persistent_create_expect(false, 2); expected_checks++;
    assert(smb2_create_dur(c, "persistent-ambiguous-put", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                           MBT_FILE_SHARE_RWD, NULL, &dur, &created) == 0xc0000185u); /* STATUS_IO_DEVICE_ERROR */
    assert(smb2_create(c, "persistent-ambiguous-put", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS, 0,
                       NULL, &setup) == ST_SUCCESS);
    assert(smb2_close(c, setup.file_id) == ST_SUCCESS);
    ...
    assert(smb2_create(c, "persistent-share-denied", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, 0,
                       NULL, &setup) == ST_SUCCESS);
    dur.create_guid[2] = 3;
    smb_persistent_create_expect(false, false); expected_checks++;
    assert(smb2_create_dur(c, "persistent-share-denied", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS,
                           MBT_FILE_SHARE_RWD, NULL, &dur, &created) == ST_SHARING_VIOLATION);
    assert(smb2_close(c, setup.file_id) == ST_SUCCESS);
...
main(void)
{
    ...
    int                    replies = c->nreply_app;
    reject_first = 1; atomic_store(&finishes, 0); atomic_store(&hold_next, 1); arm(1, 0);
    smb2_create_post(c, "cache-retry", MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES, MBT_FILE_SHARE_RWD, &req);
    uint64_t               deadline = smb2c_now_ms() + 10000;
    while (!atomic_load(&held) && smb2c_now_ms() < deadline) {
        smb2_pump(&env);
    }
    ...
    * a transparent stat-open. Opportunistic admission never sends a break. */
    ...
    assert(smb2_create(peer, "cache-cap", MBT_FILE_CREATE, MBT_FILE_READ_ACCESS,
                       MBT_FILE_SHARE_RWD, &req, &holder) == ST_SUCCESS && holder.oplock == req.level);
    req.level = SMB2_OPLOCK_LEVEL_BATCH; int breaks = smb2_conn_nbreaks(peer);
    arm(1, 0);
    assert(smb2_create(c, "cache-cap", MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                       MBT_FILE_SHARE_RWD, &req, &opened) == ST_SUCCESS); disarm(1);
    ...
    assert(smb2_create(peer, "cache-cap", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, &req, &holder) == ST_SUCCESS && holder.oplock == req.level);
    ...
    assert(smb2_create(c, "same-client-cache", MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                       MBT_FILE_SHARE_RWD, &req, &opened) == ST_SUCCESS); disarm(1);
    ...
    assert(smb2_create_opts(c, "directory-oplock", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                            MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, &req, &opened) == ST_SUCCESS); disarm(1);
    assert(opened.oplock == SMB2_OPLOCK_LEVEL_NONE);
    assert(smb2_close(c, opened.file_id) == ST_SUCCESS);
    /* A real durable-v1 grant still takes the explicit legacy boundary. */
    arm(0, 0);
    assert(smb2_create_dur(c, "actual-durable", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                           MBT_FILE_SHARE_RWD, &req, &dur, &opened) == ST_SUCCESS); disarm(2);
    ...
    directory_lease_fallback(c, "directory-lease-v1", true, true);
