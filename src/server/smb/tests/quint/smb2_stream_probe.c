...
typedef void (*stream_submit_fn)(struct chimera_vfs_compound *,
...
static void
stream_close_finish(struct chimera_vfs_compound *cp, void *private_data)
{
    (void) private_data;
...
static void
stream_test_poll(struct evpl *evpl, struct evpl_timer *timer)
{
...
static void
stream_test_finish(struct chimera_vfs_compound *cp, void *private_data)
{
...
static void
stream_test_done(struct chimera_vfs_compound *cp, void *private_data)
{
    struct stream_test_ctx *ctx = private_data;
...
static void
stream_test_rebound(struct chimera_vfs_compound *cp, void *private_data)
{
    struct stream_test_ctx *ctx = private_data;
    ...
    chimera_vfs_compound_callback_t callback, void *private_data)
{
    ...
    for (uint32_t i = 0; mode && i < chimera_vfs_compound_num_ops(cp); i++) {
        ...
        if (op->type == CHIMERA_VFS_COMPOUND_OP_REMOVE_STREAM &&
            (op->remove_flags & CHIMERA_VFS_REMOVE_STREAM_MATCH_FH)) remove = op;
    }
    if (!remove) {
    ...
    if (mode == 1) {
        ...
        chimera_vfs_compound_add_open_stream(rebind, remove->name, remove->name_len,
            CHIMERA_VFS_OPEN_INFERRED | CHIMERA_VFS_OPEN_CREATE | CHIMERA_VFS_OPEN_EXCLUSIVE, NULL, 0);
...
probe_stream_lifecycle(struct smb2_conn *c)
{
    ...
    st = smb2_create(c, "ads.bin:beta", FILE_OVERWRITE, FILE_ALL_ACCESS,
                     FILE_SHARE_RWD, NULL, &reopen);
    ...
    st = smb2_create(c, "ads.bin", FILE_OVERWRITE, FILE_ALL_ACCESS,
                     FILE_SHARE_RWD, NULL, &reopen);
    CHECK(st == ST_SUCCESS && reopen.end_of_file == 0,
...
probe_stream_last_close(struct smb2_conn *c)
{
    ...
    for (int mode = 0; mode < 5; mode++) {
        ...
        st = smb2_create(c, base_name, FILE_CREATE, FILE_ALL_ACCESS,
                         FILE_SHARE_RWD, NULL, &base);
        CHECK(st == ST_SUCCESS, "stream last-close base setup %d", mode);
        if (st != ST_SUCCESS) {
            continue;
        }
        st = smb2_create_opts(c, stream_name, FILE_CREATE, FILE_ALL_ACCESS,
                              FILE_SHARE_RWD, (mode == 1 || mode == 4) ? FILE_DELETE_ON_CLOSE : 0,
                              NULL, &owner);
        ...
        st = smb2_create(peer_conn, stream_name, FILE_OPEN, FILE_ALL_ACCESS,
        ...
        st = smb2_create(c, stream_name, FILE_OPEN, FILE_READ_ACCESS,
        ...
        st = smb2_create(c, stream_name, FILE_OVERWRITE, FILE_ALL_ACCESS,
                         FILE_SHARE_RWD, NULL, &lookup);
        CHECK(st == ST_DELETE_PENDING,
        ...
        smb2_close(c, base.file_id);
        st = smb2_create(c, base_name, FILE_OPEN, FILE_READ_ACCESS,
                         FILE_SHARE_RWD, NULL, &lookup);
...
probe_checked_stream_delete(struct smb2_conn *c)
{
    ...
    CHECK(smb2_create(c, "checked-stream", FILE_OPEN_IF, FILE_ALL_ACCESS,
                      FILE_SHARE_RWD, NULL, &base) == ST_SUCCESS, "create checked stream base");
    for (unsigned mode = 1; mode <= 2; mode++) {
        CHECK(smb2_create(c, "checked-stream:fork", FILE_CREATE, FILE_ALL_ACCESS,
                          FILE_SHARE_RWD, NULL, &stream) == ST_SUCCESS, "create checked stream mode %u", mode);
        ...
        if (mode == 1) {
            uint64_t deadline = smb2c_now_ms() + 5000;
            while (!atomic_load(&stream_test_held) && smb2c_now_ms() < deadline) smb2_pump(c->env);
        ...
        if (mode == 1) CHECK(status == ST_SUCCESS, "accepted checked stream CLOSE succeeds");
        atomic_store(&stream_test_mode, 0);
        status = smb2_create(c, "checked-stream:fork", FILE_OPEN, FILE_ALL_ACCESS,
...
probe_stream_compound_close(struct smb2_conn *c)
{
    ...
    uint32_t count = 0;
    assert(smb2_create(c, "compound-stream", FILE_CREATE, FILE_READ_ATTRIBUTES,
        FILE_SHARE_RWD, NULL, &base) == ST_SUCCESS);
    assert(smb2_create(c, "compound-stream:fork", FILE_CREATE,
        FILE_READ_ACCESS | FILE_WRITE_ACCESS,
        FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, &stream) == ST_SUCCESS);
    assert(smb2_write(c, stream.file_id, 0, "payload", 7, &count) == ST_SUCCESS && count == 7);
    assert(smb2_create(c, "compound-stream", FILE_OPEN, 0x00010000u /* DELETE */,
        FILE_SHARE_RWD, NULL, &denied) == ST_SHARING_VIOLATION);
    ...
    assert(smb2_create(c, "compound-stream:fork", FILE_OPEN, FILE_READ_ACCESS,
        FILE_SHARE_RWD, NULL, &reopened) == ST_SUCCESS);
    uint8_t bytes[8];
