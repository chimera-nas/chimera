...
extern void notify_test_inspect(
    void *,
    unsigned int);
extern void notify_test_track_free(
    ...
    if (command && atomic_exchange(&arm, 0)) {
        ...
        if (pause_before_retire) {
            for (unsigned int i = 0; i < chimera_vfs_compound_num_ops(compound); i++) {
                ...
                if (op->type != CHIMERA_VFS_COMPOUND_OP_COORDINATE) {
                    continue;
                }
            ...
            chimera_vfs_compound_set_finish_handler(compound, finish_hold, ctx);
        }
    ...
    const uint8_t    *fid)
{
    ...
    uint64_t deadline = smb2c_now_ms() + 10000;
    while (c->ninterim == count && smb2c_now_ms() < deadline) {
        smb2_pump(env);
    }
    ...
    uint32_t          status)
{
    ...
    while (smb2c_now_ms() < deadline) {
        if (smb2_conn_take_notify(c, id, &reply)) {
            assert(reply.status == status); return;
        }
        smb2_pump(env);
    }
    ...
    struct smb2_conn *primary)
{
    for (unsigned int mode = 0; mode < 4; mode++) {
        ...
        assert(smb2_create_opts(c, name, MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD,
                                MBT_FILE_DIRECTORY_FILE, NULL, &file) == ST_SUCCESS);
        ...
        if (mode != 3) {
            ...
            if (mode != 2) {
                notify_wait(env, c, id, ST_FILE_CLOSED);
            }
    ...
    struct smb2_conn *c)
{
    ...
    assert(smb2_create_opts(c, "notify-before-close-fence", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                            MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, NULL, &file) == ST_SUCCESS);
    ...
    struct smb2_conn *c)
{
    ...
    assert(smb2_create_opts(c, "notify-doc-native", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                            MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE | MBT_FILE_DELETE_ON_CLOSE, NULL, &file) ==
           ST_SUCCESS);
    uint64_t               id = notify_post(env, c, file.file_id);
    ...
    assert(smb2_close(c, file.file_id) == ST_SUCCESS);
    notify_wait(env, c, id, ST_NOTIFY_CLEANUP);
    assert(smb2_create_opts(c, "notify-doc-native", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS,
                            MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, NULL, &file) == ST_OBJECT_NAME_NOT_FOUND);
} /* watched_doc_case */
    ...
    struct smb2_conn *c)
{
    ...
    assert(smb2_create_opts(c, "notify-cross-worker-a", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                            MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, NULL, &first) == ST_SUCCESS);
    assert(smb2_create_opts(peer, "notify-cross-worker-b", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                            MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, NULL, &second) == ST_SUCCESS);
    ...
    uint64_t               deadline = smb2c_now_ms() + 10000;
    while (atomic_load(&notify_test_cross_freed) != 2 && smb2c_now_ms() < deadline) {
        smb2_pump(env);
    }
...
previous_session_cases(struct smb2_env *env)
{
    for (unsigned int waiting = 0; waiting < 2; waiting++) {
        ...
        assert(smb2_create_opts(c, name, MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD,
                                MBT_FILE_DIRECTORY_FILE, NULL, &base) == ST_SUCCESS);
        ...
        assert(smb2_create_dur_opts(c, name, MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD,
                                    MBT_FILE_DIRECTORY_FILE, &lease, &durable, &file) == ST_SUCCESS && file.has_dh2q);
        if (waiting) {
        ...
        if (waiting) {
            ...
            assert(smb2_create_dur_opts(replacement, "", MBT_FILE_OPEN, MBT_FILE_ALL_ACCESS,
                                        MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, &lease, &reclaim, &file) ==
                   ST_SUCCESS);
    ...
    struct smb2_conn *peer)
{
    ...
    assert(smb2_create_opts(c, "notify-sibling-tdis", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                            MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, NULL, &file) == ST_SUCCESS);
...
logoff_security_cases(const struct smb2_env_opts *opts)
{
    for (unsigned int encrypted = 0; encrypted < 2; encrypted++) {
        ...
        if (peer) {
            sibling_tdis_case(&env, c, peer);
        }
        struct smb2_create_out file;
        assert(smb2_create_opts(c, "notify-logoff-security", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                                MBT_FILE_SHARE_RWD, MBT_FILE_DIRECTORY_FILE, NULL, &file) == ST_SUCCESS);
        if (peer) {
            atomic_store(&notify_test_wire_capture, 1);
        }
        uint64_t               id      = notify_post(&env, c, file.file_id);
        ...
        if (encrypted) {
            atomic_store(&notify_test_retired_session, 1);
        }
        if (peer) {
            signed_logoff_compound(c, false, false);
        } else {
            assert(smb2_logoff(c) == ST_SUCCESS);
        }
        notify_wait(&env, c, id, ST_NOTIFY_CLEANUP);
        if (peer) {
