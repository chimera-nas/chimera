    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        switch (cmd[i].opcode) {
            case SMB2_CREATE: {
                ...
                p32(b, 24, cmd[i].offset ? FILE_READ_ATTRIBUTES : FILE_ALL_ACCESS);
                p32(b, 32, FILE_SHARE_RWD);
                p32(b, 36, cmd[i].offset ? FILE_OPEN : FILE_CREATE);
                p32(b, 40, FILE_NON_DIRECTORY_FILE);
    ...
    const uint8_t     file_id[16])
{
    ...
    assert(smb2_create(c, "compound.txt", FILE_OPEN, FILE_ALL_ACCESS,
...
main(int argc, char **argv)
{
    ...
    smb2_handshake(c);
    assert(smb2_create(c, "compound.txt", FILE_OPEN_IF, FILE_ALL_ACCESS,
...
/* *INDENT-ON* */
    ...
    struct command created[] = {
        { SMB2_CREATE, NULL, 0, 0, ST_SUCCESS },
        { SMB2_WRITE, related, SMB2_FLAGS_RELATED_OPERATIONS, 0, ST_SUCCESS },
        { SMB2_QUERY_INFO, related, SMB2_FLAGS_RELATED_OPERATIONS, 0, ST_SUCCESS },
    ...
    struct command metadata_open[] = {
        ...
        { SMB2_QUERY_INFO, related, SMB2_FLAGS_RELATED_OPERATIONS, 0, ST_SUCCESS },
        { SMB2_READ, related, SMB2_FLAGS_RELATED_OPERATIONS, 0, ST_ACCESS_DENIED },
        { SMB2_CLOSE, related, SMB2_FLAGS_RELATED_OPERATIONS, 0, ST_SUCCESS }
    };
    ...
    assert(smb2_create_opts(c, "compound-dir", FILE_OPEN_IF, FILE_ALL_ACCESS,
