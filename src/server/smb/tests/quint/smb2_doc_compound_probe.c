...
main(void)
{
    ...
    assert(smb2_create(c, "doc-pair", FILE_OPEN_IF, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &a) == ST_SUCCESS);
    assert(smb2_create(c, "doc-pair", FILE_OPEN, FILE_ALL_ACCESS,
    ...
    assert(smb2_create(c, "doc-clear", FILE_OPEN_IF, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &a) == ST_SUCCESS);
    assert(smb2_create(c, "doc-clear", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &b) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    set(&p, c, a.file_id, SMB2_FILE_DISPOSITION_T, &yes, 1);
    query(&p, c, b.file_id, 1, 0x05);
    ...
    assert(smb2_create_opts(c, "doc-create", FILE_OPEN_IF, FILE_ALL_ACCESS,
                            FILE_SHARE_RWD, FILE_DELETE_ON_CLOSE, NULL, &a) == ST_SUCCESS);
    assert(smb2_create(c, "doc-create", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &b) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    close_op(&p, c, b.file_id);
    close_op(&p, c, a.file_id);
     ...
     * FILE_DELETE_ON_CLOSE mode, regardless of VFS cache handle sharing. */
    assert(smb2_create_opts(c, "doc-clear-control", FILE_OPEN_IF, FILE_ALL_ACCESS,
                            FILE_SHARE_RWD, FILE_DELETE_ON_CLOSE, NULL, &a) == ST_SUCCESS);
    assert(smb2_create(c, "doc-clear-control", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &b) == ST_SUCCESS);
    ...
    uint32_t peer_clear_status = smb2_create(c, "doc-clear-control", FILE_OPEN,
                                             FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &probe);
    ...
    assert(smb2_create_opts(c, "doc-create-clear", FILE_OPEN_IF, FILE_ALL_ACCESS,
                            FILE_SHARE_RWD, FILE_DELETE_ON_CLOSE, NULL, &a) == ST_SUCCESS);
    assert(smb2_create(c, "doc-create-clear", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &b) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    set(&p, c, b.file_id, SMB2_FILE_DISPOSITION_T, &no, 1);
    close_op(&p, c, a.file_id);
    ...
    doc_send(&p, c);
    assert(smb2_create(c, "doc-create-clear", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &probe) == peer_clear_status);
    if (peer_clear_status == ST_SUCCESS) {
        assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
    }

    /* Even its own disposition clear preserves the distinct CREATE mode. */
    assert(smb2_create_opts(c, "doc-self-clear", FILE_OPEN_IF, FILE_ALL_ACCESS,
                            FILE_SHARE_RWD, FILE_DELETE_ON_CLOSE, NULL, &a) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    ...
    doc_send(&p, c);
    pending(c, 1, false);
    assert(smb2_create(c, "doc-self-clear", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &probe) == ST_OBJECT_NAME_NOT_FOUND);
    ...
    assert(smb2_create(c, "doc-private-peer", FILE_OPEN_IF, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &a) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    ...
    close_op(&p, c, a.file_id);
    doc_send(&p, c);
    assert(smb2_create(c, "doc-private-peer", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &probe) == ST_OBJECT_NAME_NOT_FOUND);
    ...
    smb2_handshake(d);
    assert(smb2_create(c, "doc-held", FILE_OPEN_IF, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &a) == ST_SUCCESS);
    assert(smb2_create(d, "doc-held", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &b) == ST_SUCCESS);
    ...
    assert(smb2_create(departing, "doc-held", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &departing_open) == ST_SUCCESS);
    assert(smb2_create(d, "doc-unwind", FILE_OPEN_IF, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &unwind_open) == ST_SUCCESS);
    assert(smb2_create(departing, "doc-unwind", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &unwind_peer) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    set(&p, c, a.file_id, SMB2_FILE_DISPOSITION_T, &yes, 1);
    query(&p, c, a.file_id, 1, 0x05);
    pause_finish = 1;
    ...
    uint32_t fenced_open_status = smb2_create(d, "doc-held", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &probe);
    ...
    assert(smb2_create(c, "doc-readonly", FILE_OPEN_IF, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &a) == ST_SUCCESS);
    uint8_t basic[40] = { 0 };
    ...
    doc_send(&p, c);
    pending(c, 1, false);
    assert(smb2_create(c, "doc-readonly", FILE_OPEN, 0x00000001u /* FILE_READ_DATA */,
                       FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
    assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
    ...
    assert(smb2_create(c, "doc-retry", FILE_OPEN_IF, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &a) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    set(&p, c, a.file_id, SMB2_FILE_DISPOSITION_T, &yes, 1);
    query(&p, c, a.file_id, 1, 0x05);
    ...
    assert(length == 24 && !out[20]);
    assert(smb2_close(c, a.file_id) == ST_SUCCESS);
    assert(smb2_create(c, "doc-retry", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &probe) == ST_SUCCESS);
    assert(smb2_close(c, probe.file_id) == ST_SUCCESS);
