...
extern void smb_cache_close_inspect(void *, unsigned int);
extern void smb_durable_close_inspect(void *, bool, bool);
extern void smb_durable_close_accepted(void);
extern void smb_persisted_close_record_check(void *, bool, void (*)(void *), void *);
...
static void
delete_observed(enum chimera_vfs_error status, void *private_data)
{
...
static void
delete_missing(enum chimera_vfs_error status, void *private_data)
{
    struct delete_probe *ctx = private_data;
    assert(status == CHIMERA_VFS_OK); /* This key really existed. */
    typedef void (*delete_fn)(struct chimera_vfs_thread *, const struct chimera_vfs_cred *,
        const void *, int, const void *, uint32_t, chimera_vfs_delete_key_callback_t, void *);
...
chimera_vfs_delete_key_at(struct chimera_vfs_thread *thread, const struct chimera_vfs_cred *cred,
    const void *fh, int fh_len, const void *key, uint32_t key_len,
    chimera_vfs_delete_key_callback_t callback, void *private_data)
{
    typedef void (*delete_fn)(struct chimera_vfs_thread *, const struct chimera_vfs_cred *,
        const void *, int, const void *, uint32_t, chimera_vfs_delete_key_callback_t, void *);
    delete_fn next = (delete_fn) dlsym(RTLD_NEXT, "chimera_vfs_delete_key_at");
    ...
    if (persistent_case == PERSIST_FAILURE) { delete_observed(CHIMERA_VFS_EIO, ctx); return; }
...
static void
finish_poll(struct evpl *evpl, struct evpl_timer *timer)
{
...
static void
hold_finish(struct chimera_vfs_compound *compound, void *private_data)
{
    struct finish_hold *ctx = private_data;
    ...
    if (expected_members) { smb_cache_close_inspect(command, expected_members); }
    if (expect_durable) {
    ...
    } else { start_finish_hold(ctx); }
}

static void (*retire_complete)(struct chimera_vfs_compound *, uint32_t,
                              enum chimera_vfs_error *, void *);
static void
cancel_after_retire(struct chimera_vfs_compound *compound, uint32_t index,
...
hold_complete(struct chimera_vfs_compound *compound, void *private_data)
{
    ...
    if (check_parked) smb_durable_close_accepted();
}
            ...
            if (persistent_case == PERSIST_CANCEL || persistent_case == PERSIST_CANCEL_DELETE) {
                ...
                for (unsigned int i = 0; i < chimera_vfs_compound_num_ops(compound); i++) {
                    ...
                    if (op->type != (persistent_case == PERSIST_CANCEL ?
                        ...
                        CHIMERA_VFS_COMPOUND_OP_DELETE_KEY_AT)) { continue; }
                    ...
                    void (*prepare)(struct chimera_vfs_compound *, uint32_t,
                        enum chimera_vfs_error *, void *) = op->prepare;
...
static uint8_t *
append(struct packet *p, struct smb2_conn *c, uint16_t command, unsigned int size)
{
...
static void close_op(struct packet *p, struct smb2_conn *c, const uint8_t *fid)
{
    uint8_t *b = append(p, c, SMB2_CLOSE, 24);
    p16(b, 0, 24); p16(b, 2, 1); memcpy(b + 8, fid, 16);
}
static void send_packet(struct packet *p, struct smb2_conn *c)
{
    memcpy(c->sbuf + 4, p->data, p->length);
    smb2c_send(c, p->length - SMB2_HDR_SIZE);
    c->msg_id = p->mid + p->count;
}
static void check_packet(struct packet *p, struct smb2_conn *c, const uint32_t *statuses)
{
    ...
    for (unsigned int i = 0; i < p->count; i++) {
        ...
        if (next) { assert(!(next & 7) && next >= 64); h += next; }
    }
}
...
static void
held_cache_close(struct smb2_env *env, struct smb2_conn *c,
                 const uint8_t *fid, const uint8_t *other, unsigned int members)
{
...
static void
directory_cache_close_cases(struct smb2_env *env, struct smb2_conn *c,
                            struct smb2_conn *peer, const uint8_t *other)
{
    ...
    assert(smb2_create_opts(c, "directory-cache-close", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, FILE_DIRECTORY_FILE, &lease, &first) == ST_SUCCESS);
    assert(first.has_lease && first.lease_state == SMB2_LEASE_RH);
    assert(smb2_create_opts(c, "directory-cache-close", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, FILE_DIRECTORY_FILE, &lease, &second) == ST_SUCCESS);
    ...
    smb2_create_post(peer, "directory-cache-close\\child", FILE_CREATE,
        FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL);
    uint64_t deadline = smb2c_now_ms() + 10000;
    while (smb2_conn_nbreaks(c) == breaks && smb2c_now_ms() < deadline) smb2_pump(env);
    ...
    assert(smb2_create(peer, "directory-cache-close\\child", FILE_OPEN,
        FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &child) == ST_SUCCESS);
    assert(smb2_close(peer, child.file_id) == ST_SUCCESS);
}

static void
stream_cache_close_cases(struct smb2_env *env, struct smb2_conn *c,
                         struct smb2_conn *peer, const uint8_t *other)
{
    for (unsigned int directory = 0; directory < 2; directory++) {
        ...
        assert(smb2_create_opts(c, base, FILE_CREATE, FILE_ALL_ACCESS, FILE_SHARE_RWD,
            directory ? FILE_DIRECTORY_FILE : FILE_NON_DIRECTORY_FILE, NULL, &base_open) == ST_SUCCESS);
        assert(smb2_create(c, stream, FILE_CREATE, FILE_ALL_ACCESS, FILE_SHARE_RWD,
            &lease, &first) == ST_SUCCESS);
        assert(first.has_lease && first.lease_state == SMB2_LEASE_RWH);
        assert(smb2_create(c, stream, FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD,
        ...
        assert(smb2_create(c, sibling, FILE_CREATE, FILE_ALL_ACCESS, FILE_SHARE_RWD,
            NULL, &opened) == ST_SUCCESS);
        assert(smb2_close(c, opened.file_id) == ST_SUCCESS);
        assert(smb2_create(c, sibling, FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD,
            &lease, &opened) == ST_INVALID_PARAMETER);
        assert(smb2_create_opts(c, base, FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD,
            directory ? FILE_DIRECTORY_FILE : FILE_NON_DIRECTORY_FILE,
            &lease, &opened) == ST_INVALID_PARAMETER);
        const uint32_t create_missing[] = { FILE_CREATE, FILE_OPEN_IF,
                                           FILE_OVERWRITE_IF, FILE_SUPERSEDE };
        for (unsigned int i = 0; i < sizeof(create_missing) / sizeof(create_missing[0]); i++) {
            assert(smb2_create(c, missing, create_missing[i], FILE_ALL_ACCESS,
                FILE_SHARE_RWD, &lease, &opened) == ST_INVALID_PARAMETER);
            assert(smb2_create(c, missing, FILE_OPEN, FILE_ALL_ACCESS,
                FILE_SHARE_RWD, NULL, &opened) == ST_OBJECT_NAME_NOT_FOUND);
        }
        held_cache_close(env, c, first.file_id, other, 2);
        int breaks = smb2_conn_nbreaks(c), replies = peer->nreply_app;
        smb2_create_post(peer, stream, FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL);
        uint64_t deadline = smb2c_now_ms() + 10000;
        while (smb2_conn_nbreaks(c) == breaks && smb2c_now_ms() < deadline) smb2_pump(env);
        assert(smb2_conn_nbreaks(c) > breaks);
        ...
        assert(smb2_create_opts(peer, base, FILE_OPEN, FILE_ALL_ACCESS, 0,
            directory ? FILE_DIRECTORY_FILE : FILE_NON_DIRECTORY_FILE, NULL, &opened) == ST_SUCCESS);
        assert(smb2_close(peer, opened.file_id) == ST_SUCCESS);
    }
    ...
    struct smb2_oplock_req batch = { .level = SMB2_OPLOCK_LEVEL_BATCH };
    assert(smb2_create(c, "cache-close-doc-base", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &base) == ST_SUCCESS);
    assert(smb2_create_opts(c, "cache-close-doc-base:fork:$DATA", FILE_CREATE,
        FILE_ALL_ACCESS, FILE_SHARE_RWD, FILE_NON_DIRECTORY_FILE | FILE_DELETE_ON_CLOSE,
        &batch, &stream) == ST_SUCCESS && stream.oplock == SMB2_OPLOCK_LEVEL_BATCH);
    assert(smb2_close(c, stream.file_id) == ST_SUCCESS);
    assert(smb2_create(c, "cache-close-doc-base:fork:$DATA", FILE_OPEN,
        FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &check) == ST_OBJECT_NAME_NOT_FOUND);
    assert(smb2_close(c, base.file_id) == ST_SUCCESS);
    assert(smb2_create(c, "cache-close-doc-base", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &check) == ST_SUCCESS);
...
static void
durable_close_cases(struct smb2_env *env, struct smb2_conn *c, const uint8_t *other)
{
    for (unsigned int mode = 0; mode < 4; mode++) {
        ...
        assert(smb2_create_dur(c, name, FILE_CREATE, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, &req, &durable, &file) == ST_SUCCESS);
        assert(mode == 0 ? file.has_dhnq : file.has_dh2q);
        assert(!(file.dh2q_flags & SMB2_DHANDLE_FLAG_PERSISTENT));
        if (mode == 2) {
            assert(smb2_create(c, name, FILE_OPEN, FILE_ALL_ACCESS,
                FILE_SHARE_RWD, &req, &survivor) == ST_SUCCESS);
        ...
        if (!park_durable) query(&packet, c, other);
        int replies = c->nreply_app;
        ...
        assert(smb2_create_dur(c, "", FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD,
        ...
        assert(smb2_create(c, name, FILE_OPEN, FILE_ALL_ACCESS, 0, NULL, &reopened) == ST_SUCCESS);
...
static void
persistent_close_cases(struct smb2_env *env, struct smb2_conn *c, const uint8_t *other)
{
    for (unsigned int fixture = 0; fixture < 11; fixture++) {
        ...
        if (fixture == 10) { c = smb2_conn_open(env); smb2_handshake(c); }
        ...
        uint32_t options = kind == 2 ? FILE_DIRECTORY_FILE :
            kind == 4 ? 0 : FILE_NON_DIRECTORY_FILE;
        if (kind == 2) {
            ...
            assert(smb2_create_opts(c, base, FILE_CREATE, FILE_ALL_ACCESS, FILE_SHARE_RWD,
                FILE_DIRECTORY_FILE, NULL, &base_open) == ST_SUCCESS);
            assert(smb2_close(c, base_open.file_id) == ST_SUCCESS);
        }
        struct smb2_oplock_req lease = { .is_lease = 1, .lease_state = SMB2_LEASE_RWH };
        if (kind == 3) { lease.is_lease = 0; lease.level = SMB2_OPLOCK_LEVEL_BATCH; }
        ...
        assert(smb2_create_dur_opts(c, name, kind == 2 ? FILE_OPEN : FILE_CREATE,
        ...
        if (kind == 3) { assert(file.oplock == SMB2_OPLOCK_LEVEL_BATCH); }
        else if (kind) { assert(file.has_lease && file.lease_state); }
        if (fixture == 10) {
            ...
            assert(smb2_create_dur_opts(c, "", FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD,
        ...
        if (suffix) { query(&packet, c, other); }
        int replies = c->nreply_app;
        ...
        assert(smb2_create_opts(c, name, FILE_OPEN, FILE_ALL_ACCESS, 0, options,
...
static void
durable_specialized_close_cases(struct smb2_env *env, struct smb2_conn *c,
                                 const uint8_t *other)
{
    for (unsigned int kind = 0; kind < 3; kind++) {
        ...
        for (unsigned int mode = 0; mode < 4; mode++) {
            ...
            uint32_t options = stream ? FILE_NON_DIRECTORY_FILE : FILE_DIRECTORY_FILE;
            uint32_t access = stream ? FILE_READ_ACCESS | FILE_WRITE_ACCESS : FILE_ALL_ACCESS;
            uint32_t share = stream ? FILE_SHARE_READ | FILE_SHARE_WRITE : FILE_SHARE_RWD;
            struct smb2_create_out base_open, file, survivor, reopened;
            assert(smb2_create_opts(c, base, FILE_CREATE, FILE_ALL_ACCESS, FILE_SHARE_RWD,
                directory_base ? FILE_DIRECTORY_FILE : FILE_NON_DIRECTORY_FILE,
                NULL, &base_open) == ST_SUCCESS);
            assert(smb2_close(c, base_open.file_id) == ST_SUCCESS);
            if (directory_base) {
                assert(smb2_create(c, child, FILE_CREATE, FILE_ALL_ACCESS,
                    FILE_SHARE_RWD, NULL, &base_open) == ST_SUCCESS);
                assert(smb2_close(c, base_open.file_id) == ST_SUCCESS);
            }
            struct smb2_conn *holder = c;
            if (mode == 3) { holder = smb2_conn_open(env); smb2_handshake(holder); }
            ...
            assert(smb2_create_dur_opts(holder, name, stream ? FILE_CREATE : FILE_OPEN,
            ...
            } else { assert(file.oplock == SMB2_OPLOCK_LEVEL_BATCH); }
            if (stream) {
                uint32_t written = 0;
            ...
            if (mode == 0) {
                assert(smb2_create_opts(holder, name, FILE_OPEN, access, share, options,
            ...
            if (mode == 3) {
                ...
                assert(smb2_create_dur_opts(holder, "", FILE_OPEN, access, share, options,
                    &req, &reconnect, &file) == ST_SUCCESS);
                assert(!memcmp(file.file_id, reconnect.file_id, sizeof(file.file_id)));
                if (stream) assert(file.end_of_file == 12);
            }
            expect_durable = 1;
            reject_first = mode == 1;
            park_durable = mode == 2;
            ...
            if (stream) {
                ...
                assert(smb2_create(c, name, FILE_OPEN, FILE_READ_ACCESS,
                    FILE_SHARE_RWD, NULL, &reopened) == ST_SUCCESS);
                assert(smb2_read(c, reopened.file_id, 0, sizeof(bytes), bytes, &length) == ST_SUCCESS);
                assert(length == 12 && !memcmp(bytes, "durable-fork", 12));
                assert(smb2_close(c, reopened.file_id) == ST_SUCCESS);
            }
            if (directory_base) {
                assert(smb2_create(c, child, FILE_OPEN, FILE_READ_ACCESS,
                    FILE_SHARE_RWD, NULL, &reopened) == ST_SUCCESS);
                assert(smb2_close(c, reopened.file_id) == ST_SUCCESS);
            }
            if (mode == 3) smb2_conn_disconnect(holder);
        }
    }
}
...
int main(void)
{
    ...
    assert(smb2_create(c, "cache-close-other", FILE_CREATE, FILE_ALL_ACCESS,
    ...
    for (unsigned int i = 0; i < sizeof(levels); i++) {
        ...
        assert(smb2_create(c, name, FILE_CREATE, FILE_ALL_ACCESS, FILE_SHARE_RWD, &oplock, &file) == ST_SUCCESS);
        ...
        assert(smb2_close(c, file.file_id) == ST_FILE_CLOSED);
        assert(smb2_create(peer, name, FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &opened) == ST_SUCCESS);
        assert(smb2_close(peer, opened.file_id) == ST_SUCCESS);
    }
    ...
    assert(smb2_create(c, "cache-close-held", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, &oplock, &file) == ST_SUCCESS && file.oplock == SMB2_OPLOCK_LEVEL_BATCH);
    ...
    smb2_create_post(peer, "cache-close-held", FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL);
    ...
    assert(smb2_create(c, "cache-close-error", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, &oplock, &file) == ST_SUCCESS);
    ...
    assert(smb2_create(c, "shared-close", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, &lease, &file) == ST_SUCCESS && file.has_lease &&
        (file.lease_state & SMB2_LEASE_WRITE));
    assert(smb2_create(c, "shared-close", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, &lease, &opened) == ST_SUCCESS && opened.has_lease &&
    ...
    smb2_create_post(peer, "shared-close", FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL);
    deadline = smb2c_now_ms() + 10000;
    while (smb2_conn_nbreaks(c) == breaks && smb2c_now_ms() < deadline) smb2_pump(&env);
    assert(smb2_conn_nbreaks(c) > breaks);
    ...
    assert(smb2_create(c, "persistent-close-other", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &other) == ST_SUCCESS);
