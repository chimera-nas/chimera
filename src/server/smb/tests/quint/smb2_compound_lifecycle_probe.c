...
static void
reject_directory_admission(struct chimera_vfs_compound *compound, uint32_t index,
                            enum chimera_vfs_error *status, void *private_data)
{
    (void) private_data;
    if (directory_prepare) { directory_prepare(compound, index, status, directory_prepare_private); }
    if (*status == CHIMERA_VFS_OK) { *status = CHIMERA_VFS_EACCES; }
}

static void
reject_overwrite(struct chimera_vfs_compound *compound, uint32_t index,
                 enum chimera_vfs_error *status, void *private_data)
{
    (void) private_data;
    if (overwrite_prepare) { overwrite_prepare(compound, index, status, overwrite_prepare_private); }
    if (*status == CHIMERA_VFS_OK) { *status = CHIMERA_VFS_EACCES; }
}
...
chimera_vfs_notify_emit_lease(struct chimera_vfs_notify *notify, const uint8_t *fh,
    ...
    const char *old_name, uint16_t old_len, uint64_t skip_lo, uint64_t skip_hi, bool has_skip)
{
    typedef void (*emit_fn)(struct chimera_vfs_notify *, const uint8_t *, uint16_t,
    ...
    if ((action & CHIMERA_VFS_NOTIFY_DIR_ADDED) && name_len == sizeof(failed_name) - 1 &&
        !memcmp(name, failed_name, name_len)) { atomic_fetch_add(&failed_directory_notices, 1); }
...
chimera_vfs_notify_emit_actor(struct chimera_vfs_notify *notify, const uint8_t *fh,
    uint16_t fh_len, uint32_t action, const char *name, uint16_t name_len,
    const char *old_name, uint16_t old_len, const struct chimera_claim_actor *actor)
{
    typedef void (*emit_fn)(struct chimera_vfs_notify *, const uint8_t *, uint16_t,
...
static void
cancel_after_retirement(struct chimera_vfs_compound *compound, uint32_t index,
                         enum chimera_vfs_error *status, void *private_data)
{
    struct close_injection *ctx = private_data;
    if (ctx->retired) { ctx->retired(compound, index, status, ctx->retired_private); }
    if (*status == CHIMERA_VFS_OK) { assert(chimera_vfs_compound_cancel(compound)); }
}
...
{
    typedef struct chimera_vfs_compound *(*alloc_fn)(struct chimera_vfs_thread *, const struct chimera_vfs_cred *);
...
static void
close_finish_poll(struct evpl *evpl, struct evpl_timer *timer)
{
...
static void
close_finish(struct chimera_vfs_compound *compound, void *private_data)
{
    struct close_injection *ctx = private_data;
...
static void
close_injected_complete(struct chimera_vfs_compound *compound, void *private_data)
{
    struct close_injection *ctx = private_data;
    ...
    if (atomic_load(&armed)) {
        ...
        assert(chimera_vfs_compound_num_groups(compound) ==
            (ea_budget_boundary ? (submission ? 0 : 1) :
             data_boundary && submission ? 5 : expected_groups));
        if (legacy_ea_adapter) {
        ...
        if (fail_overwrite) {
            ...
            for (uint32_t i = 0; i < chimera_vfs_compound_num_ops(compound); i++) {
                ...
                if (op->type == CHIMERA_VFS_COMPOUND_OP_RESERVE_ACCESS) { reserved = true; }
        ...
        if (fail_directory_admission || boundary_first) {
            ...
            for (uint32_t i = 0; i < chimera_vfs_compound_num_ops(compound); i++) {
                const struct chimera_vfs_compound_op *op = chimera_vfs_compound_op(compound, i);
                if (op->type != CHIMERA_VFS_COMPOUND_OP_RESERVE_ACCESS) { continue; }
                directory_prepare = op->prepare; directory_prepare_private = op->prepare_private;
        ...
        if (reject_count >= 0 && (!data_boundary || boundary_first)) {
            ...
            if (cancel_close) {
                ...
                for (uint32_t i = 0; i < chimera_vfs_compound_num_ops(compound); i++) {
                    const struct chimera_vfs_compound_op *op = chimera_vfs_compound_op(compound, i);
                    if (op->type != CHIMERA_VFS_COMPOUND_OP_RETIRE_OPEN_CLAIMS) { continue; }
...
static void
data_open_chain(struct smb2_conn *conn, const uint8_t prefix_fid[16], int rejects)
{
    const char *name = data_chain_name ? data_chain_name : "contexts.txt";
    int body_len = smb2c_build_create_full(conn, name, FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, FILE_NON_DIRECTORY_FILE, NULL, NULL, 0);
    uint8_t create[2048];
    ...
    assert(atomic_load(&submissions) == (rejects >= 0 && !exhausted ? 2 : 1));
    if (rejects >= 0) { assert(atomic_load(&attempts) == (exhausted ? rejects : rejects + 1)); }
...
static void
live_cache_open(struct smb2_env *env, struct smb2_conn *conn, uint8_t level)
{
    ...
    assert(smb2_create(holder, name, FILE_CREATE, FILE_ALL_ACCESS, FILE_SHARE_RWD,
    ...
    atomic_store(&release_finish, 1); atomic_store(&armed, 1);
    smb2_create_post(conn, name, FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL);
...
static void
create_query_close(struct smb2_conn *conn, const char *name, uint32_t disposition,
                   uint32_t options, const struct smb2_cctx *contexts, int count,
                   int directory)
{
    ...
    int body_len = smb2c_build_create_full(conn, name, disposition,
        disposition == FILE_CREATE || data_open_access ? FILE_ALL_ACCESS : FILE_READ_ATTRIBUTES,
    ...
    if (expected_create_action >= 0) { assert(created.action == (unsigned int) expected_create_action); }
    if (expected_create_size >= 0) { assert(created.end_of_file == (uint64_t) expected_create_size); }
    ...
    if (expected_ea) {
        ...
        for (uint32_t i = expected_ea_len; i < expected_length; i++) { assert(data[i] == 0); }
    } else if (directory) {
...
static void
existing_query_close(struct smb2_conn *conn, const char *name, int rejects)
{
    struct smb2_create_out opened;
    assert(smb2_create_opts(conn, name, FILE_OPEN, FILE_READ_ATTRIBUTES,
                            FILE_SHARE_RWD, 0, NULL, &opened) == ST_SUCCESS);
    /* Build a normal QUERY followed by a related CLOSE of the existing FID. */
    int body_len = smb2c_build_create_full(conn, name, FILE_OPEN, FILE_READ_ATTRIBUTES,
                                          FILE_SHARE_RWD, 0, NULL, NULL, 0);
    (void) body_len;
...
static void
overwrite_denied(struct smb2_conn *conn, const char *name, uint32_t disposition,
                 const uint8_t held_fid[16], uint32_t expected,
                 uint32_t desired, uint32_t share)
{
    int length = smb2c_build_create_full(conn, name, disposition, desired,
        share, FILE_NON_DIRECTORY_FILE, NULL, NULL, 0);
...
static void
overwrite_failure_continuation(struct smb2_conn *conn, int failure_phase,
                                uint32_t disposition, uint32_t desired, bool stream)
{
    char name[96];
    ...
    uint32_t bytes;
    assert(smb2_create(conn, name, FILE_CREATE, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &seed) == ST_SUCCESS);
    ...
    int first_body = smb2c_build_create_full(conn, name, disposition,
        desired, failure_phase ? FILE_SHARE_READ : FILE_SHARE_RWD, FILE_NON_DIRECTORY_FILE, NULL, NULL, 0);
    ...
    int second_body = smb2c_build_create_full(conn, name, FILE_OPEN,
        failure_phase ? FILE_ALL_ACCESS : FILE_READ_ACCESS,
        failure_phase ? FILE_SHARE_RWD : FILE_SHARE_READ, FILE_NON_DIRECTORY_FILE, NULL, NULL, 0);
    ...
    if (!failure_phase) { smb2c_parse_create(conn, &first_open); }
    const uint8_t *reply = conn->rbuf + 4;
    for (unsigned int i = 0; i < 4; i++) {
        ...
        assert(g32(reply, 8) == expected);
        if (i < 3) { assert(g32(reply, 20)); reply += g32(reply, 20); }
    }
    if (!failure_phase) { assert(smb2_close(conn, first_open.file_id) == ST_SUCCESS); }
    assert(smb2_create(conn, name, FILE_OPEN, FILE_READ_ACCESS, FILE_SHARE_RWD, NULL, &seed) == ST_SUCCESS);
    uint8_t data[5];
...
overwrite_cases(struct smb2_conn *conn)
{
    ...
    for (unsigned int i = 0; i < sizeof(dispositions) / sizeof(dispositions[0]); i++) {
        ...
        assert(smb2_write(conn, seed.file_id, 0, "keep!", 5, &bytes) == ST_SUCCESS && bytes == 5);
        assert(smb2_close(conn, seed.file_id) == ST_SUCCESS);
        assert(smb2_create(conn, name, FILE_OPEN, FILE_READ_ACCESS,
            FILE_SHARE_READ, NULL, &held) == ST_SUCCESS);
        ...
        assert(smb2_create(conn, name, FILE_OPEN, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &held) == ST_SUCCESS);
        ...
        overwrite_denied(conn, name, dispositions[i], held.file_id, ST_SHARING_VIOLATION,
            FILE_READ_ATTRIBUTES, FILE_SHARE_READ | FILE_SHARE_DELETE);
        overwrite_denied(conn, name, dispositions[i], held.file_id, ST_SHARING_VIOLATION,
            FILE_READ_ATTRIBUTES, FILE_SHARE_WRITE | FILE_SHARE_DELETE);
        ...
        assert(smb2_set_info(conn, SMB2_INFO_FILE_T, SMB2_FILE_BASIC_INFO_T,
            held.file_id, basic, sizeof(basic)) == ST_SUCCESS);
        data_open_access = 1;
        expected_create_action = dispositions[i] == FILE_SUPERSEDE ? 0 : 3;
        expected_create_size = 0;
        create_query_close(conn, name, dispositions[i], FILE_NON_DIRECTORY_FILE, NULL, 0, 0);
        uint8_t data[5];
...
static uint32_t
ea_encode(uint8_t *out, const char *name, const void *value, uint16_t length, bool last)
{
    ...
    if (length) { memcpy(out + 9 + namelen, value, length); }
    return size;
}
...
create_ea_cases(struct smb2_conn *conn)
{
    ...
    for (unsigned int i = 0; i < sizeof(value); i++) { value[i] = i; }
    ...
    create_query_close(conn, "create-ea.txt", FILE_CREATE, FILE_NON_DIRECTORY_FILE, &ctx, 1, 0);
    ...
    create_query_close(conn, "create-ea-dir", FILE_CREATE, FILE_DIRECTORY_FILE, &ctx, 1, 0);
    ...
    create_query_close(conn, "create-ea.txt", FILE_OPEN, FILE_NON_DIRECTORY_FILE, &ctx, 1, 0);
    ...
    create_query_close(conn, "create-ea.txt", FILE_OVERWRITE, FILE_NON_DIRECTORY_FILE, &ctx, 1, 0);
    ctx.data = large; ctx.data_len = large_len; expected_ea = large; expected_ea_len = large_len;
    create_query_close(conn, "create-large-ea.txt", FILE_CREATE, FILE_NON_DIRECTORY_FILE, &ctx, 1, 0);
    ...
    expected_ea = small; expected_ea_len = small_len;
    create_query_close(conn, "create-duplicate-ea.txt", FILE_CREATE, FILE_NON_DIRECTORY_FILE, duplicate, 2, 0);
    uint32_t pos = ea_encode(list, "absent", NULL, 0, false);
    memcpy(list + pos, small, small_len);
    ctx.data = list; ctx.data_len = pos + small_len;
    create_query_close(conn, "create-delete-ea.txt", FILE_CREATE, FILE_NON_DIRECTORY_FILE, &ctx, 1, 0);
    ...
    create_query_close(conn, "create-case-ea.txt", FILE_CREATE, FILE_NON_DIRECTORY_FILE, &ctx, 1, 0);
    ...
    create_query_close(conn, "create-case-ea.txt", FILE_OPEN, FILE_NON_DIRECTORY_FILE, &ctx, 1, 0);
    ...
    create_query_close(conn, "create-case-delete-ea.txt", FILE_CREATE, FILE_NON_DIRECTORY_FILE, &ctx, 1, 0);
    ...
    ctx.data_len = pos;
    int len = smb2c_build_create_full(conn, "create-error-ea.txt", FILE_CREATE,
        FILE_ALL_ACCESS, FILE_SHARE_RWD, FILE_NON_DIRECTORY_FILE, NULL, &ctx, 1);
    ...
    assert(atomic_load(&submissions) == 1);
    struct smb2_create_out opened;
    assert(smb2_create(conn, "create-error-ea.txt", FILE_OPEN, FILE_ALL_ACCESS,
    ...
    for (uint32_t i = want; i < result_len; i++) { assert(result[i] == 0); }
    assert(smb2_close(conn, opened.file_id) == ST_SUCCESS);
    data_open_access = 0;
}
...
legacy_create_ea_cases(struct smb2_conn *conn)
{
    ...
    for (unsigned int phase = 0; phase < 2; phase++) {
        ...
        int body_len = smb2c_build_create_full(conn, "legacy-create-ea.txt",
            phase ? FILE_OPEN : FILE_CREATE, FILE_ALL_ACCESS, FILE_SHARE_RWD,
            FILE_NON_DIRECTORY_FILE | 0x00010000u /* FILE_OPEN_REQUIRING_OPLOCK */, &caching, &ctx, 1);
        ...
        for (uint32_t j = expected_len; j < actual_len; j++) { assert(actual[j] == 0); }
        assert(smb2_close(conn, opened.file_id) == ST_SUCCESS);
    }
    ...
    struct smb2_cctx ctx = { (const uint8_t *) "ExtA", 4, list, length };
    int body_len = smb2c_build_create_full(conn, "legacy-error-ea.txt", FILE_CREATE,
        FILE_ALL_ACCESS, 0, FILE_NON_DIRECTORY_FILE | 0x00010000u /* FILE_OPEN_REQUIRING_OPLOCK */, &caching, &ctx, 1);
    expected_groups = 0; legacy_ea_adapter = 1;
    atomic_store(&submissions, 0); atomic_store(&armed, 1);
    uint32_t status = smb2c_xfer(conn, body_len);
    atomic_store(&armed, 0); legacy_ea_adapter = 0;
    assert(status == 0x80000013u && atomic_load(&submissions) == 1);
    struct smb2_create_out opened;
    assert(smb2_create(conn, "legacy-error-ea.txt", FILE_OPEN, FILE_ALL_ACCESS,
...
static void
create_ea_budget_case(struct smb2_conn *conn, const char *name)
{
    enum { ENTRIES = 1100 };
    ...
    struct smb2_cctx ctx = { (const uint8_t *) "ExtA", 4, list, length };
    int body_len = smb2c_build_create_full(conn, name, FILE_CREATE,
...
stream_create_cases(struct smb2_conn *conn)
{
    ...
    uint32_t length;
    uint8_t data[4];
    assert(smb2_create(conn, "stream-native.txt", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &base) == ST_SUCCESS);
    assert(smb2_write(conn, base.file_id, 0, "base", 4, &length) == ST_SUCCESS && length == 4);
    data_open_access = 1;
    expected_create_action = 2;
    create_query_close(conn, "stream-native.txt:fork:$DATA", FILE_CREATE,
        FILE_NON_DIRECTORY_FILE, NULL, 0, 0);
    expected_create_action = 1;
    create_query_close(conn, "stream-native.txt:fork", FILE_OPEN_IF,
        FILE_NON_DIRECTORY_FILE, NULL, 0, 0);
    expected_create_action = -1;
    ...
    expected_groups = 1; atomic_store(&submissions, 0); atomic_store(&armed, 1);
    uint32_t status = smb2_create(conn, "stream-native.txt:fork", FILE_OPEN,
        FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &stream);
    atomic_store(&armed, 0);
    assert(status == ST_SUCCESS && atomic_load(&submissions) == 1);
    ...
    assert(smb2_create(conn, "stream-native.txt:fork", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &collision) == ST_OBJECT_NAME_COLLISION);
    assert(smb2_close(conn, stream.file_id) == ST_SUCCESS);
    assert(smb2_close(conn, base.file_id) == ST_SUCCESS);
    expected_create_action = 2;
    create_query_close(conn, "stream-created-base.txt:new", FILE_OPEN_IF,
        FILE_NON_DIRECTORY_FILE, NULL, 0, 0);
    ...
    atomic_store(&submissions, 0); atomic_store(&armed, 1);
    status = smb2_create(conn, "stream-failed-base.txt:fork", FILE_CREATE,
        FILE_ALL_ACCESS, 0, NULL, &stream);
    atomic_store(&armed, 0); fail_stream_share = 0;
    assert(status == ST_SHARING_VIOLATION && atomic_load(&submissions) == 1);
    assert(smb2_create(conn, "stream-failed-base.txt", FILE_OPEN, FILE_ALL_ACCESS,
        0, NULL, &base) == ST_SUCCESS);
    assert(smb2_close(conn, base.file_id) == ST_SUCCESS);
    assert(smb2_create(conn, "stream-failed-base.txt:fork", FILE_OPEN, FILE_ALL_ACCESS,
...
stream_extended_cases(struct smb2_conn *conn)
{
    ...
    uint32_t bytes;
    uint8_t data[8];
    assert(smb2_create(conn, "stream-overwrite", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &base) == ST_SUCCESS);
    assert(smb2_write(conn, base.file_id, 0, "base", 4, &bytes) == ST_SUCCESS);
    assert(smb2_create(conn, "stream-overwrite:sibling", FILE_CREATE, FILE_ALL_ACCESS,
    ...
    for (unsigned int i = 0; i < 3; i++) {
        ...
        assert(smb2_create(conn, name, FILE_CREATE, FILE_ALL_ACCESS,
    ...
    expected_groups = 1; atomic_store(&submissions, 0); atomic_store(&armed, 1);
    assert(smb2_create(conn, "stream-overwrite-absent:fork", FILE_OVERWRITE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &rejected) == ST_OBJECT_NAME_NOT_FOUND);
    atomic_store(&armed, 0); assert(atomic_load(&submissions) == 1);
    assert(smb2_create(conn, "stream-overwrite-absent", FILE_OPEN, FILE_READ_ACCESS,
        FILE_SHARE_RWD, NULL, &rejected) == ST_OBJECT_NAME_NOT_FOUND);
    assert(smb2_create(conn, "stream-overwrite:absent", FILE_OVERWRITE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &rejected) == ST_OBJECT_NAME_NOT_FOUND);
    ...
    for (unsigned int bit = 1; bit <= 4; bit <<= 1) {
        ...
        for (unsigned int i = 0; i < 3; i++) {
            ...
            assert(smb2_create(conn, "stream-overwrite:protected-new", dispositions[i],
                FILE_READ_ATTRIBUTES, FILE_SHARE_RWD, NULL, &rejected) == ST_ACCESS_DENIED);
            atomic_store(&armed, 0); assert(atomic_load(&submissions) == 1);
            assert(smb2_create(conn, "stream-overwrite:protected-new", FILE_OPEN,
                FILE_READ_ATTRIBUTES, FILE_SHARE_RWD, NULL, &rejected) == ST_OBJECT_NAME_NOT_FOUND);
        }
    ...
    create_query_close(conn, "stream-ea:fork", FILE_CREATE, FILE_NON_DIRECTORY_FILE, &ctx, 1, 0);
    ...
    create_query_close(conn, "stream-ea:fork", FILE_OVERWRITE, FILE_NON_DIRECTORY_FILE, &ctx, 1, 0);
    expected_ea = NULL; expected_ea_len = 0;
    assert(smb2_create(conn, "stream-ea", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &base) == ST_SUCCESS);
    uint32_t result_len;
    ...
    length += ea_encode(list + length, "bad:name", "x", 1, true);
    ctx.data_len = length;
    int body = smb2c_build_create_full(conn, "stream-ea-failed:fork", FILE_CREATE,
        FILE_ALL_ACCESS, 0, FILE_NON_DIRECTORY_FILE, NULL, &ctx, 1);
    expected_groups = 1; atomic_store(&submissions, 0); atomic_store(&armed, 1);
    assert(smb2c_xfer(conn, body) == 0x80000013u);
    atomic_store(&armed, 0); assert(atomic_load(&submissions) == 1);
    assert(smb2_create(conn, "stream-ea-failed", FILE_OPEN, FILE_ALL_ACCESS, 0, NULL, &base) == ST_SUCCESS);
    assert(smb2_query_info(conn, 1, SMB2_FILE_FULL_EA_INFO_T, base.file_id, 0,
        result, sizeof(result), &result_len) == ST_SUCCESS);
    want = ea_encode(expected, "Prefix", "ok", 2, true);
    assert(result_len == ((want + 3) & ~3u) && !memcmp(result, expected, want));
    assert(smb2_close(conn, base.file_id) == ST_SUCCESS);
    assert(smb2_create(conn, "stream-ea-failed:fork", FILE_OPEN, FILE_ALL_ACCESS, 0, NULL, &seed) == ST_SUCCESS);
    assert(smb2_close(conn, seed.file_id) == ST_SUCCESS);
    data_open_access = 0;
}
...
{
    const uint32_t access[] = { FILE_READ_ATTRIBUTES | 4u, 2u | 4u, 0x40000000u, 0x10000000u, 0x02000000u };
    const uint32_t granted[] = { FILE_READ_ATTRIBUTES, 2u, 0x00120112u, 0x001f01fbu, 0 };
    for (unsigned int i = 0; i < sizeof(access) / sizeof(access[0]); i++) {
        char name[64]; snprintf(name, sizeof(name), "unbuffered-%u.txt", i);
        int length = smb2c_build_create_full(conn, name, FILE_CREATE, access[i],
            FILE_SHARE_RWD, FILE_NON_DIRECTORY_FILE | 8u, NULL, NULL, 0);
        uint8_t *first = conn->sbuf + 4;
        uint64_t mid = g64(first, 24);
        unsigned int first_len = (SMB2_HDR_SIZE + length + 7) & ~7u;
        ...
        uint8_t *close = query + SMB2_HDR_SIZE + 40;
        related_header(close, SMB2_CLOSE, mid + 2, 0);
        body = close + SMB2_HDR_SIZE; memset(body, 0, 24); p16(body, 0, 24); memset(body + 8, 0xff, 16);
        expected_groups = 3; atomic_store(&submissions, 0); atomic_store(&armed, 1);
        ...
        struct smb2_create_out opened;
        expected_groups = 1; atomic_store(&submissions, 0); atomic_store(&armed, 1);
        assert(smb2_create_opts(conn, name, FILE_OPEN, access[i], FILE_SHARE_RWD,
            FILE_NON_DIRECTORY_FILE | 8u, NULL, &opened) == ST_SUCCESS);
        atomic_store(&armed, 0); assert(atomic_load(&submissions) == 1);
        uint32_t bytes;
    ...
    expected_groups = 1; atomic_store(&submissions, 0); atomic_store(&armed, 1);
    assert(smb2_create_opts(conn, "unbuffered-append-only.txt", FILE_CREATE, 4u,
        FILE_SHARE_RWD, FILE_NON_DIRECTORY_FILE | 8u, NULL, &denied) == ST_ACCESS_DENIED);
    atomic_store(&armed, 0); assert(atomic_load(&submissions) == 1);
    assert(smb2_create(conn, "unbuffered-append-only.txt", FILE_OPEN, FILE_READ_ATTRIBUTES,
        FILE_SHARE_RWD, NULL, &denied) == ST_OBJECT_NAME_NOT_FOUND);
}
...
reparse_option_cases(struct smb2_conn *conn)
{
    ...
    data_open_access = 1;
    create_query_close(conn, "reparse-option.txt", FILE_CREATE, FILE_NON_DIRECTORY_FILE | option, NULL, 0, 0);
    const uint32_t dispositions[] = { FILE_OPEN, FILE_OPEN_IF, FILE_OVERWRITE, FILE_OVERWRITE_IF, FILE_SUPERSEDE };
    for (unsigned int i = 0; i < sizeof(dispositions) / sizeof(dispositions[0]); i++) {
        create_query_close(conn, "reparse-option.txt", dispositions[i], FILE_NON_DIRECTORY_FILE | option, NULL, 0, 0);
    }
    create_query_close(conn, "reparse-option-dir", FILE_CREATE, FILE_DIRECTORY_FILE | option, NULL, 0, 1);
    create_query_close(conn, "reparse-option-dir", FILE_OPEN_IF, FILE_DIRECTORY_FILE | option, NULL, 0, 1);
    data_open_access = 0;
    struct smb2_create_out target, link, rejected;
    const char *target_name = "reparse-target.txt";
    assert(smb2_create(conn, target_name, FILE_CREATE, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &target) == ST_SUCCESS);
    uint32_t bytes;
    assert(smb2_write(conn, target.file_id, 0, "keep!", 5, &bytes) == ST_SUCCESS);
    assert(smb2_create(conn, "reparse-link.txt", FILE_CREATE, FILE_ALL_ACCESS, FILE_SHARE_RWD, NULL, &link) == ST_SUCCESS);
     ...
     * legacy FILE_CREATE retains its collision result rather than following. */
    expected_groups = 1; atomic_store(&submissions, 0); atomic_store(&armed, 1);
    assert(smb2_create_opts(conn, "reparse-link.txt", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, FILE_NON_DIRECTORY_FILE | option, NULL, &rejected) == ST_OBJECT_NAME_COLLISION);
    atomic_store(&armed, 0); assert(atomic_load(&submissions) == 1);
    assert(smb2_create_opts(conn, "reparse-link.txt", FILE_OPEN, FILE_READ_ATTRIBUTES,
        FILE_SHARE_RWD, option, NULL, &link) == ST_SUCCESS);
...
{
    uint8_t packet[2048] = { 0 };
    int first_body = smb2c_build_create_full(conn, "related-first.txt", FILE_CREATE,
        FILE_ALL_ACCESS, FILE_SHARE_RWD, FILE_NON_DIRECTORY_FILE, NULL, NULL, 0);
    uint64_t mid = g64(conn->sbuf + 4, 24);
    unsigned int first_len = (SMB2_HDR_SIZE + first_body + 7) & ~7u;
    memcpy(packet, conn->sbuf + 4, SMB2_HDR_SIZE + first_body);
    p32(packet, 20, first_len);
    int second_body = smb2c_build_create_full(conn, "related-second.txt", FILE_CREATE,
        FILE_ALL_ACCESS, FILE_SHARE_RWD, FILE_NON_DIRECTORY_FILE, NULL, NULL, 0);
    unsigned int second_len = (SMB2_HDR_SIZE + second_body + 7) & ~7u;
    uint8_t *second = packet + first_len;
    ...
    for (unsigned int i = 0; i < 4; i++) {
        ...
        uint32_t offset = g32(reply, 20);
        if (i < 3) { assert(offset); reply += offset; }
        else { assert(!offset); }
    }
...
main(void)
{
    ...
    for (unsigned int length = 256; length <= 512; length += 256) {
        ...
        assert(smb2_create(conn, oversized, FILE_OPEN_IF, FILE_ALL_ACCESS,
            FILE_SHARE_RWD, NULL, &rejected) == 0xc0000106u); /* NAME_TOO_LONG */
    }
    ...
    p16(sd, 30, 20); p32(sd, 32, FILE_ALL_ACCESS); /* allow Everyone */
    ...
    create_query_close(conn, "contexts.txt", FILE_CREATE, FILE_NON_DIRECTORY_FILE,
                       contexts, 4, 0);
    create_query_close(conn, "contexts.txt", FILE_OPEN, FILE_NON_DIRECTORY_FILE,
                       contexts, 2, 0);
    data_open_access = 1;
    create_query_close(conn, "contexts.txt", FILE_OPEN, FILE_NON_DIRECTORY_FILE,
                       contexts, 2, 0);
    ...
    assert(smb2_create(conn, "contexts.txt", FILE_OPEN, FILE_READ_ATTRIBUTES,
        FILE_SHARE_RWD, NULL, &prefix_open) == ST_SUCCESS);
    data_open_chain(conn, NULL, -1);
    ...
    create_query_close(conn, "open-if-new.txt", FILE_OPEN_IF, FILE_NON_DIRECTORY_FILE, NULL, 0, 0);
    expected_create_action = 1; /* FILE_OPENED */
    create_query_close(conn, "open-if-new.txt", FILE_OPEN_IF, FILE_NON_DIRECTORY_FILE, NULL, 0, 0);
    create_query_close(conn, "contexts.txt", FILE_OPEN_IF, FILE_NON_DIRECTORY_FILE, contexts, 2, 0);
    struct smb2_create_out preserved;
    uint8_t preserved_data[4]; uint32_t preserved_length;
    assert(smb2_create(conn, "contexts.txt", FILE_OPEN, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &preserved) == ST_SUCCESS);
    ...
    expected_create_action = 2;
    create_query_close(conn, "open-if-dir", FILE_OPEN_IF, FILE_DIRECTORY_FILE, NULL, 0, 1);
    expected_create_action = 1;
    create_query_close(conn, "open-if-dir", FILE_OPEN_IF, FILE_DIRECTORY_FILE, NULL, 0, 1);
    expected_create_action = -1; data_open_access = 0;
    ...
    overwrite_failure_continuation(conn, 1, FILE_OVERWRITE, FILE_ALL_ACCESS, false);
    overwrite_failure_continuation(conn, 1, FILE_OVERWRITE, FILE_ALL_ACCESS, true);
    overwrite_failure_continuation(conn, 2, FILE_OVERWRITE, FILE_ALL_ACCESS, false);
    overwrite_failure_continuation(conn, 2, FILE_OVERWRITE, FILE_ALL_ACCESS, true);
    const uint32_t truncate_dispositions[] = { FILE_OVERWRITE, FILE_OVERWRITE_IF, FILE_SUPERSEDE };
    for (unsigned int i = 0; i < 3; i++) {
        overwrite_failure_continuation(conn, 0, truncate_dispositions[i], FILE_READ_ATTRIBUTES, false);
        overwrite_failure_continuation(conn, 0, truncate_dispositions[i], FILE_READ_ATTRIBUTES, true);
        overwrite_failure_continuation(conn, 0, truncate_dispositions[i], FILE_READ_ACCESS, false);
        overwrite_failure_continuation(conn, 0, truncate_dispositions[i], FILE_READ_ACCESS, true);
    }
    ...
    create_query_close(conn, "", FILE_OPEN, FILE_DIRECTORY_FILE, contexts, 2, 1);
    struct smb2_create_out dir;
    assert(smb2_create_opts(conn, "nested", FILE_OPEN_IF, FILE_ALL_ACCESS,
                           FILE_SHARE_RWD, FILE_DIRECTORY_FILE, NULL, &dir) == ST_SUCCESS);
    assert(smb2_close(conn, dir.file_id) == ST_SUCCESS);
    create_query_close(conn, "nested\\child.txt", FILE_CREATE, FILE_NON_DIRECTORY_FILE,
                       contexts, 2, 0);
    create_query_close(conn, "nested", FILE_OPEN, FILE_DIRECTORY_FILE, contexts, 2, 1);
    create_query_close(conn, "compound-directory", FILE_CREATE, FILE_DIRECTORY_FILE, contexts, 2, 1);
    create_query_close(conn, "compound-directory\\nested", FILE_CREATE, FILE_DIRECTORY_FILE, contexts, 2, 1);
    ...
    atomic_store(&submissions, 0); atomic_store(&armed, 1);
    uint32_t failed_status = smb2_create_opts(conn, "created-admission-failure", FILE_CREATE,
        FILE_ALL_ACCESS, FILE_SHARE_RWD, FILE_DIRECTORY_FILE, NULL, &failed_directory);
    ...
    for (unsigned int i = 0; i < 257; i++) {
        ...
        atomic_store(&submissions, 0);
        atomic_store(&armed, 1);
        uint32_t result = smb2_create(conn, name, FILE_CREATE, FILE_ALL_ACCESS,
                                     FILE_SHARE_RWD, NULL, &opened);
        atomic_store(&armed, 0);
    ...
    create_query_close(conn, "registry-256.txt", FILE_OPEN, FILE_NON_DIRECTORY_FILE,
                       contexts, 2, 0);
