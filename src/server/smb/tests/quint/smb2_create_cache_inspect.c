...
void
smb_create_cache_inspect(void *private_data, unsigned int expected_members, unsigned int expected_epoch, unsigned int expected_state)
{
...
void
smb_persistent_create_expect(bool persistent, int fail_put)
{
...
static void
ambiguous_put_done(enum chimera_vfs_error status, void *private_data)
{
...
chimera_vfs_put_key_at(struct chimera_vfs_thread *thread, const struct chimera_vfs_cred *cred,
    const void *fh, int fh_len, const void *key, uint32_t key_len,
    const void *value, uint32_t value_len, chimera_vfs_put_key_callback_t callback, void *private_data)
{
    typedef void (*put_fn)(struct chimera_vfs_thread *, const struct chimera_vfs_cred *,
        const void *, int, const void *, uint32_t, const void *, uint32_t,
        chimera_vfs_put_key_callback_t, void *);
    ...
    if (fail == 1) { callback(CHIMERA_VFS_EIO, private_data); return; }
    if (fail == 2) {
...
static void
persistent_create_record_checked(struct chimera_vfs_compound *compound, void *private_data)
{
...
chimera_smb_complete_request(struct chimera_smb_request *request, unsigned int status)
{
    ...
    if (status == SMB2_STATUS_SUCCESS) {
        ...
        pthread_mutex_lock(&request->compound->thread->shared->durable.lock);
        HASH_FIND(hh, request->compound->thread->shared->durable.by_pid,
            &pid, sizeof(pid), entry);
        assert(!!entry == !!open->durable_flags);
        if (entry) assert(entry->open_file == open && entry->persistent == persistent);
        pthread_mutex_unlock(&request->compound->thread->shared->durable.lock);
    } else {
...
{
    typedef uint32_t (*fn)(struct chimera_smb_request *);
    fn next = (fn) dlsym(RTLD_NEXT, "chimera_smb_lease_key_begin");
    assert(next);
    uint32_t status = next(request);
    if (status == SMB2_STATUS_PENDING) { atomic_fetch_add(&lease_key_pending, 1); }
    return status;
}
...
void smb_lease_unlink_after_lookup(const char *name)
{
...
static void lease_unlink_done(enum chimera_vfs_error status, struct chimera_vfs_attrs *pre,
...
static void lease_unlink_lookup_done(enum chimera_vfs_error status, struct chimera_vfs_attrs *attr,
                                     struct chimera_vfs_attrs *dir_attr, void *private_data)
{
    struct lease_unlink_ctx *ctx = private_data;
    assert(status == CHIMERA_VFS_OK && attr && (attr->va_set_mask & CHIMERA_VFS_ATTR_FH));
    ctx->attr = *attr;
    if (dir_attr) { ctx->dir_attr = *dir_attr; }
...
__attribute__((visibility("default"))) void
chimera_vfs_lookup_at(struct chimera_vfs_thread *thread, const struct chimera_vfs_cred *cred,
    struct chimera_vfs_open_handle *handle, const char *name, uint32_t name_len,
    uint64_t mask, uint64_t dir_mask, chimera_vfs_lookup_at_callback_t callback, void *private_data)
{
    typedef void (*fn)(struct chimera_vfs_thread *, const struct chimera_vfs_cred *,
        struct chimera_vfs_open_handle *, const char *, uint32_t, uint64_t, uint64_t,
        chimera_vfs_lookup_at_callback_t, void *);
