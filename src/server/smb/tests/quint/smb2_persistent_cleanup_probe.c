...
extern void smb_persistent_cleanup_test_arm_park(
    void);
extern int smb_persistent_cleanup_test_park(
    void);
...
main(void)
{
    ...
    for (unsigned int parked = 0; parked < 2; parked++) {
        ...
        assert(smb2_create(conn, name, MBT_FILE_OPEN,
                           MBT_FILE_ALL_ACCESS, 0, NULL, &opened) == ST_SUCCESS);
        uint32_t               result_len;
