...
extern void reparse_test_before(
    struct chimera_vfs_compound *,
    bool);
extern void reparse_test_after(
    bool);
    ...
    void                        *private_data)
{
    ...
    if (!reparse_teardown) {
        reparse_test_before(compound, ready);
    }
    if (retry) {
    ...
    struct evpl_timer *timer)
{
    ...
    atomic_store(&reparse_pause, 0);
    if (ctx->getattr) {
        ctx->getattr(ctx->error, &ctx->attr, ctx->private_data);
    } else {
        ctx->symlink(ctx->error, &ctx->attr, &ctx->pre, &ctx->post, ctx->private_data);
    }
    free(ctx);
} /* reparse_poll_io */
static void
reparse_getattr_done(
    enum chimera_vfs_error    error,
    ...
    void                           *private_data)
{
    typedef void (*fn)(
        ...
        uint64_t,
        chimera_vfs_getattr_callback_t,
        void *);
    ...
    if (atomic_exchange(&reparse_getattr_hold, 0)) {
        ...
        next(thread, cred, handle, mask, callback, private_data);
    }
} /* chimera_vfs_getattr */
    ...
    void                     *private_data)
{
    ...
    if (fault == 2) {
        atomic_store(&fail_reparse_open, 1);
    }
    ...
    void                             *private_data)
{
    typedef void (*fn)(
        ...
        uint64_t,
        uint64_t,
        chimera_vfs_symlink_at_callback_t,
        void *);
    ...
    void                          *private_data)
{
    typedef void (*fn)(
        ...
        int,
        unsigned int,
        chimera_vfs_open_fh_callback_t,
        void *);
    ...
    void                           *private_data)
{
    ...
    if (chimera_vfs_compound_num_ops(cp) == 10 &&
        ...
        if (reparse_retry) {
            chimera_vfs_compound_set_op_prepare(cp, 0, reparse_readonly_stop, ctx);
        }
    ...
    atomic_int      *flag)
{
    ...
    while (!atomic_load(flag) && smb2c_now_ms() < deadline) {
        smb2_pump(env);
    }
    assert(atomic_load(flag));
} /* reparse_wait_flag */
static void
reparse_arm_case(void)
{
    ...
    uint32_t          status)
{
    ...
    while (!(c->reply_ready && c->reply_mid == id) && smb2c_now_ms() < deadline) {
        smb2_pump(c->env);
    }
    ...
    struct smb2_conn *c)
{
    ...
    assert(smb2_create(c, "reparse-retry", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, NULL, &open) ==
           ST_SUCCESS);
    ...
    assert(smb2_create(c, "reparse-held", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, NULL, &open) ==
           ST_SUCCESS);
    ...
    assert(smb2_create(c, "reparse-reader-first", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, NULL, &open)
           ==
           ST_SUCCESS);
    ...
    assert(smb2_create(c, "reparse-cancel-before", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, NULL, &open
                       ) ==
           ST_SUCCESS);
    ...
    assert(smb2_create(c, "reparse-cancel-after", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, NULL, &open)
           ==
           ST_SUCCESS);
    ...
    for (unsigned i = 0; i < 4; ++i) {
        ...
        assert(smb2_create(c, name, MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, NULL, &open) == ST_SUCCESS
               );
        memset(input, 0, sizeof(input)); p32(input, 0, 0x80000014u); /* IO_REPARSE_TAG_NFS */
        p16(input, 4, i < 2 ? 16 : 8); p64(input, 8, types[i]);
        if (i < 2) {
            p32(input, 16, 1); p32(input, 20, 7);
        }
        if (i < 2) {
            ...
            assert(smb2_read(c, open.file_id, 0, 4, bytes, &length) == ST_SUCCESS && length == 4 && !memcmp(bytes,
                                                                                                            "keep", 4));
            struct smb2_create_out peer;
            assert(smb2_create(c, name, MBT_FILE_OPEN, MBT_FILE_READ_ACCESS, MBT_FILE_SHARE_RWD, NULL, &peer) ==
                   ST_SUCCESS);
            assert(smb2_read(c, peer.file_id, 0, 4, bytes, &length) == ST_SUCCESS && length == 4 && !memcmp(bytes,
                                                                                                            "keep", 4));
    ...
    input_len = reparse_symlink_input(input, "target");
    int            create_len = smb2c_build_create(c, "reparse-related", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
    ...
    for (unsigned i = 0; i < 3; ++i) {
        ...
        if (i < 2) {
            assert(g32(header, 20)); header += g32(header, 20);
        }
    }
    assert(smb2_close(c, open.file_id) == ST_SUCCESS);
    ...
    assert(smb2_create(tdis, "reparse-tdis", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, NULL, &open) ==
           ST_SUCCESS);
    ...
    assert(smb2_create(drop, "reparse-disconnect", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, NULL, &open
                       ) ==
           ST_SUCCESS);
    reparse_teardown = 1; reparse_arm_case(); atomic_store(&reparse_create_hold, 1);
...
reparse_identity_checks(struct smb2_conn *c)
{
    ...
    assert(smb2_create(c, "reparse-stale", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, NULL, &old) == ST_SUCCESS);
    assert(smb2_write(c, old.file_id, 0, "old", 3, &written) == ST_SUCCESS);
    assert(smb2_create(c, "reparse-replacement", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
    ...
    assert(smb2_create(c, "reparse-stale", MBT_FILE_OPEN, MBT_FILE_READ_ACCESS,
                       MBT_FILE_SHARE_RWD, NULL, &verify) == ST_SUCCESS);
    ...
    assert(smb2_create(c, "reparse-unsupported", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                       MBT_FILE_SHARE_RWD, NULL, &old) == ST_SUCCESS);
    ...
    assert(smb2_create(c, "reparse-unsupported", MBT_FILE_OPEN, MBT_FILE_READ_ACCESS,
                       MBT_FILE_SHARE_RWD, NULL, &verify) == ST_SUCCESS);
    assert(smb2_read(c, verify.file_id, 0, sizeof(bytes), bytes, &length) == ST_SUCCESS &&
           length == 4 && !memcmp(bytes, "keep", 4));
    assert(smb2_close(c, verify.file_id) == ST_SUCCESS);
    ...
    input_len = reparse_symlink_input(input, "target");
    for (int i = 0; i < 2; i++) {
        assert(smb2_create(c, names[i], MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS,
                           MBT_FILE_SHARE_RWD, NULL, &old) == ST_SUCCESS);
        ...
        assert(smb2_create_opts(c, names[i], MBT_FILE_OPEN, MBT_FILE_READ_ATTRIBUTES,
                                MBT_FILE_SHARE_RWD, MBT_FILE_NON_DIRECTORY_FILE | 0x00200000u, NULL, &verify) ==
               ST_SUCCESS);
...
main(void)
{
    ...
    assert(smb2_create(c, "copy-src:alt", MBT_FILE_OPEN_IF, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD, NULL, &stream) ==
           ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    ...
    assert(smb2_create(c, "resilient", MBT_FILE_CREATE, MBT_FILE_ALL_ACCESS, MBT_FILE_SHARE_RWD,
