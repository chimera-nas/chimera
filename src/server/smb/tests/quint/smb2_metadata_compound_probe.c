...
static void
link_poll(struct evpl *evpl, struct evpl_timer *timer)
{
...
chimera_vfs_link_at_flags_actor(struct chimera_vfs_thread *thread,
    const struct chimera_vfs_cred *cred, const void *fh, int fhlen,
    const void *dir_fh, int dir_fhlen, const char *name, int namelen,
    unsigned int replace, unsigned int flags, uint64_t attr_mask,
    uint64_t pre_mask, uint64_t post_mask, const uint8_t *lease_key,
    struct chimera_vfs_open_handle *handle, const struct chimera_claim_actor *actor,
    chimera_vfs_link_at_callback_t callback, void *private_data)
{
    __typeof__(&chimera_vfs_link_at_flags_actor) next = dlsym(RTLD_NEXT, "chimera_vfs_link_at_flags_actor");
    assert(next);
    if (atomic_load(&armed)) {
        atomic_fetch_add(replace ? &replacement_links : &link_attempts, 1);
        if (replace && watched_link) assert(!atomic_load(&link_notifications));
    }
    if (!replace && atomic_exchange(&hold_link, 0)) {
        ...
        if (actor) link_gate.actor = *actor;
...
__attribute__((visibility("default"))) void
chimera_vfs_notify_emit_lease(struct chimera_vfs_notify *notify,
    const uint8_t *fh, uint16_t fh_len, uint32_t action,
...
__attribute__((visibility("default"))) void
chimera_vfs_notify_emit_actor(struct chimera_vfs_notify *notify,
    const uint8_t *fh, uint16_t fh_len, uint32_t action,
    const char *name, uint16_t name_len, const char *old_name, uint16_t old_name_len,
    ...
    if (atomic_load(&armed)) {
        if (atomic_exchange(&expect_cold_root, 0)) {
            ...
            for (uint32_t i = 0; i < chimera_vfs_compound_num_ops(cp); i++) {
                ...
                if (op->type == CHIMERA_VFS_COMPOUND_OP_LOOKUP_PATH) root_lookups++;
            }
        ...
        if (chunk_mode && !groups) {
            ...
            assert(chimera_vfs_compound_op(cp, 1)->type == (chunk ?
                CHIMERA_VFS_COMPOUND_OP_CHECKPOINT : CHIMERA_VFS_COMPOUND_OP_LISTXATTRS));
        } else {
            if (atomic_load(&expected_groups) < 0) {
                assert(run < 4 && groups == expected_run_groups[run]);
            } else assert(groups == (uint32_t) atomic_load(&expected_groups));
        }
    }
...
static uint32_t
ea_entry(uint8_t *buffer, const char *name, uint8_t name_len,
         const void *value, uint16_t value_len, int last)
{
    ...
    if (value_len) { memcpy(buffer + 9 + name_len, value, value_len); }
    return aligned;
}
...
static void
replace_link(struct packet *p, struct smb2_conn *c, const uint8_t *fid, const char *name)
{
...
static void
create_op(struct packet *p, struct smb2_conn *c, const char *name)
{
    uint8_t *b = append(p, c, SMB2_CREATE, 56 + strlen(name) * 2);
    p16(b, 0, 57); p32(b, 4, 2); p32(b, 24, FILE_ALL_ACCESS);
    p32(b, 28, FILE_ATTRIBUTE_NORMAL); p32(b, 32, FILE_SHARE_RWD);
    p32(b, 36, FILE_CREATE); p32(b, 40, FILE_NON_DIRECTORY_FILE);
...
static void
metadata_check(struct packet *p, struct smb2_conn *c)
{
    unsigned int off = 4;
...
static void
metadata_send_split(struct packet *p, struct smb2_conn *c, unsigned first, unsigned second)
{
    atomic_store(&submissions, 0);
    atomic_store(&link_attempts, 0); atomic_store(&replacement_links, 0);
    atomic_store(&link_notifications, 0);
    expected_run_groups[0] = first; expected_run_groups[1] = second;
...
static void
replace_link_cases(struct smb2_env *env, struct smb2_conn *c)
{
    ...
    smb2_handshake(peer);
    assert(smb2_create(c, "replace-link-source", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &source) == ST_SUCCESS);
    assert(smb2_write(c, source.file_id, 0, "source", 6, &written) == ST_SUCCESS && written == 6);
    assert(smb2_create_opts(c, "replace-link-dir", FILE_CREATE, FILE_ALL_ACCESS,
    ...
    expected_run_groups[0] = 3; expected_run_groups[1] = 1;
    ...
    atomic_store(&armed, 0);
    assert(smb2_create(peer, watched_link, FILE_CREATE, FILE_ALL_ACCESS,
    ...
    assert(atomic_load(&submissions) == 2 && atomic_load(&link_attempts) == 1 &&
        atomic_load(&replacement_links) == 1 && atomic_load(&link_notifications) == 1);
    out = result(c, 0, &length);
    assert(length == 24 && g32(out, 16) == 3);
    out = result(c, 2, &length);
    assert(length == 24 && g32(out, 16) == 4);
    assert(smb2_create(c, watched_link, FILE_OPEN, FILE_ALL_ACCESS,
...
main(void)
{
    ...
    assert(smb2_create(c, "metadata.txt", FILE_OPEN_IF, FILE_ALL_ACCESS,
    ...
    assert(smb2_create_opts(c, "linkdir", FILE_OPEN_IF, FILE_ALL_ACCESS,
                            FILE_SHARE_RWD, FILE_DIRECTORY_FILE, NULL, &directory) == ST_SUCCESS);
    memset(&p, 0, sizeof(p));
    ...
    out = result(c, 3, &length);
    assert(length == 24 && g32(out, 16) == 3);
    assert(smb2_create(c, "linkdir\\nested.txt", FILE_OPEN, FILE_ALL_ACCESS,
    ...
    assert(smb2_create(c, "many-eas.txt", FILE_CREATE, FILE_ALL_ACCESS,
        FILE_SHARE_RWD, NULL, &many) == ST_SUCCESS);
