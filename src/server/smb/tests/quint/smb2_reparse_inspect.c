    ...
    uint32_t       length)
{
    typedef void (*fn)(
        ...
        const uint8_t *,
        uint32_t,
        void (*)(void *, const struct chimera_smb_namespace_snapshot *, void *),
        void *);
    fn       each = (fn) dlsym(RTLD_DEFAULT, "chimera_smb_namespace_foreach_locked");
    assert(each);
    unsigned count = 0;
    evpl_mutex_lock(&saved_shared->namespace_registry.lock);
    each(&saved_shared->namespace_registry, fh, length, count_peer, &count);
    evpl_mutex_unlock(&saved_shared->namespace_registry.lock);
    return count;
} /* peers */
...
void
reparse_test_after(bool migrated)
{
