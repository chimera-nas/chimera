...
extern void *smb_access_retire_test_handle(void *open);
extern int smb_access_retire_test_closed(void *open);
extern int smb_access_retire_test_timer(void *open, struct evpl_timer *timer);
                                     ...
                                     const struct chimera_vfs_claim *template_claim)
{
    typedef struct chimera_vfs_claim_access_owner *(*alloc_fn)(
        struct chimera_vfs_file_state *, const struct chimera_vfs_claim *);
...
evpl_add_oneshot_timer(struct evpl *evpl, struct evpl_timer *timer,
                       evpl_timer_callback_t callback, uint64_t delay_us)
{
    typedef void (*timer_fn)(struct evpl *, struct evpl_timer *, evpl_timer_callback_t, uint64_t);
...
static uint32_t
app_open(struct smb2_conn *conn, const char *name, uint32_t disposition,
          struct smb2_create_out *result)
{
    ...
    smb2c_send(conn, smb2c_build_create_full(conn, name, disposition,
        FILE_ALL_ACCESS, 0, FILE_NON_DIRECTORY_FILE, NULL, &context, 1));
    smb2c_wait(conn);
...
static void
run(struct smb2_env *env, struct smb2_conn *first, struct smb2_conn *second,
    const char *name, int accepted)
{
    struct smb2_create_out original, replacement;
    atomic_store(&captured, NULL);
    atomic_store(&capture, 1);
    assert(app_open(first, name, FILE_CREATE, &original) == ST_SUCCESS);
    ...
    assert(app_open(second, name, FILE_OPEN, &replacement) == ST_SHARING_VIOLATION);
    ...
    assert(app_open(second, name, FILE_OPEN, &replacement) == ST_SUCCESS);
...
park_pipe_read(struct smb2_env *env)
{
    ...
    assert(smb2_create(conn, "srvsvc", FILE_OPEN, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, NULL, &pipe) == ST_SUCCESS);
    ...
    smb2c_send(conn, 49);
    while (!conn->interim_pending) { smb2_pump(env); }
    return conn;
}
...
abandon_parked_create(struct smb2_env *env)
{
    ...
    assert(smb2_create(holder, "abandon-create.txt", FILE_CREATE, FILE_ALL_ACCESS,
                       FILE_SHARE_RWD, &caching, &opened) == ST_SUCCESS);
    assert(opened.oplock == SMB2_OPLOCK_LEVEL_BATCH);
    smb2_create_post(waiter, "abandon-create.txt", FILE_OPEN, FILE_READ_ACCESS,
                      FILE_SHARE_RWD, NULL);
...
static void
shutdown_pending(struct smb2_env *env, struct smb2_conn *first, struct smb2_conn *second)
{
    struct smb2_create_out original, replacement;
    ...
    atomic_store(&captured, NULL);
    atomic_store(&capture, 1);
    assert(app_open(first, "app-shutdown.txt", FILE_CREATE, &original) == ST_SUCCESS);
    struct chimera_vfs_claim_access_owner *owner = atomic_load(&captured);
    assert(owner);
    ...
    assert(app_open(second, "app-shutdown.txt", FILE_OPEN, &replacement) == ST_SHARING_VIOLATION);
