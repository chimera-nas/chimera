    ...
    unsigned int expected_members)
{
    ...
    if (count == 1) {
        assert(grant->claim.op_handle == open->handle);
    }
...
smb_durable_close_accepted(void)
{
    ...
    evpl_mutex_lock(&parked_shared->durable.lock);
    HASH_FIND(hh, parked_shared->durable.by_pid, &parked_pid, sizeof(parked_pid), entry);
    assert(!entry);
    evpl_mutex_unlock(&parked_shared->durable.lock);
...
void
smb_persisted_close_record_check(
    void   *private_data,
    bool    present,
    void ( *done )(void *),
    void   *arg)
