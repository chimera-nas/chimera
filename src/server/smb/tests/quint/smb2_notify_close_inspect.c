...
static pthread_mutex_t free_target_lock = PTHREAD_MUTEX_INITIALIZER;
...
{
    pthread_mutex_lock(&free_target_lock);
    ...
    pthread_mutex_unlock(&free_target_lock);
}
...
{
    typedef void (*fn)(struct chimera_smb_notify_state *);
    fn next = (fn) dlsym(RTLD_NEXT, "chimera_smb_notify_state_put");
    assert(next); next(state);
}

void
notify_test_inspect(void *private_data, unsigned int expected)
{
    ...
    for (struct chimera_smb_notify_request *nr = command->request->compound->conn->parked_notifies;
         nr; nr = nr->next) {
        if (nr->open_file != open) { continue; }
        if (atomic_load(&notify_test_attached)) {
            assert(!nr->admitting && nr->state == state);
            pthread_mutex_lock(&state->lock);
            assert(!nr->cleanup);
            pthread_mutex_unlock(&state->lock);
        } else { assert(nr->admitting && !nr->state); }
        assert(nr->session);
        pthread_mutex_lock(&nr->thread->shared->sessions_lock);
        assert(nr->session->compound_pins);
        pthread_mutex_unlock(&nr->thread->shared->sessions_lock);
        count++;
    }
    if (count != expected) {
...
chimera_smb_notify_q_push(struct chimera_smb_notify_state *state,
                         struct chimera_smb_notify_request *nr)
{
    typedef void (*fn)(struct chimera_smb_notify_state *, struct chimera_smb_notify_request *);
    ...
    if (atomic_load(&notify_test_wire_capture)) {
        ...
        if (!index) { wire_state = state; wire_thread = nr->thread; }
        else { assert(index == 1 && wire_state == state && wire_thread != nr->thread); }
    ...
    if (atomic_load(&notify_test_cross_capture)) {
        ...
        if (index == 1) { assert(cross_nr[0]->thread != nr->thread); }
        atomic_store(&notify_test_cross_count, index + 1);
    }
}

__attribute__((visibility("default"))) void
chimera_smb_notify_close(struct chimera_vfs_notify *vfs, struct chimera_smb_notify_state *state)
{
    typedef void (*fn)(struct chimera_vfs_notify *, struct chimera_smb_notify_state *);
    fn next = (fn) dlsym(RTLD_NEXT, "chimera_smb_notify_close");
    assert(next);
    if (state) { atomic_fetch_add(&notify_test_cleanups, 1); }
    ...
    if (transfer) {
        ...
        typedef void (*push_fn)(struct chimera_smb_notify_state *, struct chimera_smb_notify_request *);
        ...
        typedef void (*callback_fn)(struct chimera_vfs_notify_watch *, void *);
...
{
    typedef void (*fn)(struct chimera_smb_notify_request *);
    ...
    next(nr);
    if (target) { atomic_fetch_add(&notify_test_target_freed, 1); }
    if (retired) { atomic_fetch_add(&notify_test_retired_freed, 1); }
    if (tracked) { atomic_fetch_add(&notify_test_cross_freed, 1); }
}
