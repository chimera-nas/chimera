...
/* A module with none of the derived capabilities (the smb proxy shape)
 * advertises only the base set: no sparse, no refcounting, no ACL
 * persistence, no streams. */
static int
test_no_caps(void)
{
 ...
 * smb_named_streams knob (chimera_smb_named_streams_enabled, the gate
 * FileStreamInformation applies) so a client is never told about streams the
 * server would then refuse.  The knob-on / no-capability half is test_no_caps. */
static int
