...
smb_vfs_cancel_untrack(struct chimera_smb_vfs_batch *batch)
{
    ...
    while (*link && *link != batch) {
        link = &(*link)->cancel_next;
    }
...
smb_vfs_cancel_track(struct chimera_smb_vfs_batch *batch)
{
    ...
    if (batch->count != 1 || batch->commands[0].ops != &chimera_smb_close_compound_ops ||
        conn->generation != batch->wire->conn_generation || conn->disconnecting) {
        return;
    }
...
} /* smb_vfs_cancel_track */

bool
chimera_smb_vfs_cancel(
    ...
    for (struct chimera_smb_vfs_batch *batch = conn->cancel_batches;
        ...
        if (batch->wire->conn_generation != conn->generation ||
            ...
            (async ? (!request->async_id || request->async_id != target_id) :
             request->smb2_hdr.message_id != target_id)) {
            continue;
        }
...
chimera_smb_compound_defer(struct smb_vfs_command *command)
{
    ...
    if (index < command->batch->defer_at) {
        command->batch->defer_at = index;
    }
} /* chimera_smb_compound_defer */
...
chimera_smb_compound_doc_batch_ensure(struct smb_vfs_command *command)
{
    ...
    if (!batch->doc) {
        batch->doc = smb_doc_batch_alloc(batch->count);
    }
    return batch->doc;
} /* chimera_smb_compound_doc_batch_ensure */

bool
chimera_smb_compound_open_closed(
    struct smb_vfs_command             *command,
    const struct chimera_smb_open_file *open)
{
    ...
    for (unsigned int i = 0; i < batch->num_states; i++) {
        if (batch->states[i].open == open) {
            return batch->states[i].closed;
        }
    }
    return false;
} /* chimera_smb_compound_open_closed */
...
bool
chimera_smb_compound_claim_closed(
    struct smb_vfs_command         *command,
...
chimera_smb_compound_directory_isolated(struct smb_vfs_command *command)
{
    ...
    for (unsigned int i = 0; i < batch->num_states; i++) {
        ...
        if (state->producer) {
            if (state->producer >= command || state->handle_from < 0) {
                continue;
            }
        ...
        if (!chimera_smb_namespace_directory_peer_allowed(command->handle->fh,
                                                          command->handle->fh_len, &source_path, handle->fh, handle->
                                                          fh_len,
                                                          &peer_path)) {
            return false;
        }
    ...
    return true;
} /* chimera_smb_compound_directory_isolated */
    ...
    struct chimera_smb_file_id id)
{
    ...
    if (!file->open) {
        ...
        if (pin.input_status) {
            command->input_status = pin.input_status;
        }
        file->handle = file->owned_handle = pin.handle;
        file->actor  = pin.actor;
        if (!file->open || !file->handle) {
            if (file->owned_open) {
                chimera_smb_open_file_release(command->request, file->owned_open);
            }
            free(file);
            return NULL;
        }
...
unsigned int
chimera_smb_compound_file_prepare(
    struct chimera_vfs_compound *compound,
    struct smb_vfs_file         *file)
{
    ...
    if (file->state) {
        ...
        if (!file->state->available || file->state->closed) {
            return SMB2_STATUS_FILE_CLOSED;
        }
    ...
    if (!file->handle || (open->flags & CHIMERA_SMB_OPEN_FILE_CLOSED)) {
        return SMB2_STATUS_FILE_CLOSED;
    }
...
} /* chimera_smb_compound_open_available */
    ...
    void                        *private_data)
{
    ...
    if (command->inherits_file) {
        command->state = NULL;
        command->open  = NULL;
        if (command->initial_state && command->initial_state->producer) {
            command->handle = NULL;
        }
        for (unsigned int i = 0; i < batch->num_states; i++) {
            struct smb_vfs_open_state *state = &batch->states[i];
...
smb_vfs_terminal(struct chimera_smb_vfs_batch *batch)
{
    ...
    while (batch->files) {
        ...
        if (file->owned_handle) {
            chimera_vfs_release(wire->thread->vfs_thread, file->owned_handle);
        }
        if (file->owned_open) {
            chimera_smb_open_file_release(file->command->request, file->owned_open);
        }
        free(file);
    }
...
static void smb_vfs_publish_next(
    struct chimera_smb_vfs_batch *batch);

static void
smb_vfs_publish_done(
    struct smb_vfs_command *command,
    unsigned int            status)
{
    struct chimera_smb_vfs_batch *batch = command->batch;
...
} /* chimera_smb_vfs_compound_cleanup */
    ...
    for (unsigned int i = 0; i < batch->count; i++) {
        ...
        if (!command->ops->creates_open) {
            for (uint32_t oi = checkpoint + 1; oi < chimera_vfs_compound_num_ops(compound); oi++) {
                ...
                if (!binding) {
                    command->input_status = SMB2_STATUS_INSUFFICIENT_RESOURCES; break;
                }
...
smb_vfs_gather_next(struct chimera_smb_vfs_batch *batch)
{
    ...
    while (batch->next_command < batch->count) {
        ...
        if (command->input_status != SMB2_STATUS_SUCCESS) {
            continue;
        }
    ...
    for (int n = wire->complete_requests; n < wire->num_requests; n++) {
        ...
        if (command->ops->creates_open) {
            ...
            if (command->open && command->open->type == CHIMERA_SMB_OPEN_FILE_TYPE_PIPE) {
                if (command->handle) {
                    chimera_vfs_release(wire->thread->vfs_thread, command->handle);
                }
            ...
            if (command->open && !command->state) {
                ...
                for (j = 0; j < batch->num_states; j++) {
                    if (batch->states[j].open == command->open) {
                        break;
                    }
                }
        ...
        if ((wants_doc && (batch->has_waiting_lock || batch->has_other_close_fence)) ||
            ...
            if (command->ops->creates_open) {
                if (command->ops->release) {
                    command->ops->release(command);
                }
                if (command->ops->reply_release) {
                    command->ops->reply_release(command);
                }
        ...
        batch->count++;
        if (command->ops->creates_open) {
            private_producer = command;
        }
        if (command->ops->ends_batch && command->ops->ends_batch(command)) {
            break;
        }
    }
