...
int chimera_smb_vfs_compound_try(struct chimera_smb_compound *compound);
void chimera_smb_vfs_compound_cleanup(struct chimera_smb_compound *compound);
...
bool chimera_smb_vfs_cancel(struct chimera_smb_conn *conn, uint64_t session_id,
    uint64_t target_id, bool async);
void chimera_smb_vfs_cancel_drain(struct chimera_smb_conn *conn);

void chimera_smb_compound_complete_batch(struct chimera_smb_compound *compound,
                                        unsigned int count);
...
struct smb_vfs_command_ops {
    int (*eligible)(struct chimera_smb_request *request);
    /* Construction-only capability gate, after resolving/pinning a handle. */
    int (*bound_eligible)(struct smb_vfs_command *command);
    /* Accepted publication must precede execution of the following command. */
    bool (*ends_batch)(struct smb_vfs_command *command);
    ...
    bool (*private_suffix_eligible)(struct smb_vfs_command *producer,
                                    struct chimera_smb_request *next);
    struct chimera_claim_owner (*private_actor_owner)(struct smb_vfs_command *producer);
    int creates_open;
    int (*initialize)(struct smb_vfs_command *command);
    struct chimera_smb_file_id (*file_id)(struct chimera_smb_request *request);
    unsigned int (*map_error)(enum chimera_vfs_error error);
    int (*build)(struct chimera_vfs_compound *, struct smb_vfs_command *);
    chimera_vfs_compound_op_callback_t prepare;
    chimera_vfs_compound_op_callback_t complete;
    void (*publish)(struct chimera_vfs_compound *, struct smb_vfs_command *);
    ...
    void (*gather_inputs)(struct smb_vfs_command *,
                          void (*done)(struct smb_vfs_command *, unsigned int));
    void (*publish_async)(struct smb_vfs_command *,
                          void (*done)(struct smb_vfs_command *, unsigned int));
    void (*reset)(struct smb_vfs_command *);
    void (*release)(struct smb_vfs_command *);
    void (*reply_release)(struct smb_vfs_command *);
};
...
bool chimera_smb_close_compound_uses_doc(struct smb_vfs_command *command);
extern const struct smb_vfs_command_ops chimera_smb_rename_compound_ops;
void chimera_smb_compound_open_available(struct smb_vfs_command *command);
...
void chimera_smb_compound_defer(struct smb_vfs_command *command);
...
bool chimera_smb_compound_directory_isolated(struct smb_vfs_command *command);
...
struct smb_vfs_file *chimera_smb_compound_pin_file(struct smb_vfs_command *command,
                                                 struct chimera_smb_file_id id);
unsigned int chimera_smb_compound_file_prepare(struct chimera_vfs_compound *compound,
                                              struct smb_vfs_file *file);
const struct smb_vfs_command_ops *chimera_smb_ioctl_compound_ops_for(struct chimera_smb_request *request);
