    ...
    void                     *private_data)
{
    ...
    /* Publish the handle's timestamp policy only after the metadata change
     * succeeds. A failed SET_INFO must not change subsequent write behavior. */
    ...
    if (!error_code && request->set_info.open_file->parent_fh_len > 0) {
        ...
        struct chimera_claim_actor actor = {
            .owner = chimera_smb_open_actor_owner(request->set_info.open_file),
        };
        memcpy(actor.owner.key, request->set_info.open_file->parent_lease_key, 16);
    ...
    void                           *private_data)
{
    ...
    struct chimera_claim_actor actor = {
        .owner = chimera_smb_open_actor_owner(open_file),
    ...
    chimera_vfs_link_at_flags_actor(
        ...
        0, /* notify immediately on the legacy completion */
        0,
        0,
        0,
...
chimera_smb_set_info_size(struct chimera_smb_request *request)
{
    ...
    chimera_vfs_setattr_owned(request->compound->thread->vfs_thread,
                              &request->session_handle->session->cred,
                              open_file->handle,
                              &request->set_info.vfs_attrs, 0, 0, &actor,
                              chimera_smb_set_info_callback, request);
...
static int
smb_ea_resolve_name(char *name, size_t size,
                    const struct chimera_smb_ea_entry *entry,
                    const char *names, uint32_t names_len,
                    const struct smb_ea_name_change *changes, uint32_t count)
{
    ...
    for (uint32_t i = count; i > 0; i--) {
        ...
        if (!chimera_smb_ea_name_eq(change->name + CHIMERA_VFS_XATTR_USER_PREFIX_LEN,
            change->length - CHIMERA_VFS_XATTR_USER_PREFIX_LEN, entry->name, entry->name_len)) { continue; }
        if (!change->removed) {
...
static void
smb_ea_record_name(struct smb_ea_name_change *change,
                   const char *name, uint32_t length, bool removed)
{
...
static void
smb_ea_apply_next(struct chimera_vfs_compound *compound, uint32_t index,
                   enum chimera_vfs_error *status, void *private_data)
{
    ...
    if (done->type == CHIMERA_VFS_COMPOUND_OP_LISTXATTRS) {
        ...
        if (a->list_len) { memcpy(a->list, done->buffer, a->list_len); }
        ...
        *status = CHIMERA_VFS_OK;
        ...
        if (*status != CHIMERA_VFS_OK) { a->status = chimera_smb_ea_status(*status); return; }
    ...
    if (a->ea_off >= a->ea_buf_len ||
        chimera_vfs_compound_num_ops(compound) == CHIMERA_VFS_COMPOUND_MAX_OPS) { return; }
    ...
    int op = entry.value_len ? chimera_vfs_compound_add_setxattr(compound,
        CHIMERA_VFS_XATTR_EITHER, name, length, entry.value, entry.value_len) :
...
static void
smb_ea_apply_reset(struct chimera_vfs_compound *compound, void *private_data)
{
    struct chimera_smb_ea_apply *a = private_data;
    (void) compound;
    a->ea_off = a->chunk_start;
    a->change_count = a->chunk_changes;
    if (!a->chunk_start) { a->list_len = 0; }
    a->status = SMB2_STATUS_SUCCESS;
}

static void smb_ea_apply_start(struct chimera_smb_ea_apply *a);

static void
smb_ea_apply_done(struct chimera_vfs_compound *compound, void *private_data)
{
    struct chimera_smb_ea_apply *a = private_data;
    ...
    void (*done)(uint32_t, void *) = a->done;
    void *arg = a->arg;
...
chimera_smb_ea_apply(
    ...
    struct chimera_vfs_open_handle *handle,
    const uint8_t *ea_buf, uint32_t ea_buf_len,
    void (*done)(uint32_t status, void *arg), void *arg)
{
    if (!(handle->vfs_module->capabilities & CHIMERA_VFS_CAP_XATTR)) {
        done(SMB2_STATUS_EAS_NOT_SUPPORTED, arg);
        return;
    }
    if (!ea_buf_len) { done(SMB2_STATUS_SUCCESS, arg); return; }
    struct chimera_smb_ea_apply *a = calloc(1, sizeof(*a));
    if (!a) { done(SMB2_STATUS_INSUFFICIENT_RESOURCES, arg); return; }
    a->changes = calloc(ea_buf_len / 9 + 1, sizeof(*a->changes));
    if (!a->changes) { free(a); done(SMB2_STATUS_INSUFFICIENT_RESOURCES, arg); return; }
...
smb_set_info_admitted(struct chimera_smb_request *request)
{
    ...
    /* MS-SMB2 3.3.5.21.1: check the information class's required right
     * before touching protocol state, metadata, or namespace. Use resolved
     * granted rights, including the ACL result for MAXIMUM_ALLOWED. */
    uint32_t required_access = 0;
    if (request->set_info.info_type == SMB2_INFO_FILE) {
        switch (request->set_info.info_class) {
            ...
            case SMB2_FILE_ALLOCATION_INFO:
                required_access = SMB2_FILE_WRITE_DATA;
                break;
            case SMB2_FILE_BASIC_INFO:
                required_access = SMB2_FILE_WRITE_ATTRIBUTES;
                break;
            case SMB2_FILE_DISPOSITION_INFO:
            case SMB2_FILE_DISPOSITION_INFO_EX:
            case SMB2_FILE_RENAME_INFO:
                required_access = SMB2_DELETE;
                break;
            case SMB2_FILE_FULL_EA_INFO:
                required_access = SMB2_FILE_WRITE_EA;
                break;
            default:
                break;
        } /* switch */
    }
    if ((request->set_info.open_file->granted_access & required_access) != required_access) {
        chimera_smb_open_file_release(request, request->set_info.open_file);
...
static void
smb_set_info_namespace_poll(struct evpl *evpl, struct evpl_timer *timer)
{
    ...
    if (status == SMB2_STATUS_SUCCESS) smb_set_info_admitted(request);
...
chimera_smb_set_info(struct chimera_smb_request *request)
{
    ...
    if (status == SMB2_STATUS_SUCCESS) { smb_set_info_admitted(request); return; }
    if (status != SMB2_STATUS_PENDING) {
        chimera_smb_complete_request(request, status);
        return;
    }
    ...
    void                        *private_data)
{
    ...
    char name[CHIMERA_VFS_XATTR_NAME_MAX + 1];
    int length = smb_ea_resolve_name(name, sizeof(name), &entry,
        ctx->ea_list_valid ? list->buffer : NULL, list->buffer_len,
    ...
    enum chimera_vfs_error *status, void *private_data)
{
    ...
    if (!path.view_fh_len) { *status = CHIMERA_VFS_EINVAL; return; }
    struct chimera_vfs_compound_op *op = chimera_vfs_compound_op_args(compound, index);
    ...
    struct smb_vfs_command      *command)
{
    ...
    if (request->set_info.info_class == SMB2_FILE_LINK_INFO) {
        ...
        chimera_vfs_compound_set_op_callbacks(compound, parent, NULL,
                                              smb_set_link_parent_complete, command);
        int result = chimera_vfs_compound_add_link(compound, link->new_name, link->new_name_len, 0);
    ...
    int op = chimera_vfs_compound_add_setattr(compound, command->handle, NULL, 0);
    ...
    switch (request->set_info.info_class) {
        ...
        case SMB2_FILE_FULL_EA_INFO: {
            ...
            while (offset < request->set_info.ea_buf_len) {
                needed++;
                if (chimera_smb_ea_full_parse_one(request->set_info.ea_buf,
                        request->set_info.ea_buf_len, &offset, &entry)) { break; }
            }
