    ...
    const struct chimera_smb_namespace_path *after)
{
    ...
    struct smb_doc_path_check              check = { .path = record,            .command
                                                           =
                                                             command,
                                                     .locked_file = event->file->file, .valid
                                                                  =
                                                             true };
    struct chimera_smb_namespace_registry *registry = event->file->fence.registry;
    chimera_smb_namespace_lock(registry);
    evpl_mutex_lock(&event->file->file->lock);
    ...
    evpl_mutex_unlock(&event->file->file->lock);
...
smb_doc_seed(struct smb_doc_event *event)
{
    ...
    chimera_smb_namespace_lock(registry);
    evpl_mutex_lock(&slot->file->lock);
    ...
    slot->seeded = true;
    evpl_mutex_unlock(&slot->file->lock);
    chimera_smb_namespace_unlock(registry);
} /* smb_doc_seed */
    ...
    struct smb_vfs_command      *command)
{
    ...
    int                   remove = chimera_vfs_compound_add_remove(compound, "doc", 3, CHIMERA_VFS_REMOVE_NO_NOTIFY, 0,
                                                                   0);
