...
bool
chimera_smb_open_namespace_pending_begin(struct chimera_server_smb_thread *thread,
    struct chimera_smb_open_file *open, const uint8_t *root_fh, uint32_t root_fh_len)
{
    if (!open->namespace_participant || !root_fh_len || root_fh_len > CHIMERA_VFS_FH_SIZE ||
        open->parent_fh_len > CHIMERA_VFS_FH_SIZE || open->name_len > SMB_FILENAME_MAX ||
        open->full_path_len >= SMB_PATH_MAX) { return false; }
    ...
    return chimera_smb_namespace_pending_begin(&thread->shared->namespace_registry,
        open->base_namespace_participant ? open->base_namespace_participant : open->namespace_participant, &initial);
}

bool
chimera_smb_open_namespace_pending_bind(struct chimera_smb_open_file *open,
    struct chimera_vfs_open_handle *handle)
{
...
bool
chimera_smb_open_namespace_pending_path(struct chimera_smb_open_file *open,
    struct chimera_smb_namespace_path *path)
{
    if (open->namespace_registered) { return false; }
