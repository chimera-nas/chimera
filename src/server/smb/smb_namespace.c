...
chimera_smb_namespace_init(struct chimera_smb_namespace_registry *registry)
{
    ...
    evpl_mutex_init(&registry->lock, NULL);
...
chimera_smb_namespace_destroy(struct chimera_smb_namespace_registry *registry)
{
    ...
    evpl_mutex_destroy(&registry->lock);
...
{
    assert(owner);
    evpl_mutex_lock(&registry->lock);
    if (token->registry) {
        bool same = token->registry == registry && token->owner == owner;
        evpl_mutex_unlock(&registry->lock);
        return same;
    }
...
chimera_smb_namespace_open_end(struct chimera_smb_namespace_open_token *token)
{
    ...
    *p = token->next;
    ...
    evpl_mutex_unlock(&registry->lock);
} /* chimera_smb_namespace_open_end */

bool
chimera_smb_namespace_replace_begin(
...
chimera_smb_namespace_replace_end(struct chimera_smb_namespace_replace_token *token)
{
    ...
    *p = token->next;
    memset(token, 0, sizeof(*token));
    evpl_mutex_unlock(&registry->lock);
} /* chimera_smb_namespace_replace_end */

bool
chimera_smb_namespace_identity_present_locked(
    struct chimera_smb_namespace_registry *registry,
    const uint8_t                         *fh,
    uint32_t                               fh_len)
{
    for (struct chimera_smb_namespace_participant *p = registry->participants; p; p = p->next) {
        if (identity(p->fh, p->fh_len, fh, fh_len)) {
            return true;
        }
    }
    return false;
...
bool
chimera_smb_namespace_directory_peer_allowed(
    const uint8_t                           *source_fh,
    uint32_t                                 source_fh_len,
    const struct chimera_smb_namespace_path *source_path,
    const uint8_t                           *peer_fh,
    uint32_t                                 peer_fh_len,
    const struct chimera_smb_namespace_path *peer_path)
{
    if (!peer_fh_len || !path_valid(peer_path) || !same_view(source_path, peer_path)) {
        return false;
    }
    if (identity(source_fh, source_fh_len, peer_fh, peer_fh_len) ||
        identity(source_path->view_fh, source_path->view_fh_len, peer_fh, peer_fh_len)) {
        return true;
    }
...
bool
chimera_smb_namespace_directory_isolated_locked(
    struct chimera_smb_namespace_registry   *registry,
    const uint8_t                           *source_fh,
    uint32_t                                 source_fh_len,
    ...
    for (struct chimera_smb_namespace_participant *p = registry->participants; p; p = p->next) {
        ...
        if ((p->pending && !p->bound) || p->invalid_path ||
            !chimera_smb_namespace_directory_peer_allowed(source_fh, source_fh_len,
                                                          path, p->fh, p->fh_len, &p->snapshot.path)) {
            return false;
        }
    }
    return true;
...
bool
chimera_smb_namespace_rebind_locked(
    struct chimera_smb_namespace_participant *p,
    const uint8_t                            *old_fh,
    uint32_t                                  old_fh_len,
    const uint8_t                            *new_fh,
    uint32_t                                  new_fh_len)
{
    if (!p || !p->registry || p->pending || !new_fh || !new_fh_len ||
        new_fh_len > CHIMERA_VFS_FH_SIZE || !identity(p->fh, p->fh_len, old_fh, old_fh_len)) {
        return false;
    }
    ...
    const struct chimera_smb_namespace_edit  *own_guard)
{
    ...
    if (valid && ((own_guard && (own_guard->registry != registry ||
                  ...
                  busy(registry, fh, fh_len, own_guard))) {
        valid = false;
    }
    if (valid) {
        ...
        * both fully attached identities, never a half-published pair. */
        ...
        if (base->publish) {
            base->publish(base->context, &base->snapshot);
        }
        base->pending          = false;
...
{
    evpl_mutex_lock(&p->registry->lock);
    p->snapshot.closing = true;
    evpl_mutex_unlock(&p->registry->lock);
...
chimera_smb_namespace_edit_try_acquire(struct chimera_smb_namespace_edit *edit)
{
    ...
    evpl_mutex_lock(&registry->lock);
    if (edit->acquired) {
        evpl_mutex_unlock(&registry->lock); return true;
    }
    if (edit->global && registry->active) {
        evpl_mutex_unlock(&registry->lock); return false;
    }
    for (uint32_t i = 0; i < edit->object_count; i++) {
    ...
    const uint8_t                            parent_lease_key[16])
{
    ...
    evpl_mutex_lock(&edit->registry->lock);
    for (struct chimera_smb_namespace_participant *p = edit->registry->participants; p; p = p->next) {
        ...
        if (same_link(projected, &object_path->current) && !same_view(projected, next)) {
            evpl_mutex_unlock(&edit->registry->lock);
            return -1;
        }
    }
    evpl_mutex_unlock(&edit->registry->lock);
    ...
    uint32_t                                  fh_len)
{
    ...
    evpl_mutex_lock(&p->registry->lock);
    if (!p->pending) {
        evpl_mutex_unlock(&p->registry->lock); return false;
    }
