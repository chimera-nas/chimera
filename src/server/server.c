...
chimera_mkpath_open_child(struct chimera_mkpath_ctx *ctx)
{
    chimera_vfs_open_fh(ctx->thread, chimera_vfs_get_server_cred(),
                        chimera_vfs_compound_loose(ctx->thread), /* standalone init-time mkpath; loose */
    ...
    void                     *private_data)
{
    ...
    if (error_code == CHIMERA_VFS_ENOENT) {
        ...
        chimera_vfs_mkdir_at(ctx->thread, chimera_vfs_get_server_cred(),
                             chimera_vfs_compound_loose(ctx->thread), /* standalone init-time mkpath; loose */
                             ctx->oh,
...
chimera_mkpath_next(struct chimera_mkpath_ctx *ctx)
{
    ...
    chimera_vfs_lookup_at(ctx->thread, chimera_vfs_get_server_cred(),
                          chimera_vfs_compound_loose(ctx->thread), /* standalone init-time mkpath; loose */
                          ctx->oh,
    ...
    void                      *private_data)
{
    ...
    chimera_vfs_open_fh(thread, chimera_vfs_get_server_cred(),
                        chimera_vfs_compound_loose(thread), /* standalone init-time mkpath; loose */
    ...
    void                     *private_data)
{
    ...
    chimera_vfs_symlink_at(ctx->thread, chimera_vfs_get_server_cred(),
                           chimera_vfs_compound_loose(ctx->thread), /* standalone init-time seed; loose */
    ...
    void                      *private_data)
{
    ...
    chimera_vfs_open_fh(thread, chimera_vfs_get_server_cred(),
                        chimera_vfs_compound_loose(thread), /* standalone init-time seed; loose */
                        m->root_fh, m->root_fh_len,
                        CHIMERA_VFS_OPEN_DIRECTORY | CHIMERA_VFS_OPEN_PATH,
    ...
    void                     *private_data)
{
    ...
    chimera_vfs_open_at(ctx->thread, chimera_vfs_get_server_cred(),
                        chimera_vfs_compound_loose(ctx->thread), /* standalone init-time seed; loose */
                        ctx->root_oh,
                        "ExistingFile.txt", 16,
