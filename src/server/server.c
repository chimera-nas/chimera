    ...
    void *data)
{
    ...
    {
        thread->protocol_private[i] = server->protocols[i]->thread_init(evpl, server);
    }
...
chimera_server_init(const char *cfgfile)
{
    ...
    server->pool = evpl_threadpool_create(1, chimera_server_thread_init, NULL, chimera_server_thread_destroy, server);
