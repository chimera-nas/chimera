...
chimera_nfs3_readlink_reply(struct nfs_request *req)
{
    ...
    if (req->compound_op_status != CHIMERA_VFS_OK) {
        req->res_readlink.status = chimera_vfs_error_to_nfsstat3(req->compound_op_status)
        ;
    ...
    void                     *private_data)
{
    ...
    if (error_code == CHIMERA_VFS_OK) {
         ...
         * protocol layer rather than relying on every backend to map a
         * non-symlink to EINVAL.  The type comes from the attrs already
         * fetched, so no extra round-trip. */
    ...
    void                           *private_data)
{
    ...
    chimera_vfs_readlink(req->thread->vfs_thread, &req->cred, req->compound,
                         handle,
                         res->resok.data.str,
                         res->resok.data.len,
