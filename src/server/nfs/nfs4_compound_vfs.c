    ...
    uint32_t                     name_len)
{
    ...
    int           lookup = chimera_vfs_compound_add_lookup(compound, name, name_len, CHIMERA_VFS_ATTR_FH);
        ...
        switch (argop->argop) {
            ...
            case OP_READ_PLUS:
            {
                ...
                idx = chimera_vfs_compound_add_read(
                    compound, NULL, argop->opread_plus.rpa_offset,
                    argop->opread_plus.rpa_count, riov, NFS4_VFS_READ_MAX_IOV, NULL);
                ...
                * one re-opens the current object and needs nothing first. */
                ...
                if (map->reserved_open && idx >= 0) {
                    ...
                    chimera_vfs_compound_set_op_callbacks(compound, idx,
                                                          ...
                                                          map);
                    idx = chimera_vfs_compound_add_open(compound, NULL, 0, 0, 0, NULL, 0);
                    if (idx < 0) {
                        goto refuse;
                    }
                    chimera_vfs_compound_set_op_callbacks(compound, idx,
                    ...
                    if (map->open_trunc_if_existed) {
                        ...
                        trunc.va_size     = 0;
                        idx               = chimera_vfs_compound_add_setattr(compound, NULL, &trunc, 0);
                        if (idx < 0) {
                            goto refuse;
                        }
                        chimera_vfs_compound_set_op_callbacks(compound, idx,
            ...
            case OP_READDIR:
                ...
                idx = chimera_vfs_compound_add_readdir_stream(
                    ...
                    chimera_nfs4_attr2mask(argop->opreaddir.attr_request,
                                           argop->opreaddir.num_attr_request) | CHIMERA_VFS_ATTR_FH,
                    nfs4_vfs_readdir_reset,
                    nfs4_vfs_readdir_append,
                    ctx);
                map->vfs_res = idx;
            ...
            case OP_CLONE:
            {
                ...
                map->range_src_open = chimera_vfs_compound_add_open(compound, NULL, 0,
                                                                    NFS4_VFS_OPEN_DATA | CHIMERA_VFS_OPEN_READ_ONLY, 0,
                                                                    NULL, 0);
                ...
                map->range_dst_open = chimera_vfs_compound_add_open(compound, NULL, 0,
                                                                    NFS4_VFS_OPEN_DATA | CHIMERA_VFS_OPEN_WRITE_ONLY, 0,
                                                                    NULL, 0);
