    ...
    void                  *private_data)
{
    ...
    ectx->value_len = nfs_recovery_epoch_serialize(ectx->value,
                                                   rec->current_boot_id);
    chimera_vfs_put_key(ctx->thread->vfs_thread, chimera_vfs_compound_loose(ctx->thread->vfs_thread), ectx->key, ectx->
                        key_len,
...
nfs_recovery_kickoff(struct chimera_server_nfs_thread *thread)
{
     ...
     * the delegation thread reads it after this function returns. */
    {
        ...
        chimera_vfs_get_key(thread->vfs_thread, chimera_vfs_compound_loose(thread->vfs_thread), ctx->ekey, ekey_len,
