    ...
    void                           *private_data)
{
    ...
    chimera_vfs_remove_at(ctx->req->thread->vfs_thread, &ctx->req->cred, ctx->req->compound,
                          ctx->ds_root_handle,
    ...
    void                     *private_data)
{
    ...
    chimera_vfs_open_fh(ctx->req->thread->vfs_thread, &ctx->req->cred, ctx->req->compound,
...
nfs4_remove_mds(struct nfs4_remove_ctx *ctx)
{
    ...
    chimera_vfs_remove_at(req->thread->vfs_thread, &req->cred, req->compound,
                          ctx->parent_handle,
