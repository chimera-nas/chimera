    ...
    void                           *private_data)
{
    ...
    if (error_code != CHIMERA_VFS_OK) {
         ...
         * choice in chimera_nfs3_setattr_start), and a non-regular object
         * refuses one: ELOOP for a symlink, ENXIO for a device or socket.
         * RFC 1813 says SETATTR's size is meaningful only for a regular file
         * and any other type answers NFS3ERR_INVAL, which is what the
         * in-engine backends return from their own setattr -- they accept the
         * open and reject the size.  Without this the passthrough backends
         * failed at the open instead, and the unmapped errno surfaced as
