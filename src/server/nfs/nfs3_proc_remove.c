    ...
    void                     *private_data)
{
    ...
    * the FH in req->saved_fh: req->fh holds the decoded parent-dir FH that a
    * transaction-conflict replay re-opens, so it must not be clobbered. */
    if (error_code == CHIMERA_VFS_OK &&
        (attr->va_set_mask & CHIMERA_VFS_ATTR_FH)) {
        memcpy(req->saved_fh, attr->va_fh, attr->va_fh_len);
        req->saved_fhlen = attr->va_fh_len;
        chimera_nfs3_remove_dispatch(req, req->saved_fh, req->saved_fhlen);
    } else {
