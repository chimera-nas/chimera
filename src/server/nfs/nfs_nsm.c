    ...
    uint32_t                   state_number)
{
    ...
    ctx->value_len = nsm_state_value_serialize(ctx->value, sizeof(ctx->value),
                                               state_number);
    chimera_vfs_put_key(vfs_thread, chimera_vfs_compound_loose(vfs_thread),
                        ctx->key, ctx->key_len,
    ...
    const char                       *peer_addr)
{
    ...
    ctx->value_len = nsm_monitor_value_serialize(ctx->value, sizeof(ctx->value),
                                                 peer_addr);
    chimera_vfs_put_key(thread->vfs_thread,
    ...
    const char                       *host)
{
    ...
    ctx->key_len = nfs_kv_nsm_monitor_key(ctx->key, (const uint8_t *) host,
                                          host_len);
    chimera_vfs_delete_key(thread->vfs_thread,
                           chimera_vfs_compound_loose(thread->vfs_thread),
                           ctx->key, ctx->key_len,
                           nsm_kv_done, ctx);
} /* nsm_unmonitor */
...
chimera_nfs_nsm_kickoff(struct chimera_server_nfs_thread *thread)
{
    ...
    chimera_vfs_get_key(thread->vfs_thread,
                        chimera_vfs_compound_loose(thread->vfs_thread),
                        ctx->skey, klen,
