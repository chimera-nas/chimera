    ...
    void                        *private_data)
{
    ...
    ctx->read = chimera_vfs_compound_add_read(compound, NULL, ctx->offset,
                                              count, ctx->io->iov, CHIMERA_S3_IOV_MAX, NULL);
    ...
    struct chimera_s3_request *request)
{
    ...
    chimera_vfs_compound_add_read(compound, request->file_handle,
                                  request->file_cur_offset, count,
                                  io->iov, CHIMERA_S3_IOV_MAX, NULL);
