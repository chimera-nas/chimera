    ...
    struct chimera_s3_part          *part)
{
    ...
    chimera_vfs_compound_add_open_current(compound,
                                          ...
                                          , 0);
    chimera_vfs_compound_add_remove(compound, part->tmp_name, part->tmp_name_len, 0);
...
{
    if (ctx->count) {
        int index = chimera_vfs_compound_add_write(compound, ctx->attempt_file,
                                                   ctx->offset, ctx->count, 1, &ctx->data, 1, NULL);
    ...
    void                        *private_data)
{
    ...
    if (ctx->unnamed) {
        ctx->file_result = chimera_vfs_compound_add_create_unlinked(
            compound, &attrs, CHIMERA_VFS_ATTR_FH);
    } else {
        ctx->file_result = chimera_vfs_compound_add_open(compound,
                                                         ctx->request->multipart.tmp_name, strlen(ctx->request->
                                                                                                  multipart.tmp_name),
                                                         CHIMERA_VFS_OPEN_CREATE, 0, &attrs, CHIMERA_VFS_ATTR_FH);
    }
    ...
    void                        *private_data)
{
    ...
    write_index = chimera_vfs_compound_add_write(compound, ctx->attempt_file,
                                                 ctx->attempt_copied, op->read_len, 1, op->iov, op->niov, NULL);
    ...
    struct chimera_s3_upload_copy_ctx *ctx)
{
    ...
    if (ctx->mode == CHIMERA_S3_UPC_COPY) {
        ...
        index = chimera_vfs_compound_add_read(compound, ctx->attempt_src,
                                              ctx->src_first + ctx->attempt_copied, count,
    ...
    void                        *private_data)
{
    ...
    if (module && (module->capabilities & CHIMERA_VFS_CAP_CREATE_UNLINKED)) {
        ...
        ctx->file_result          = chimera_vfs_compound_add_open(compound,
                                                                  request->multipart.tmp_name, ctx->attempt_tmp_name_len
                                                                  ,
                                                                  CHIMERA_VFS_OPEN_CREATE, 0, &attrs,
                                                                  CHIMERA_VFS_ATTR_FH);
    }
    chimera_vfs_compound_set_op_callbacks(compound, ctx->file_result, NULL,
    ...
    struct chimera_s3_complete_ctx *ctx)
{
    ...
    if (ctx->attempt_tmp_name_len) {
        ...
        index = chimera_vfs_compound_add_rename(
            compound, request->multipart.tmp_name, ctx->attempt_tmp_name_len,
    ...
    void                        *private_data)
{
    ...
    write_index            = chimera_vfs_compound_add_write(
        compound, ctx->attempt_file, ctx->attempt_write_offset,
        op->read_len, 1, op->iov, op->niov, NULL);
    chimera_vfs_compound_set_op_callbacks(compound, write_index, NULL,
    ...
    struct chimera_s3_complete_ctx *ctx)
{
    ...
    if (ctx->attempt_mode == CHIMERA_S3_ASSEMBLE_COPY) {
        ...
        index                  = chimera_vfs_compound_add_read(
            compound, part->file_handle, ctx->attempt_part_offset,
            chunk, ctx->rw_iov, CHIMERA_S3_IOV_MAX, NULL);
    }
    ...
    void                        *private_data)
{
    ...
    if (module && (module->capabilities & CHIMERA_VFS_CAP_CREATE_UNLINKED)) {
        ...
        ctx->attempt_tmp_name_len = strlen(request->multipart.tmp_name);
        ctx->file_result          = chimera_vfs_compound_add_open(
            compound, request->multipart.tmp_name, ctx->attempt_tmp_name_len,
            CHIMERA_VFS_OPEN_CREATE, 0, &attrs, CHIMERA_VFS_ATTR_FH);
    }
    chimera_vfs_compound_set_op_callbacks(compound, ctx->file_result, NULL,
