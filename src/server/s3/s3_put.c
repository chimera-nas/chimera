    ...
    struct s3_put_transfer      *ctx)
{
    ...
    if (ctx->count) {
        index = chimera_vfs_compound_add_write(compound, ctx->attempt_file, ctx->offset, ctx->count, 1, &ctx->data, 1, 0
                                               , 0, NULL);
        if (index < 0) {
            return -1;
        }
    ...
    if (ctx->unnamed) {
        ...
        chimera_vfs_compound_add_putfh(compound, ctx->attempt_directory->fh,
                                       ctx->attempt_directory->fh_len);
        chimera_vfs_compound_add_savefh(compound);
        index = chimera_vfs_compound_add_rename(compound, ctx->temporary, strlen(ctx->temporary), ctx->name, strlen(ctx
                                                                                                                    ->
                                                                                                                    name),
                                                0, 0, 0);
    }
    ...
    void                        *private_data)
{
    ...
    if (ctx->unnamed) {
        created = chimera_vfs_compound_add_create_unlinked(compound, CHIMERA_VFS_OPEN_INFERRED, &ctx->create_attr,
                                                           CHIMERA_VFS_ATTR_FH);
    } else {
