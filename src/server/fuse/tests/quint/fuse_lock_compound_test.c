...
static evpl_mutex_t fixture_mutex = EVPL_MUTEX_INITIALIZER;
static evpl_cond_t  fixture_cond  = PTHREAD_COND_INITIALIZER;
...
wait_held(void)
{
    ...
    deadline.tv_sec += 10;
    evpl_mutex_lock(&fixture_mutex);
    while (!atomic_load(&fixture_held)) {
        CHECK(chimera_cond_timedwait(&fixture_cond, &fixture_mutex, &deadline) == 0);
    }
    evpl_mutex_unlock(&fixture_mutex);
} /* wait_held */
    ...
    void                        *private_data)
{
    ...
    if (atomic_exchange(&fixture_pause, false)) {
        ...
        evpl_cond_broadcast(&fixture_cond);
        evpl_mutex_unlock(&fixture_mutex);
        return;
    }
    if (left) {
